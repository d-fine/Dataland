...
class TemplateComponentBuilder(
    ...
    return if (row.category.isBlank()) {
      ...
      val section =
        base.getOrNull<ComponentGroup>(sectionName)
          ?: base.create(sectionName) { label = row.category }
      ...
      if (row.subCategory.isBlank()) {
        ...
        val subsection =
          section.getOrNull<ComponentGroup>(subsectionName)
            ?: section.create(subsectionName) { label = row.subCategory }
  ...
  private fun initialBuildPassForCreatingComponents(
    into: ComponentGroupApi
  ): Map<String, ComponentBase> {
  ...
  private fun secondBuildPassForDefiningDependencies(
  ...
  /**
   * Use the Excel template to build a high-level intermediate representation of a framework into
   * the provided ComponentGroup
   */
