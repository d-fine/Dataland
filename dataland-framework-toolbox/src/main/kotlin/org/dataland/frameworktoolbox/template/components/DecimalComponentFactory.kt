...
@Component
class DecimalComponentFactory(@Autowired val templateDiagnostic: TemplateDiagnostic) :
  TemplateComponentFactory {

  companion object {
    /**
     * Pareses bounds for a numeric variable given in the format [LOWER BOUND, UPPER BOUND]. An
     * "INF" may be used to specify that there is no bound
     */
    fun parseBounds(input: String): Pair<Long?, Long?> {
      if (input.isBlank()) return Pair(null, null)

      val pattern =
        """Allowed Range:\s*\[\s*(?<lower>(?:\-?\d+|-INF))\s*,\s*(?<upper>(?:\-?\d+|INF))\s*\]"""
          .toRegex()
      val matchResult =
        pattern.find(input)
  ...
  override fun generateComponent(
    ...
    return componentGroup.create<DecimalComponent>(utils.generateFieldIdentifierFromRow(row)) {
