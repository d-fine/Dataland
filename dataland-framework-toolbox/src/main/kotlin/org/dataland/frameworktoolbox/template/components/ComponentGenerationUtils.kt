...
/**
 * A utility class used during Template conversion. Can be overridden / replaced to cater to
...
open class ComponentGenerationUtils {
  ...
  open fun getSelectionOptionsFromOptionColumn(row: TemplateRow): Set<SelectionOption> {
    val stringOptions = row.options.split("|").map { it.trim() }

    val mappedOptions =
      stringOptions
        .map {
          SelectionOption(identifier = Naming.getNameFromLabel(it).capitalizeEn(), label = it)
        }
        .toSet()
