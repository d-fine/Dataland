    ...
    fun fromFile(file: File): ExcelTemplate {
      return when (file.extension) {
        ...
        "csv" -> fromCsv(file)
          ...
          throw IllegalArgumentException("Can only parse CSV and XLSX files. Got ${file.name}.")
      }
    }

    /** Parse an Excel Template from a xlsx file. */
    fun fromXlsx(xlsxFile: File): ExcelTemplate {
      val targetCsvFile = xlsxFile.parentFile.resolve("${xlsxFile.nameWithoutExtension}.csv")
