...
/**
 * A EsgQuestionnaireYearlyDecimalTimeseriesDataComponent is an in-memory representation of a
 * generic field that encodes several values across a span of multi years. It is displayed / upload
 * in a matrix
 */
class EsgQuestionnaireYearlyDecimalTimeseriesDataComponent(
  identifier: String,
  ...
  override fun generateDefaultDataModel(dataClassBuilder: DataClassBuilder) {
    require(decimalRows.isNotEmpty()) {
      "Please add at least one decimal row to this EsgQuestionnaireYearlyDecimalTimeseriesDataComponent " +
    ...
    for (decimalRow in decimalRows) {
      fieldDataClass.addProperty(decimalRow.identifier, TypeReference("java.math.BigDecimal", true))
    }
  ...
  override fun generateDefaultViewConfig(sectionConfigBuilder: SectionConfigBuilder) {
    ...
    for (row in decimalRows) {
      configurationObject[row.identifier] =
        mapOf("label" to row.label, "unitSuffix" to row.unitSuffix)
    }
  ...
  override fun generateDefaultFixtureGenerator(sectionBuilder: FixtureSectionBuilder) {
    ...
    val jsIdentifierArray =
      "[${decimalRows.joinToString(", ")
