...
/**
 * A PavedRoadFramework is the simplest way to integrate a new framework into Dataland or update an
 * existing one! It provides a template for implementing frameworks.
 */
...
abstract class PavedRoadFramework(
  ...
  val framework =
    Framework(identifier = identifier, label = label, explanation = explanation, order = order)
  ...
  /**
   * Can be overwritten to programmatically customize the excelTemplate (to e.g, add a prefix to all
   * fields)
   */
  ...
  /**
   * Retrieve the ComponentGenerationUtils instance for this framework. Can be overwritten to supply
   * a custom, framework-specific instance (to e.g., customize the field-name generation)
   */
  ...
  /**
   * Retrieve a list of TemplateComponentFactories that are responsible for converting template
   * rows. Can be overwritten to e.g., insert factories for framework-specific components
   */
  ...
  /**
   * Can be overwritten to programmatically customize the framework representation (to e.g, add new
   * fields)
   */
  ...
  /**
   * Can be overwritten to programmatically customize the dataModel (to e.g, change the JVM type of
   * certain fields)
   */
  ...
  /**
   * Can be overwritten to programmatically customize the QA dataModel (to e.g, change the JVM type
   * of certain fields)
   */
  ...
  /**
   * Can be overwritten to programmatically customize the viewModel (to e.g, change the way certain
   * fields are displayed in the frontend)
   */
  ...
  /**
   * Can be overwritten to programmatically customize the uploadModel (to e.g, change the way
   * certain fields are displayed in the frontend)
   */
  ...
  /**
   * Can be overwritten to programmatically customize the fixtureGenerator (to e.g, change the
   * probabilities for certain outcomes of the fixture generation)
   */
  ...
  private fun compileDataModel(datalandProject: DatalandRepository) {
    ...
    dataModel.build(
      ...
      buildApiController =
        enabledFeatures.contains(FrameworkGenerationFeatures.BackendApiController),
  ...
  private fun compileQaModel(datalandProject: DatalandRepository) {
    ...
    qaModelBuilder.build(into = datalandProject)
  }
  ...
  open fun compileFramework(datalandProject: DatalandRepository) {
    ...
    datalandProject.gradleInterface.executeGradleTasks(
      listOf(":dataland-frontend:npm_run_typecheck")
    )
    diagnostics.finalizeDiagnosticStream()
