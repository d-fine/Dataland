...
abstract class PavedRoadFramework(
    ...
    protected fun compileDataModel(datalandProject: DatalandRepository) {
        ...
        addSupressAnnotationToPackageBuilder(dataModel.rootPackageBuilder, "\"MaxLineLength\"")
    ...
    /**
     * Adds an annotation to suppress MaxLineLength linting error in the autogenerated files
     * that is otherwise throw e.g. due to long descriptions
     */
    fun addSupressAnnotationToPackageBuilder(
        packageBuilder: PackageBuilder,
        rawParameterSpec: String,
        classesToAnnotate: List<String>? = null,
    ) {
        packageBuilder.childElements.forEach { dataModelElement ->
            when (dataModelElement) {
                is PackageBuilder -> {
                    addSupressAnnotationToPackageBuilder(dataModelElement, rawParameterSpec, classesToAnnotate)
                }
    ...
    /**
     * Adds an annotation to suppress a given linting error in the autogenerated files
     * that is otherwise throw e.g. due to long descriptions
     */
    private fun addSuppressAnnotation(
        dataModelElement: DataClassBuilder,
        rawParameterSpec: String,
        classesToAnnotate: List<String>?,
    ) {
        val fullyQualifiedName = "Suppress"

        if (classesToAnnotate.isNullOrEmpty() || classesToAnnotate.contains(dataModelElement.fullyQualifiedName)) {
            ...
            if (index >= 0) {
                ...
                dataModelElement.annotations[index] =
                    Annotation(
                        fullyQualifiedName,
                        "${oldAnnotation.rawParameterSpec}, $rawParameterSpec",
                    )
            } else {
