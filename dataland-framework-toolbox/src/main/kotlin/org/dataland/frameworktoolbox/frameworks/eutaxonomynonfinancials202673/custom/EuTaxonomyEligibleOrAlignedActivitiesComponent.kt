...
class EuTaxonomyEligibleOrAlignedActivitiesComponent(
                ...
                setOf(
                    TypeScriptImport(
                        ...
                        "@/components/resources/dataTable/conversion/" +
                            "EuTaxonomyNonFinancialsEligibleOrAlignedActivitiesDataGetterFactory",
                    ),
                ),
            ),
        )
                ...
                listOf(
                    TypeReference(
                        "org.dataland.datalandbackend.openApiClient.model" +
                            ".ExtendedDataPointListEuTaxonomyEligibleOrAlignedActivity",
                        isNullable,
                    ),
                ),
            ),
        ...
        val fixtureExpression =
            if (isNullable) {
                "dataGenerator.randomExtendedDataPoint(dataGenerator.randomArray(() => " +
                    "dataGenerator.generateEligibleOrAlignedActivity(), 0, 10))"
            } else {
    ...
    override fun generateDefaultSpecification(specificationCategoryBuilder: CategoryBuilder) {
        ...
        specificationCategoryBuilder.addDefaultDatapointAndSpecification(
            this,
            "EuTaxonomyEligibleOrAlignedActivitiesComponent",
        )
    }
}
