...
/**
 * A hierarchical group of subelements with identifiers. Allows users to manipulate (CRUD)
 * subelements using their identifier. Uses Kotlin's Type-Safe-Builders to allow for DSL-Like
 * intuitive manipulation of Elements
 */
interface ComponentGroupApi {
  /**
   * Create a new subcomponent. Throws an exception when the identifier is already in use or the
   * provided class cannot be constructed.
   */
  ...
  /**
   * Edit an existing subcomponent. Throws an exception when the identifier is not existent or the
   * identified element has a different type than specified
   */
  fun <T : ComponentBase> edit(identifier: String, clazz: KClass<T>, editFunction: T.() -> Unit): T

  /**
   * Get an existing subcomponent or null. Throws an exception when the identified element has a
   * different type than specified
   */
  fun <T : ComponentBase> getOrNull(identifier: String, clazz: KClass<T>): T?

  /**
   * Get an existing subcomponent. Throws an exception when the identified element has a different
   * type than specified or does not exist.
   */
  fun <T : ComponentBase> get(identifier: String, clazz: KClass<T>): T

  /**
   * Delete an existing subcomponent. Throws an exception when the identifier is not existent or the
   * identified element has a different type than specified.
   */
...
/**
 * Get an existing subcomponent. Throws an exception when the identified element has a different
 * type than specified or does not exist.getOrNull
 */
