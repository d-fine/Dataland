...
/**
 * A generic implementation of the ComponentGroupApi. This class is intended to provide the backbone
 * for components that require dynamic subcomponents (e.g., Sections or Lists of Objects)
 */
class ComponentGroupApiImpl(var parent: FieldNodeParent? = null) : ComponentGroupApi {
  ...
  private fun <T : ComponentBase> initComponent(
    ...
    if (insertBeforeIdentifier == null) {
      ...
      val indexOfInsertBeforeIdentifier =
        components.indexOfFirst { it.identifier == insertBeforeIdentifier }
  ...
  /**
   * Searches for a suitable constructor of the provided class. A constructor is suitable if it has
   * two required arguments (1: Identifier as a string, 2: Parent as a FieldNodeParent)
   */
    ...
    for (constructor in componentConstructors) {
      ...
      if (validConstructor) {
        return { identifier, parent ->
          constructor.callBy(
            mapOf(nonOptimalParameters[0] to identifier, nonOptimalParameters[1] to parent)
          )
        }
      }
    }
  ...
  override fun <T : ComponentBase> create(
    ...
    requireNotNull(localParent) {
      "Cannot initialize a new component without an initialized parent."
    }

    val componentConstructor = getComponentConstructorByReflection(clazz)
    requireNotNull(componentConstructor) {
      "Could not find any suitable constructor for creating $clazz"
    }
