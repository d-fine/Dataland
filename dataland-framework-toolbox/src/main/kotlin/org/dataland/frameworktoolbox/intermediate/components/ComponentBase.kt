...
/**
 * A component is a higher-level abstraction for framework elements. Components are arranged in a
 * hierarchy and typically correspond to a single question in a questionnaire.
 *
...
@ComponentMarker
open class ComponentBase(var identifier: String, override var parent: FieldNodeParent) :
  ...
  /**
   * The explanation of a component is a longer description of the component. This variant will be
   * displayed on the upload page.
   */
  var uploadPageExplanation: String? = null

  /**
   * The explanation of a component is a longer description. If set, it will overwrite the
   * explanation of the upload page. If unset, it will default to the upload page explanation.
   */
  var viewPageExplanation: String? = null

  /**
   * The dataModelGenerator allows users to overwrite the DataClass generation of this specific
   * component instance
   */
  var dataModelGenerator: ((dataClassBuilder: DataClassBuilder) -> Unit)? = null

  /**
   * The qaModelGenerator allows users to overwrite the DataClass generation of this specific
   * component instance for the QA model
   */
  var qaModelGenerator: ((dataClassBuilder: DataClassBuilder) -> Unit)? = null

  /**
   * The viewConfigGenerator allows users to overwrite the ViewConfig generation of this specific
   * component instance
   */
  var viewConfigGenerator: ((sectionConfigBuilder: SectionConfigBuilder) -> Unit)? = null

  /**
   * The uploadConfigGenerator allows users to overwrite the UploadConfig generation of this
   * specific component instance
   */
  var uploadConfigGenerator: ((uploadCategoryBuilder: UploadCategoryBuilder) -> Unit)? = null

  /**
   * The fixtureGeneratorGenerator allows users to overwrite the FixtureGeneration generation of
   * this specific component isntance
   */
      ...
      return parents()
        ...
        .mapNotNull {
          when (it) {
            ...
            is TopLevelComponentGroup ->
              Naming.getNameFromLabel(it.parent.identifier).capitalizeEn()
            else -> null
          }
        }
  ...
  /**
   * Build this component instance into the provided Kotlin DataClass using the default generator
   */
  ...
  fun generateDataModel(dataClassBuilder: DataClassBuilder) {
    return dataModelGenerator?.let { it(dataClassBuilder) }
  ...
  /**
   * Build this component instance into the provided view-section configuration using the default
  ...
  /**
   * Build this component instance into the provided upload-section configuration using the default
   * generator for this component
   */
  ...
  fun generateViewConfig(sectionConfigBuilder: SectionConfigBuilder) {
    return viewConfigGenerator?.let { it(sectionConfigBuilder) }
      ?: generateDefaultViewConfig(sectionConfigBuilder)
  }
  ...
  fun generateFixtureGenerator(sectionBuilder: FixtureSectionBuilder) {
    return fixtureGeneratorGenerator?.let { it(sectionBuilder) }
      ?: generateDefaultFixtureGenerator(sectionBuilder)
  }
