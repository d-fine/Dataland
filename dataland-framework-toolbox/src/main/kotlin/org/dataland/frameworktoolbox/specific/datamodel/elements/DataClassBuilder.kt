...
/**
 * A DataClassBuilder is an in-memory Representation of a Kotlin-DataClass that supports
 * code-generation.
 *
...
data class DataClassBuilder(
  ...
  override fun build(into: Path) {
    ...
    require(!name[0].isLowerCase()) {
      "The class-name '$name' does not start with an upper-case letter"
    }
    ...
    val freemarkerTemplate =
      FreeMarker.configuration.getTemplate("/specific/datamodel/elements/DataClass.kt.ftl")
