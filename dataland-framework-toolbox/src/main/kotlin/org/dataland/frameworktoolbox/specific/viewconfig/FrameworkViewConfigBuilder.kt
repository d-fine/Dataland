...
/**
 * A FrameworkViewConfigBuilder converts an Intermediate-Representation framework to a TypeScript
 * View-Configuration and integrates the generated code into a Dataland Repository.
 *
 * @param framework the framework DataModel to convert
 */
class FrameworkViewConfigBuilder(private val framework: Framework) {
  ...
  private fun buildViewConfig(viewConfigTsPath: Path) {
    ...
    val freemarkerTemplate =
      FreeMarker.configuration.getTemplate("/specific/viewconfig/ViewConfig.ts.ftl")
  ...
  private fun buildApiClient(apiClientTsPath: Path, privateFrameworkBoolean: Boolean) {
    ...
    val freemarkerTemplate = FreeMarker.configuration.getTemplate(nameOfApiClient)
