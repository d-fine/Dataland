...
/**
 * A FrameworkDataModelBuilder converts an Intermediate-Representation framework to a
 * Kotlin-DataModel and integrates the generated code into a Dataland Repository.
 *
 * @param framework the framework DataModel to convert
 */
class FrameworkDataModelBuilder(private val framework: Framework) {
  ...
  val rootPackageBuilder: PackageBuilder =
    PackageBuilder("$frameworkBasePackageQualifier.model", null)
  ...
  val rootDataModelClass: DataClassBuilder =
    rootPackageBuilder.addClass(
      ...
      mutableListOf(
        Annotation("Suppress", "\"MagicNumber\""),
        Annotation(
  ...
  private fun buildFrameworkSpecificApiController(
    ...
    logger.trace(
      "Building framework API controller for '{}' into '{}'",
    ...
    val freemarkerTemplate = FreeMarker.configuration.getTemplate(nameOfDataApiController)
  ...
  /**
   * Generate the code for the DataModel and integrates it into the Dataland Repository. Check if
   * compilation succeeds and re-generates the OpenApi definition.
   */
  fun build(
    into: DatalandRepository,
    ...
    logger.info(
      "Starting to build to backend data-model into the dataland-repository at ${into.path}"
    )
    ...
    into.gradleInterface.executeGradleTasks(
      listOf("dataland-backend:generateOpenApiDocs"),
      force = true,
    )
