 ...
 * Kotlin-DataModel for QA and integrates the generated code into a Dataland Repository.
 *
 * @param framework the framework DataModel to convert
 */
class FrameworkQaModelBuilder(private val framework: Framework) {
  ...
  private fun buildFrameworkSpecificApiController(into: DatalandRepository) {
    ...
    logger.trace(
      "Building framework QA controller for '{}' into '{}'",
      framework.identifier,
      targetPath,
    )
    val freemarkerTemplate =
      FreeMarker.configuration.getTemplate("/specific/qamodel/FrameworkQaController.kt.ftl")
  ...
  fun build(into: DatalandRepository) {
    ...
    into.gradleInterface.executeGradleTasks(
      listOf("dataland-qa-service:generateOpenApiDocs"),
      force = true,
    )
