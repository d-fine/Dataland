...
fun DataClassBuilder.addQaPropertyWithDocumentSupport(
  ...
  val qaJvmTypeReference = documentSupport.getQaJvmTypeReference(type, type.nullable)

  if (qaJvmTypeReference != null) {
    addProperty(name, qaJvmTypeReference, annotations + documentSupport.getJvmAnnotations())
  }
}

/**
 * Given a type reference to the dataland backend, returns the corresponding type reference as it's
 * made available int the OpenApi client.
 */
