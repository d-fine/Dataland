...
/**
 * A FrameworkUploadConfigBuilder converts an Intermediate-Representation framework to a TypeScript
 * Upload-Configuration and integrates the generated code into a Dataland Repository.
 *
 * @param framework the framework DataModel to convert
 */
class FrameworkUploadConfigBuilder(private val framework: Framework) {
  ...
  private fun buildUploadConfig(uploadConfigTsPath: Path) {
    ...
    val freemarkerTemplate =
      FreeMarker.configuration.getTemplate("/specific/uploadconfig/UploadConfig.ts.ftl")
  ...
  fun build(into: DatalandRepository) {
    ...
    with(frameworkConfigDir.toFile()) { mkdirs() }
