...
/**
 * A PackageBuilder is an in-memory Representation of a Kotlin package that supports
 * code-generation.
 *
...
data class PackageBuilder(
  ...
  fun addPackage(name: String): PackageBuilder {
    val newPackage = PackageBuilder(name = name, parentPackage = this)
  ...
  fun addEnum(name: String, options: Set<SelectionOption>, comment: String): EnumBuilder {
    val newEnum =
      EnumBuilder(name = name, parentPackage = this, options = options, comment = comment)
