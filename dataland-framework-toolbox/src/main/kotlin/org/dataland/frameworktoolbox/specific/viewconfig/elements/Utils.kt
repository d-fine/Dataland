...
/**
 * Assuming standard kotlin-dataclass generation, this function generates a null-safe accessor from
 * the root dataset object to the field in TypeScript
 */
fun ComponentBase.getTypescriptFieldAccessor(valueAccessor: Boolean = false): String {
  val parentsSequence =
    parents().toList().reversed().mapNotNull {
      when (it) {
...
/**
 * Assuming standard kotlin-dataclass generation, this function generates a null-safe accessor from
 * the root dataset object to the field in Kotlin
 */
