...
/**
 * The FrameworkRegistryImportsUpdater generates the FrameworkRegistryImports.ts file in the
 * dataland repository
 */
class FrameworkRegistryImportsUpdater {
  ...
  fun update(repository: DatalandRepository) {
    ...
    val allRegisteredFrameworks =
      pathToFrameworkDirectory.toFile().listFiles { file ->
        file.isDirectory &&
          file.listFiles()?.any { it.name == "BaseFrameworkDefinition.ts" } ?: false
      }!!
      ...
      mapOf(
        "frameworks" to
          allRegisteredFrameworks
            .sortedBy { it.name }
            .map {
              mapOf("identifier" to it.name, "baseNameInCamelCase" to getNameFromLabel(it.name))
            }
      )
    ...
    writeAllTestFiles(
      repository,
      allRegisteredPrivateFrameworks,
      freeMarkerContextForAllFrameworks,
  ...
  private fun writeAllTestFiles(
    ...
    val freemarkerContextForAllPrivateFrameworks =
      createFreeMarkerContextFile(privateFrameworkNames, "privateFrameworks")
    val freemarkerContextForPublicFrameworks =
      createFreeMarkerContextFile(publicFrameworkNames, "publicFrameworks")
    val frontendFrameworkRegistryImportsTemplateNames =
      Pair("FrontendFrameworkRegistryImports.ts.ftl", "FrontendFrameworkRegistryImports.ts")
    val basePrivateFrameworkregistryImportsTemplateNames =
      Pair("BasePrivateFrameworkRegistryImports.ts.ftl", "BasePrivateFrameworkRegistryImports.ts")
    val basePublicFrameworkregistryImportsTemplateNames =
      Pair("BasePublicFrameworkRegistryImports.ts.ftl", "BasePublicFrameworkRegistryImports.ts")
    ...
    writeIntoRegistryTsFiles(
      repository,
      freemarkerContextForAllPrivateFrameworks,
      basePrivateFrameworkregistryImportsTemplateNames,
    )
    writeIntoRegistryTsFiles(
      repository,
      freemarkerContextForPublicFrameworks,
  ...
  private fun createFreeMarkerContextFile(
    ...
    val freemarkerContextFile =
      mapOf(
        argumentName to
          frameworkList.sorted().map {
            mapOf("identifier" to it, "baseNameInCamelCase" to getNameFromLabel(it))
          }
      )
  ...
  private fun writeIntoRegistryTsFiles(
    ...
    for ((templateName, outputPath) in jobs) {
      ...
      val template = FreeMarker.configuration.getTemplate(templateName)
