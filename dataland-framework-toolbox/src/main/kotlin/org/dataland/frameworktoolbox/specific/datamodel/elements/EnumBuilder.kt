...
/**
 * A EnumBuilder is an in-memory Representation of a Kotlin-Enum that supports code-generation.
 *
...
data class EnumBuilder(
  ...
  override fun build(into: Path) {
    ...
    require(!name[0].isLowerCase()) {
      "The enum-name '$name' does not start with an upper-case letter"
    }
    ...
    val freemarkerTemplate =
      FreeMarker.configuration.getTemplate("/specific/datamodel/elements/Enum.kt.ftl")
