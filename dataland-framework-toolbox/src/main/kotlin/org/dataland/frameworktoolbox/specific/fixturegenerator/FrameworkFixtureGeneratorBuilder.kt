...
/**
 * A FrameworkFixtureGeneratorBuilder converts an Intermediate-Representation framework to a
 * fake-fixture generation script and integrates the generated code into a Dataland Repository.
 *
 * @param framework the framework DataModel to convert
 */
class FrameworkFixtureGeneratorBuilder(private val framework: Framework) {
  private val logger by LoggerDelegate()
  private val generatedTsFiles = mutableListOf<Path>()

  val rootSectionBuilder =
    FixtureSectionBuilder(parentSection = null, identifier = "root", elements = mutableListOf())
  ...
  private fun buildIndexTs(indexTsPath: Path) {
    ...
    val freemarkerTemplate =
      FreeMarker.configuration.getTemplate("/specific/fixturegenerator/index.ts.ftl")
  ...
  private fun buildPreparedFixturesTs(preparedFixturesTsPath: Path) {
    ...
    val freemarkerTemplate =
      FreeMarker.configuration.getTemplate("/specific/fixturegenerator/PreparedFixtures.ts.ftl")
  ...
  private fun buildFrameworkGeneratorsTs(frameworkGeneratorTsPath: Path) {
    val freeMarkerContext =
      mapOf("frameworkBaseName" to getNameFromLabel(framework.identifier).capitalizeEn())

    val freemarkerTemplate =
      FreeMarker.configuration.getTemplate("/specific/fixturegenerator/FrameworkGenerator.ts.ftl")
  ...
  private fun buildDataFixtures(dataFixturesTsPath: Path) {
    val imports =
      rootSectionBuilder.imports +
        ...
        TypeScriptImport(
          "type ${getNameFromLabel(framework.identifier).capitalizeEn()}" + "Data",
          "@clients/backend",
        )
    ...
    val freemarkerTemplate =
      FreeMarker.configuration.getTemplate("/specific/fixturegenerator/DataFixtures.ts.ftl")
  ...
  fun build(into: DatalandRepository) {
    logger.info(
      "Starting to build the fixture generator into the dataland-repository at ${into.path}"
    )
