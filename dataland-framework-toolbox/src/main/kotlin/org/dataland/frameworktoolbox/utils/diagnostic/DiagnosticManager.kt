...
/**
 * The DiagnosticManager handles DiagnosticMessages. A DiagnosticMessage is information that is of
 * interest to the developer and may contain code-smells. It is designed to be used for suppressible
 * error messages that developers should take into account, but can also ignore if they deem
 * appropriate.
 */
@Component
class DiagnosticManager {
  ...
  private fun logMessageToSlf4j(message: DiagnosticMessage) {
    logger.makeLoggingEventBuilder(message.type.logLevel).log(message.toString())
  }
  ...
  private fun assertThatThereAreNoUnnecessarilySuppressedMessages() {
    val messageIdsThatWereUselesslySuppressed =
      messageIdsThatShouldBeSuppressed - messageIdsThatHaveBeenSuppressed
  ...
  /**
   * Marks a message-id as "suppressed". Suppressed messages do not appear in the log and do not
   * cause exceptions (unless they were marked as un-suppressible)
   */
   ...
   *
   * @param id A unique identifier for the diagnostic. Must be unique. Should be as stable as
   *   possible.
   * @param summary A short summary of the diagnostic.
   */
  fun info(id: String, summary: String) =
    register(DiagnosticMessage(DiagnosticType.INFO, id, summary))

  /**
   * Logs a warning diagnostic that appears in the log, and must be suppressed or causes an
   * exception.
   *
   * @param id A unique identifier for the diagnostic. Must be unique. Should be as stable as
   *   possible.
   * @param summary A short summary of the diagnostic.
   */
  fun warning(id: String, summary: String) =
    register(DiagnosticMessage(DiagnosticType.WARNING, id, summary))

  /**
   * Logs a warning diagnostic that appears in the log if the condition is true, and must be
   * suppressed or causes an exception.
   *
   * @param id A unique identifier for the diagnostic. Must be unique. Should be as stable as
   *   possible.
   * @param summary A short summary of the diagnostic.
   ...
   * @param summary A short summary of the diagnostic.
   */
  fun critical(id: String, summary: String) =
