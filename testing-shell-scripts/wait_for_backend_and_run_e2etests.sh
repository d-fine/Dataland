...
timeout 240 sh -c "while ! wget http://backend:8080/actuator/health; do echo 'backend server not yet there - retrying in 1s'; sleep 1; done; rm health; echo 'backend server responded'" || exit
