 ...
 *
 * @param safeDepositDatabaseResourceClient the EuroDaT client to create the safe deposit box used
 *   to store private data on eurodat
 */
@ConditionalOnProperty(
  name = ["dataland.eurodatclient.initialize-safe-deposit-box"],
  havingValue = "true",
...
class EurodatSafeDepositBoxInitializer(
  ...
  @Value("\${dataland.eurodatclient.ignore-error}") private val isErrorIgnored: Boolean,
) {
  private val logger = LoggerFactory.getLogger(javaClass)

  /**
   * Tries to create a safe deposit box in EuroDaT for storage of Dataland data a pre-defined number
   * of times and then throws a final exception after the retries are used up.
   */
  ...
  fun createSafeDepositBox() {
    ...
    if (isErrorIgnored) {
      try {
        retryWrapperMethod("create SafeDepositBox in EuroDaT") { isSafeDepositBoxAvailable() }
      } catch (e: Exception) {
        logger.error(
  ...
  fun postSafeDepositBoxCreationRequest(): SafeDepositDatabaseResponse {
    ...
    return safeDepositDatabaseResourceClient.apiV1ClientControllerDatabaseServicePost(
      creationRequest
    )
  }
}
