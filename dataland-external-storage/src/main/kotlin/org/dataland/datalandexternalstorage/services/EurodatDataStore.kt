 ...
 * @param temporarilyCachedDocumentClient the service for retrieven documents from the temporary
 *   storage
 * @param databaseCredentialResourceClient the service to retrieve eurodat storage credentials on
 *   eurodat
 */
@Component
class EurodatDataStore(
  ...
  @Autowired
  var temporarilyCachedDocumentClient: StreamingTemporarilyCachedPrivateDocumentControllerApi,
  @Autowired var databaseCredentialResourceClient: DatabaseCredentialResourceApi,
  @Value("\${dataland.eurodatclient.app-name}") private val eurodatAppName: String,
) {
  ...
  /**
   * Stores a Data Item in EuroDaT while ensuring that there is no active transaction. This will
   * guarantee that the write is committed after exit of this method.
   *
  ...
  fun storeJsonInEurodat(
    ...
    val conn =
      getConnection(
        eurodatCredentials.username,
        eurodatCredentials.password,
        eurodatCredentials.jdbcUrl,
      )
    val sqlReturn = insertDataIntoSqlDatabase(conn, insertStatement, dataId, data)
    if (!sqlReturn) {
      throw SQLException(
        "An error occured while storing dataId $dataId with correlationId $correlationId"
      )
    }
  }

  /**
   * Stores a Blob Item in EuroDaT while ensuring that there is no active transaction. This will
   * guarantee that the write is commited after exit of this method.
   *
  ...
  fun storeBlobInEurodat(
    ...
    val sqlReturn =
      insertByteArrayIntoSqlDatabase(conn, insertStatement, eurodatId, resultByteArray)
    if (!sqlReturn) {
      throw SQLException(
  ...
  /**
   * Sends a message to the queue to inform other services that the storage to EuroDaT has been
   * successful.
   *
   * @param payload contains meta info about the stored assets (dataId and hashes)
   * @param correlationId makes it possible to match the message to one specific storage
   *   process/thread
   */
  fun sendMessageAfterSuccessfulStorage(payload: String, correlationId: String) {
    ...
    cloudEventMessageHandler.buildCEMessageAndSendToQueue(
      payload,
      MessageType.PrivateDataStored,
      correlationId,
      ExchangeName.PrivateItemStored,
  ...
  fun selectPrivateDocument(eurodatId: String, correlationId: String): ByteArray {
    logger.info(
      "Select document for eurodatId $eurodatId from eurodat storage. CorrelationId $correlationId"
    )
    val eurodatCredentials =
      retryWrapperMethod(loggingMessageGetEurodatConnection) {
        databaseCredentialResourceClient
