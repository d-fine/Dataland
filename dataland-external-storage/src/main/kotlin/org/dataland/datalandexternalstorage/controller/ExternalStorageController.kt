...
class ExternalStorageController(@Autowired private val eurodatDataStore: EurodatDataStore) :
  ExternalStorageAPI {
  private val logger = LoggerFactory.getLogger(javaClass)

  override fun selectDataById(dataId: String, correlationId: String): ResponseEntity<String> {
    logger.info(
      "Selecting data from EuroDaT storage with data ID: $dataId. Correlation ID: $correlationId."
    )
  ...
  override fun selectBlobById(
    ...
    logger.info(
      "Selecting blob from EuroDaT storage with blobId: $blobId. Correlation id: $correlationId."
    )
