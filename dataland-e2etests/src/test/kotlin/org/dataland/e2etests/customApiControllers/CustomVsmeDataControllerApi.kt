...
class CustomVsmeDataControllerApi(private val token: String) {
  ...
  private fun transferCompanyAssociatedDataVsmeDataToJson(
    input: CompanyAssociatedDataVsmeData
  ): String {
  ...
  fun postCompanyAssociatedDataVsmeData(
    ...
    val response =
      client
        .newCall(
          buildRequestForPostingCompanyAssociatedVsmeData(companyAssociatedVsmeData, documents)
        )
        .execute()
    ...
    require(response.isSuccessful) {
      "Request failed with unexpected reason, response is: $response"
    }
