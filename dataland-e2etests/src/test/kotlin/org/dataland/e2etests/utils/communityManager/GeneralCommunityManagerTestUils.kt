...
fun findAggregatedDataRequestDataTypeForFramework(
  ...
  return AggregatedDataRequest.DataType.entries.find { dataType ->
...
fun getIdForUploadedCompanyWithIdentifiers(
  ...
  return apiAccessor
    .uploadOneCompanyWithIdentifiers(lei, isins, permId)!!
    .actualStoredCompany
    .companyId
}
...
fun checkThatMessageIsAsExpected(
  ...
  val totalNumberOfCompanyIdentifiers =
    expectedNumberOfAcceptedIdentifiers + expectedNumberOfRejectedIdentifiers
  val errorMessage =
    "The message sent as part of the response to the bulk data request is not as expected."
...
fun patchDataRequestAndAssertNewStatusAndLastModifiedUpdated(
  ...
  val oldLastUpdatedTimestamp =
    requestControllerApi.getDataRequestById(dataRequestId).lastModifiedDate
  val storedDataRequestAfterPatch = requestControllerApi.patchDataRequest(dataRequestId, newStatus)
  val newLastUpdatedTimestamp =
    requestControllerApi.getDataRequestById(dataRequestId).lastModifiedDate
...
fun generateCompaniesWithOneRandomValueForEachIdentifierType(
  ...
  val baseCompany =
    CompanyInformation(
      companyName = "Name",
      headquarters = "HQ",
      identifiers = mapOf("Dummmy" to listOf()),
      countryCode = "DE",
    )
  jwtHelper.authenticateApiCallsWithJwtForTechnicalUser(TechnicalUser.Admin)
        ...
        identifiers =
          mapOf(
            identifierType.value to
              listOf("Test-${identifierType.value}${uniqueIdentifiersMap.getValue(identifierType)}")
          ),
      )
    )
  }
