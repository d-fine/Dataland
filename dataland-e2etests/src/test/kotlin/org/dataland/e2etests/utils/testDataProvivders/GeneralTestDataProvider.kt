...
class GeneralTestDataProvider {
  ...
  fun generateCompanyInformation(companyName: String, sector: String?): CompanyInformation {
    return CompanyInformation(
      companyName,
      "DummyCity",
      (mapOf(IdentifierType.PermId.value to listOf(getRandomAlphaNumericString()))),
      "DE",
      sector = sector,
    )
  }
  ...
  fun generateCompanyInformationWithNameAndIdentifiers(
    ...
    if (!lei.isNullOrEmpty())
      identifiers = identifiers + mapOf(IdentifierType.Lei.value to listOf(lei))
    if (!isins.isNullOrEmpty())
      identifiers = identifiers + mapOf(IdentifierType.Isin.value to isins)
    if (!permId.isNullOrEmpty())
      identifiers = identifiers + mapOf(IdentifierType.PermId.value to listOf(permId))
    return CompanyInformation("DummyCompany", "DummyCity", identifiers, "DE")
  }

  private fun getListOfBackendOnlyFrameworks(): List<DataTypeEnum> {
    return DataTypeEnum.values().toMutableList().filter {
      !FRONTEND_DISPLAYED_FRAMEWORKS.contains(it)
    }
  }

  fun generateOneCompanyInformationPerBackendOnlyFramework():
    Map<DataTypeEnum, List<CompanyInformation>> {
