...
class FrameworkTestDataProvider<T>(private val clazz: Class<T>) {
  ...
  private val jsonFilesForTesting =
    mapOf(
      ...
      LksgData::class.java to File("./build/resources/test/CompanyInformationWithLksgData.json"),
      SfdrData::class.java to File("./build/resources/test/CompanyInformationWithSfdrData.json"),
      VsmeData::class.java to File("./build/resources/test/CompanyInformationWithVsmeData.json"),
  ...
  private fun convertJsonToList(jsonFile: File): List<CompanyInformationWithT<T>> {
    ...
    val parameterizedTypeOfCompanyInformationWithT =
      Types.newParameterizedType(CompanyInformationWithT::class.java, clazz)
    val parameterizedTypeOfConverterOutput =
      Types.newParameterizedType(List::class.java, parameterizedTypeOfCompanyInformationWithT)
  ...
  fun getCompanyInformationWithoutIdentifiers(requiredQuantity: Int): List<CompanyInformation> {
    return testCompanyInformationWithTData.slice(0 until requiredQuantity).map {
  ...
  private fun companyListForTestingEuTaxonomyNonFinancialsSpecificValidation():
    List<CompanyInformationWithT<T>> {
    return convertJsonToList(
      File(
        "./build/resources/test/CompanyInformationWithEutaxonomyNonFinancialsPreparedFixtures.json"
      )
    )
  }

  private fun companyListForTestingEuTaxonomyFinancialsSpecificValidation():
    List<CompanyInformationWithT<T>> {
    return convertJsonToList(
      File(
        "./build/resources/test/CompanyInformationWithEuTaxonomyDataForFinancialsPreparedFixtures.json"
      )
    )
  }

  fun getSpecificCompanyByNameFromLksgPreparedFixtures(
    companyName: String
  ...
  fun getSpecificCompanyByNameFromSfdrPreparedFixtures(
    companyName: String
  ): CompanyInformationWithT<T>? {
  ...
  fun getTData(numberOfDataSets: Int): List<T> {
    return testCompanyInformationWithTData.slice(0 until numberOfDataSets).map { it.t }
  }
}
