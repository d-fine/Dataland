...
private fun checkThatTheNumberOfStoredReportingPeriodsIsAsExpected(
  ...
  val actualNumberOfStoredReportingPeriods =
    singleDataRequestResponse.reportingPeriodsOfStoredDataRequests.size
  assertEquals(
...
private fun checkThatTheNumberOfDuplicateReportingPeriodsIsAsExpected(
  ...
  val actualNumberOfDuplicateReportingPeriods =
    singleDataRequestResponse.reportingPeriodsOfDuplicateDataRequests.size
  assertEquals(
...
private fun checkThatResponseMessageIsAsExpectedForOnlyOneReportingPeriod(
  ...
  val errorMessage =
    "The message sent as part of the response to the single data request is not as expected."
...
fun causeClientExceptionBySingleDataRequest(
  ...
  val clientException =
    assertThrows<ClientException> {
      requestControllerApi.postSingleDataRequest(
        SingleDataRequest(identifier, dataType, reportingPeriods)
      )
    }
...
fun checkErrorMessageForNonUniqueIdentifiersInSingleRequest(clientException: ClientException) {
  ...
  Assertions.assertTrue(
    responseBody.contains("Multiple companies have been found for the identifier you specified.")
  )
}
