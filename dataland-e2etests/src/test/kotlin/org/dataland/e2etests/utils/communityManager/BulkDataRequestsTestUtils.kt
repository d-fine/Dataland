...
fun checkThatAllIdentifiersWereAccepted(
  ...
  checkThatTheNumberOfAcceptedIdentifiersIsAsExpected(
    requestResponse,
    expectedNumberOfAcceptedIdentifiers,
  )
  checkThatTheNumberOfRejectedIdentifiersIsAsExpected(
    requestResponse,
...
fun sendBulkRequestWithEmptyInputAndCheckErrorMessage(
  ...
  if (identifiers.isNotEmpty() && dataTypes.isNotEmpty() && reportingPeriods.isNotEmpty()) {
    ...
    val clientException =
      causeClientExceptionByBulkDataRequest(identifiers, dataTypes, reportingPeriods)
...
fun checkErrorMessageForInvalidIdentifiersInBulkRequest(clientException: ClientException) {
  ...
  assertTrue(
    responseBody.contains(
      "All provided company identifiers are not unique or could not be " + "recognized."
    )
  )
  assertTrue(
    responseBody.contains(
