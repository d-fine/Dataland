...
class QaApiAccessor {
  ...
  val euTaxonomyNonFinancialsQaReportControllerApi =
    EutaxonomyNonFinancialsDataQaReportControllerApi(BASE_PATH_TO_QA_SERVICE)

  /**
   * Wait until QaStatus is accepted for all Upload Infos or throw error. The metadata of the
   * provided uploadInfos are updated in the process.
   *
   * @param uploadInfos List of UploadInfo for which an update of the QaStatus should be checked and
   *   awaited
  ...
  /**
   * Waits until the status of all provided [metaDatas] is QaStatus.Accepted. Then returns an
   * updated list of metaData each of which has qaStatus = QaStatus.Accepted.
   *
