...
class P2p {
  ...
  private val listOfOneCompanyInformation =
    apiAccessor.testDataProviderForP2pData.getCompanyInformationWithoutIdentifiers(1)

  private fun getP2pDatasetWithSortedSectors(dataset: PathwaysToParisData): PathwaysToParisData {
    return dataset.copy(
      general =
        dataset.general.copy(
          general = dataset.general.general.copy(sectors = dataset.general.general.sectors.sorted())
        )
    )
  }
  ...
  fun `post a company with P2p data and check if the data can be retrieved correctly`() {
    ...
    val downloadedAssociatedData =
      apiAccessor.dataControllerApiForP2pData.getCompanyAssociatedP2pData(
