...
class CompanyDataControllerGetCompaniesEndpointTest {
  ...
  fun `post a dummy company and check if that specific company can be queried by its country code and sector`() {
    ...
    val expectedStoredCompany =
      uploadInfo.actualStoredCompany.copy(dataRegisteredByDataland = listOf(expectedDataMetaInfo))
    ...
    assertTrue(
      getCompaniesByCountryCodeAndSectorResponse.contains(
        convertStoredToBasicCompanyInformation(expectedStoredCompany)
      ),
      "The posted company could not be found in the query results when querying for its country code and sector.",
    )
  }

  private fun convertStoredToBasicCompanyInformation(
  ...
  fun `post a dummy company and check that it is not returned if filtered by a different sector`() {
    ...
    assertFalse(
      getCompaniesByCountryCodeAndSectorResponse.contains(
        convertStoredToBasicCompanyInformation(uploadInfo.actualStoredCompany)
      ),
  ...
  fun `post a dummy company and check if it can be searched for by identifier at the right time`() {
    ...
    val firstIdentifier =
      uploadInfo.inputCompanyInformation.identifiers.values.first { it.isNotEmpty() }.first()
    assertTrue(
      apiAccessor.companyDataControllerApi
        .getCompanies(firstIdentifier, setOfAllDataTypes)
        .isEmpty(),
      "The posted company was found in the query results.",
    )
    apiAccessor.companyDataControllerApi.existsIdentifier(IdentifierType.PermId, firstIdentifier)
    uploadTestEuTaxonomyFinancialsDataSet(uploadInfo.actualStoredCompany.companyId)
    assertTrue(
      apiAccessor.companyDataControllerApi.getCompanies(searchString = firstIdentifier).any {
        it.companyId == uploadInfo.actualStoredCompany.companyId
      },
  ...
  private fun testThatSearchForCompanyIdentifierWorks(
    ...
    val searchResponse =
      apiAccessor.companyDataControllerApi.getCompanies(searchString = identifierValue)
    val companyInformationOfSearchResponse =
      searchResponse.map { apiAccessor.companyDataControllerApi.getCompanyInfo(it.companyId) }
    assertTrue(
      companyInformationOfSearchResponse.all { results ->
        results.identifiers[identifierType]?.any { it == identifierValue } ?: false
      },
  ...
  fun `upload a company with a dataset and check if it can be found via an empty name search`() {
    ...
    val companyInformation =
      CompanyInformation(
        ...
        "",
        mapOf(IdentifierType.Lei.value to listOf(UUID.randomUUID().toString())),
        "",
        listOf(),
      )
    ...
    apiAccessor.jwtHelper.authenticateApiCallsWithJwtForTechnicalUser(TechnicalUser.Uploader)
    val uploadedData =
      uploadTestEuTaxonomyFinancialsDataSet(uploadedCompany.companyId).copy(uploaderUserId = null)
    val expectedCompany =
      StoredCompany(
  ...
  fun `search for name and check the ordering of results`() {
    ...
    val sortedCompanyNames =
      apiAccessor.companyDataControllerApi.getCompanies(searchString = testString).map {
        it.companyName
      }
    assertEquals(
    ...
    val otherCompanyNames =
      apiAccessor.companyDataControllerApi.getCompanies(searchString = "other_name").map {
        it.companyName
      }
  ...
  fun `search for name and check that chunking does not change the ordering of results`() {
    ...
    val firstChunkSortedCompanyNames =
      apiAccessor.companyDataControllerApi
        .getCompanies(searchString = testString, chunkSize = 2, chunkIndex = 0)
        .map { it.companyName }
    assertEquals(listOf(company3, testString), firstChunkSortedCompanyNames)
    val lastChunkSortedCompanyNames =
      apiAccessor.companyDataControllerApi
        .getCompanies(searchString = testString, chunkSize = 2, chunkIndex = 3)
        .map { it.companyName }
  ...
  private fun createCompaniesForTestingOrdering(inputString: String): List<CompanyInformation> {
    return listOf(
      CompanyInformation(
        company9,
        "",
        mapOf(IdentifierType.Isin.value to listOf("3$inputString")),
        "",
        listOf(),
      ),
      ...
      CompanyInformation(
        company3,
        "",
        mapOf(IdentifierType.Isin.value to listOf(inputString)),
        "",
        listOf(),
      ),
