...
class UserUploadsControllerTest {
  ...
  private fun getUserUploads(
    userId: String,
    technicalUser: TechnicalUser,
  ): List<DataMetaInformationForMyDatasets> {
    jwtHelper.authenticateApiCallsWithJwtForTechnicalUser(technicalUser)
    return UserUploadsControllerApi(BASE_PATH_TO_DATALAND_BACKEND)
      .getUserUploadsDataMetaInformation(userId)
  }
  ...
  fun `check that a reading user has no uploads`() {
    val shouldBeEmpty =
      getUserUploads(TechnicalUser.Reviewer.technicalUserId, TechnicalUser.Reviewer)
