...
class Lksg {
  ...
  private val listOfOneCompanyInformation =
    apiAccessor.testDataProviderForLksgData.getCompanyInformationWithoutIdentifiers(1)
  ...
  private fun removeNullMapEntriesFromSupplierCountryCountAndSortAllRiskPositions(
    dataset: LksgData
  ): LksgData {
    ...
    fixedDataSet.general.productionSpecificOwnOperations?.procurementCategories?.forEach {
      ...
      keysOfEntriesToDelete.forEach { key ->
        (it.value.numberOfSuppliersPerCountryCode as? MutableMap<String, LksgProcurementCategory>)
          ?.remove(key)
      }
    }
    ...
    val fixedDataSetWithAllSortedRiskPositions =
      sortDatasetsInSecondTest(listOf(fixedDataSetWithSortedComplaintsRisks))[0]
  ...
  fun `post a company with Lksg data and check if the data can be retrieved correctly`() {
    val fixedDataSet =
      removeNullMapEntriesFromSupplierCountryCountAndSortAllRiskPositions(listOfOneLksgDataSet[0])
    ...
    val downloadedAssociatedData =
      apiAccessor.dataControllerApiForLksgData.getCompanyAssociatedLksgData(
        receivedDataMetaInformation!!.dataId
      )
    val downloadedAssociatedDataType =
      apiAccessor.metaDataControllerApi.getDataMetaInfo(receivedDataMetaInformation.dataId).dataType
  ...
  fun `check that dataset cannot be uploaded if document does not exist`() {
    ...
    val companyInformation =
      apiAccessor.testDataProviderForLksgData.getSpecificCompanyByNameFromLksgPreparedFixtures(
        companyName
      )
  ...
  private fun sortComplaintRisksInDataset(dataset: LksgData): LksgData {
    val complaintRisksIdentifiedRisks =
      dataset.governance?.grievanceMechanismOwnOperations?.complaintsRiskPosition?.toMutableList()
        ?: mutableListOf()
    val complaintRisksIdentifiedRisksSorted: MutableList<LksgGrievanceAssessmentMechanism> =
      mutableListOf()
              ...
              dataset.governance
                ...
                ?.copy(
                  complaintsRiskPosition =
                    complaintRisksIdentifiedRisksSorted.sortedBy { it.riskPositions.first() }
                )
          )
      )
  ...
  private fun sortDatasetsInFirstTest(fixedDataSet: LksgData): LksgData {
          ...
          fixedDataSet.general.copy(
            productionSpecific =
              fixedDataSet.general.productionSpecific?.copy(
                specificProcurement =
                  fixedDataSet.general.productionSpecific?.specificProcurement?.sorted()
              )
          ),
        governance =
    ...
    val complaintsRiskPosition =
      firstSorting.governance?.grievanceMechanismOwnOperations?.complaintsRiskPosition
            ...
            sortedDataset.general.copy(
              productionSpecific =
                sortedDataset.general.productionSpecific?.copy(
                  specificProcurement =
                    sortedDataset.general.productionSpecific?.specificProcurement?.sorted()
                )
            ),
          governance =
                  ...
                  ?.copy(
                    complaintsRiskPosition =
                      sortedDataset.governance
                        ?.grievanceMechanismOwnOperations
                        ?.complaintsRiskPosition
                        ?.sortedBy { it.riskPositions.first() }
                  ),
              generalViolations =
                sortedDataset.governance
                  ?.generalViolations
  ...
  private fun uploadFourDatasetsForACompany(): Pair<String, List<LksgData>> {
    ...
    val firstDataset =
      removeNullMapEntriesFromSupplierCountryCountAndSortAllRiskPositions(lksgData[0])
    val secondDataset =
      removeNullMapEntriesFromSupplierCountryCountAndSortAllRiskPositions(lksgData[1])
    val uploadPairs =
      listOf(
