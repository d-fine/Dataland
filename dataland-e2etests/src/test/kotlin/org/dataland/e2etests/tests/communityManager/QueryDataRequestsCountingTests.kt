...
class QueryDataRequestsCountingTests {
  ...
  private val dataTypeGetNumberOfRequestsSfdr =
    RequestControllerApi.DataTypeGetNumberOfRequests.sfdr
  private val dataTypeGetNumberOfRequestsP2p = RequestControllerApi.DataTypeGetNumberOfRequests.p2p
  private val dataTypeGetNumberOfRequestsVsme =
    RequestControllerApi.DataTypeGetNumberOfRequests.vsme
  ...
  fun `count requests with request status filters`() {
    ...
    assertEquals(
      ...
      api.getNumberOfRequests(
        datalandCompanyId = companyIdA,
        requestStatus = setOf(RequestStatus.Resolved),
      ),
    )
    assertEquals(
      1,
  ...
  fun `count requests with reporting period 2022 and request status filter`() {
    assertEquals(
      1,
      api.getNumberOfRequests(datalandCompanyId = companyIdA, reportingPeriod = "2022"),
    )
    assertEquals(
      1,
      api.getNumberOfRequests(
  ...
  fun `count requests with reporting period 2023 and request status filter`() {
    assertEquals(
      1,
      api.getNumberOfRequests(datalandCompanyId = companyIdB, reportingPeriod = "2023"),
    )
    assertEquals(
      1,
      api.getNumberOfRequests(
