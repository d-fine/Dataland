...
class AggregatedDataRequestsTest {
  ...
  private fun testNonTrivialIdentifierValueFilterOnAggregatedLevel(
    ...
    val aggregatedDataRequestsWithoutFilter =
      requestControllerApi.getAggregatedDataRequests(identifierValue = null)
    iterateThroughAllThreeSpecificationsAndCheckAggregationWithCount(
      aggregatedDataRequestsWithoutFilter,
      frameworks,
      reportingPeriods,
      identifiersToRecognize,
      1,
    )
    val aggregatedDataRequestsForEmptyString =
      requestControllerApi.getAggregatedDataRequests(identifierValue = "")
    iterateThroughAllThreeSpecificationsAndCheckAggregationWithCount(
  ...
  fun `post bulk data request and check that filter for the identifier value on aggregated level works properly`() {
    ...
    val aggregatedDataRequest =
      requestControllerApi.getAggregatedDataRequests(
        identifierValue =
          apiAccessor.companyDataControllerApi
            .getCompaniesBySearchString(identifiersToMap.getValue(IdentifierType.Isin))
            .first()
            .companyId
      )
    iterateThroughAllThreeSpecificationsAndCheckAggregationWithCount(
      ...
      frameworks,
      reportingPeriods,
      identifiersToMap.filterKeys { it == IdentifierType.Isin }.values.toSet(),
      1,
    )
    testNonTrivialIdentifierValueFilterOnAggregatedLevel(
      frameworks,
      reportingPeriods,
      identifiersToMap.values.toSet(),
    )
  }
  ...
  private fun checkAggregationForNonTrivialFrameworkFilter(
    ...
    listOf(1, (2 until frameworks.size).random(), frameworks.size).forEach {
      numberOfRandomFrameworks ->
  ...
  fun `post bulk requests and check that the filter for frameworks on aggregated level works properly`() {
    ...
    checkAggregationForNonTrivialFrameworkFilter(
      frameworks,
      reportingPeriods,
      identifierMap.values.toSet(),
    )
    val aggregatedDataRequestsWithoutFilter =
      requestControllerApi.getAggregatedDataRequests(dataTypes = null)
    iterateThroughAllThreeSpecificationsAndCheckAggregationWithCount(
      aggregatedDataRequestsWithoutFilter,
      frameworks,
      reportingPeriods,
    ...
    val aggregatedDataRequestsForEmptyList =
      requestControllerApi.getAggregatedDataRequests(dataTypes = emptyList())
    iterateThroughAllThreeSpecificationsAndCheckAggregationWithCount(
      aggregatedDataRequestsForEmptyList,
      frameworks,
      reportingPeriods,
      identifierMap.values.toSet(),
      1,
  ...
  fun `post bulk data request and check that the filter for reporting periods on aggregated level works properly`() {
    ...
    val aggregatedDataRequests =
      requestControllerApi.getAggregatedDataRequests(reportingPeriod = randomReportingPeriod)
    iterateThroughAllThreeSpecificationsAndCheckAggregationWithCount(
      aggregatedDataRequests,
      frameworks,
      setOf(randomReportingPeriod),
      identifierMap.values.toSet(),
      1,
    )
    reportingPeriods
      .filter { it != randomReportingPeriod }
      .forEach { filteredReportingPeriod ->
        assertFalse(aggregatedDataRequests.any { it.reportingPeriod == filteredReportingPeriod })
      }
    val aggregatedDataRequestsWithoutFilter =
      requestControllerApi.getAggregatedDataRequests(reportingPeriod = null)
    iterateThroughAllThreeSpecificationsAndCheckAggregationWithCount(
      aggregatedDataRequestsWithoutFilter,
      frameworks,
      reportingPeriods,
    ...
    val aggregatedDataRequestsForEmptyString =
      requestControllerApi.getAggregatedDataRequests(reportingPeriod = "")
    iterateThroughAllThreeSpecificationsAndCheckAggregationWithCount(
      aggregatedDataRequestsForEmptyString,
      frameworks,
      reportingPeriods,
  ...
  fun `post bulk data request and check that the filter for request status on aggregated level works properly`() {
    ...
    val datalandCompanyIDForLei =
      getUniqueDatalandCompanyIdForIdentifierValue(identifierMap.getValue(IdentifierType.Lei))
    val frameworks = setOf(BulkDataRequest.DataTypes.lksg)
  ...
  private fun assertNumberOfMatchesOnExclusivelyOpenRequestsEquals(
    ...
    countOfOpenRequests: Long =
      2, // currently only used with input 2, default added because of linting
  ) {
    ...
    assertNumberOfMatchesOnRequestStatusEquals(
      ...
      setOf(RequestStatus.Open.value),
      countOfOpenRequests,
    )
    assertNumberOfMatchesOnRequestStatusEquals(
      aggregatedDataRequests,
      companyIdentifierValue,
      setOf(RequestStatus.Answered.value),
      0,
    )
    assertNumberOfMatchesOnRequestStatusEquals(
      aggregatedDataRequests,
      companyIdentifierValue,
      setOf(RequestStatus.Resolved.value),
      0,
    )
    assertNumberOfMatchesOnRequestStatusEquals(
      aggregatedDataRequests,
  ...
  private fun assertNumberOfMatchesOnRequestStatusEquals(
    ...
    val numberOfStatusMatches =
      aggregatedDataRequests
        .filter { it.datalandCompanyId == companyIdentifier && it.requestStatus in stati }
        .sumOf { it.count }
