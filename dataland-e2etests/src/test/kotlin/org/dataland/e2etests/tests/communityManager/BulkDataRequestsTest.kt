...
class BulkDataRequestsTest {
  ...
  fun `post bulk data request for all frameworks and different valid identifiers and check stored requests`() {
    ...
    uniqueIdentifiersMap[randomUniqueDataRequestCompanyIdentifierType]?.let {
      checkThatDataRequestExistsExactlyOnceInRecentlyStored(
        newlyStoredRequests,
        dataTypes.random().value,
        reportingPeriods.random(),
  ...
  fun `post a bulk data request with at least one invalid identifier and check that this gives no stored request`() {
    ...
    val invalidIdentifiers =
      setOf(generateRandomLei() + "F", generateRandomIsin() + "F", generateRandomPermId() + "F")
    val timestampBeforeBulkRequest = retrieveTimeAndWaitOneMillisecond()
    generateCompaniesWithOneRandomValueForEachIdentifierType(uniqueIdentifiersMap)
    val response =
      requestControllerApi.postBulkDataRequest(
    ...
    assertFalse(newlyStoredRequests.any { invalidIdentifiers.contains(it.datalandCompanyId) })
  }
  ...
  fun `post bulk data request with at least one company duplicate and check that only one request is stored`() {
    ...
    val identifiersForBulkRequest =
      setOf(leiForCompany, isinForCompany, identifierValueForUnknownCompany)
    ...
    checkThatDataRequestExistsExactlyOnceInRecentlyStored(
      newlyStoredRequests,
      frameworksForBulkRequest[0].value,
      reportingPeriod,
      companyId,
    )
  }
  ...
  fun `post a bulk data request with and check that duplicates are only stored if previous in final status`() {
    ...
    retrieveDataRequestIdForReportingPeriodAndUpdateStatus(
      newlyStoredRequests,
      "2022",
    ...
    val newlyStoredRequestsAfterDuplicates =
      getNewlyStoredRequestsAfterTimestamp(timestampBeforeDuplicates)
  ...
  fun `check the expected exception is thrown when frameworks are empty or identifiers are empty or invalid only`() {
    ...
    sendBulkRequestWithEmptyInputAndCheckErrorMessage(
      validIdentifiers,
      emptySet(),
      reportingPeriods,
    )
    ...
    val invalidIdentifiers =
      setOf(generateRandomLei() + "F", generateRandomIsin() + "F", generateRandomPermId() + "F")
    val clientException =
      causeClientExceptionByBulkDataRequest(invalidIdentifiers, dataTypes, reportingPeriods)
