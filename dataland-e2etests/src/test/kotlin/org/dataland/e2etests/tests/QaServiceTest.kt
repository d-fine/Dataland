...
class QaServiceTest {
  ...
  private fun uploadDatasetAndValidatePendingState(
    user: TechnicalUser = TechnicalUser.Uploader
  ): String {
    apiAccessor.jwtHelper.authenticateApiCallsWithJwtForTechnicalUser(user)
    val dataId =
      dataController
        .postCompanyAssociatedEutaxonomyNonFinancialsData(dummyCompanyAssociatedData, false)
        .dataId
    assertEquals(
  ...
  private fun reviewDatasetAndValidateItIsNotReviewable(
    dataId: String,
    ...
    await().atMost(2, TimeUnit.SECONDS).until {
      qaServiceController.getUnreviewedDatasetsIds().contains(dataId)
    }
  ...
  private fun awaitQaStatusChange(dataId: String, expectedQaStatus: BackendQaStatus) {
    await().atMost(2, TimeUnit.SECONDS).until {
      apiAccessor.metaDataControllerApi.getDataMetaInfo(dataId).qaStatus == expectedQaStatus
    }
  }
  ...
  fun `check that the review queue is correctly ordered`() {
    ...
    val listOfDataIdsAsExpectedFromReviewQueue =
      (1..numberOfUploadedDatasets).map {
        dataController
          .postCompanyAssociatedEutaxonomyNonFinancialsData(dummyCompanyAssociatedData, false)
          .dataId
          .also { Thread.sleep(SLEEP_DURATION_MS) }
      }
    apiAccessor.jwtHelper.authenticateApiCallsWithJwtForTechnicalUser(TechnicalUser.Reviewer)
    val reviewQueue = apiAccessor.qaServiceControllerApi.getUnreviewedDatasetsIds()

    assertTrue(
      listOfDataIdsAsExpectedFromReviewQueue
        .toTypedArray()
        .contentDeepEquals(reviewQueue.toTypedArray())
    )
  }
  ...
  fun `check that an already reviewed dataset can not be assigned a different qa status`() {
    ...
    awaitQaStatusChange(dataId, BackendQaStatus.Accepted)
    val exception =
      assertThrows<QaServiceClientException> {
  ...
  fun `check the a data set with review history can only retrieved by admin reviewer and uploader of the data`() {
    ...
    val usersWithAccessToReviewHistory =
      listOf(TechnicalUser.Admin, TechnicalUser.Reviewer, TechnicalUser.Uploader)
  ...
  fun `check the a reader can access the review history of the data set he uploaded but an uploader cant`() {
    ...
    apiAccessor.companyRolesControllerApi.assignCompanyRole(
      ...
      UUID.fromString(dummyCompanyAssociatedData.companyId),
