...
class DocumentControllerTest {
  ...
  fun `test that a dummy docx document can be uploaded and retrieved as pdf after successful QA`() {
    ...
    validateResponseHeaders(
      uploadResponse,
      DocumentType.Pdf.mediaType,
      byteArrayOfDownload.size.toString(),
    )
    assertTrue(
      isByteArrayRepresentationOfPdf(byteArrayOfDownload),
  ...
  fun `test that a dummy ods document can be uploaded and retrieved after successful QA`() {
    ...
    validateResponseHeaders(
      uploadResponse,
      DocumentType.Ods.mediaType,
      byteArrayOfOds.size.toString(),
    )
  ...
  fun `test that a dummy xlsx document can be uploaded and retrieved after successful QA`() {
    ...
    validateResponseHeaders(
      uploadResponse,
      DocumentType.Xlsx.mediaType,
      byteArrayOfXlsx.size.toString(),
    )
  ...
  fun `test that a dummy xls document can be uploaded and retrieved after successful QA`() {
    ...
    validateResponseHeaders(
      uploadResponse,
      DocumentType.Xls.mediaType,
      byteArrayOfXls.size.toString(),
    )
  ...
  fun `test that users without keycloak uploader role can upload documents with certain company roles`() {
    ...
    val companyRolesAllowedToPostDocument =
  ...
  private fun removeAllRolesFromUser(userId: UUID) {
    val rolesOfUser =
      apiAccessor.companyRolesControllerApi.getCompanyRoleAssignments(null, null, userId)
    rolesOfUser.forEach {
   ...
   *
   * @param uploadResponse the DocumentUploadResponse document's id for which an update of the
   *   QaStatus should be checked and awaited
  ...
  private fun validateResponseHeaders(
    ...
    Awaitility.await().atMost(10, TimeUnit.SECONDS).until {
      try {
        val response =
          documentControllerClient.getDocumentWithHttpInfo(uploadResponse.documentId).headers
  ...
  private fun isByteArrayRepresentationOfPdf(byteArray: ByteArray): Boolean {
    ...
    return byteArray.size >= pdfSignature.size &&
      byteArray.sliceArray(pdfSignature.indices).contentEquals(pdfSignature)
  }
}
