...
class Vsme {
  ...
  fun `post VSME data check its meta info persistence and that data is only accessible for company role holders `() {
    val vsmeData =
      vsmeTestUtils.setReferencedReports(testVsmeData, FileInfos(hashAlpha, fileNameAlpha))
    ...
    val dataMetaInfoInResponse =
      vsmeTestUtils.postVsmeDataset(
        ...
        listOf(dummyFileAlpha),
        TechnicalUser.Uploader,
      )
    val persistedDataMetaInfo =
      executeDataRetrievalWithRetries(
        apiAccessor.metaDataControllerApi::getDataMetaInfo,
        dataMetaInfoInResponse.dataId,
      )
  ...
  fun `post VSME data with documents and check if data and documents match the uploaded data and documents`() {
    ...
    val retrievedCompanyAssociatedVsmeData =
      executeDataRetrievalWithRetries(
        vsmeDataControllerApi::getCompanyAssociatedVsmeData,
        dataMetaInfoInResponse.dataId,
      )
    assertEquals(companyAssociatedDataVsmeData, retrievedCompanyAssociatedVsmeData)

    val downloadedAlpha =
      vsmeDataControllerApi.getPrivateDocument(dataMetaInfoInResponse.dataId, hashAlpha)
    assertEquals(hashAlpha, downloadedAlpha.readBytes().sha256())

    val downloadedBeta =
      vsmeDataControllerApi.getPrivateDocument(dataMetaInfoInResponse.dataId, hashBeta)
  ...
  fun `post two VSME datasets for the same reporting period and company and assert correct handling`() {
    ...
    val companyAssociatedVsmeDataAlpha =
      generateVsmeDataWithSetNumberOfEmployeesInHeadCount(
        companyId,
        "2022",
        vsmeData,
        BigDecimal(1),
  ...
  fun `post an VSME dataset with duplicate file and assert that the downloaded file is unique and correct`() {
    ...
    val dataId =
      vsmeTestUtils
        .postVsmeDataset(
          companyAssociatedVsmeData,
          listOf(dummyFileAlpha, dummyFileAlpha),
          TechnicalUser.Uploader,
        )
        .dataId

    val persistedDataMetaInfo =
      executeDataRetrievalWithRetries(apiAccessor.metaDataControllerApi::getDataMetaInfo, dataId)
  ...
  fun `post a VSME dataset and verify that a normal user with access status granted can retrieve the data`() {
    ...
    apiAccessor.jwtHelper.authenticateApiCallsWithJwtForTechnicalUser(TechnicalUser.PremiumUser)
    val retrievedCompanyAssociatedVsmeData =
      executeDataRetrievalWithRetries(vsmeDataControllerApi::getCompanyAssociatedVsmeData, dataId)
  ...
  class FileInfos(val fileReference: String, val fileName: String)
}
