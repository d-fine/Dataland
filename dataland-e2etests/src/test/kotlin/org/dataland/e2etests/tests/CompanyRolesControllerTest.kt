...
class CompanyRolesControllerTest {
  ...
  private val frameworkSampleData =
    apiAccessor.testDataProviderForEuTaxonomyDataForNonFinancials.getTData(1)[0]
  ...
  private fun assertCompanyNotFoundResponseBodyInCommunityManagerClientException(
    ...
    assertTrue(responseBody.contains("\"Dataland does not know the company ID $companyId\""))
  }
  ...
  private fun getCompanyRoleAssignments(
    ...
    return apiAccessor.companyRolesControllerApi.getCompanyRoleAssignments(
      companyRole,
      companyId,
      userId,
    )
  ...
  fun `validate that creation and deletion of company role assignments works as expected`() {
    ...
    val ownersAfterRepost =
      getCompanyRoleAssignments(CompanyRole.CompanyOwner, companyId = firstCompanyId)
  ...
  fun `check that company ownership endpoints deny access if unauthorized or not sufficient rights`() {
    ...
    assertAccessDeniedResponseBodyInCommunityManagerClientException(
      postCompanyOwnerExceptionBecauseOfMissingRights
    )
    ...
    assertAccessDeniedResponseBodyInCommunityManagerClientException(
      deleteExceptionBecauseOfMissingRights
    )
    ...
    assertErrorCodeInCommunityManagerClientException(
      expectedClientExceptionWhenCallingHeadEndpoint,
      403,
    )
  ...
  fun `assure that a company owner without keycloak admin role can modify assignments for all company roles`() {
    ...
    enumValues<CompanyRole>().forEach {
      ...
      val exceptionWhenCheckingIfUserIsCompanyOwner =
        assertThrows<ClientException> { hasUserCompanyRole(it, companyId, dataUploaderUserId) }
      assertErrorCodeInCommunityManagerClientException(
        exceptionWhenCheckingIfUserIsCompanyOwner,
        404,
      )
    }
  }
  ...
  fun `assure that company member admin without keycloak admin role can only modify member and member admin roles`() {
    ...
    val rolesThatCannotBeModified = listOf(CompanyRole.CompanyOwner, CompanyRole.DataUploader)
    jwtHelper.authenticateApiCallsWithJwtForTechnicalUser(TechnicalUser.Admin)
    ...
    rolesThatCannotBeModified.forEach {
      ...
      val exceptionWhenTryingToAddCompanyMembers =
        assertThrows<ClientException> { assignCompanyRole(it, companyId, dataUploaderUserId) }
  ...
  fun `assure that a user with no role or only member or uploader company role can not modify role assignments`() {
    ...
    val companyRolesWithoutModificationRights = listOf(CompanyRole.DataUploader, CompanyRole.Member)
  ...
  fun `assure that existing role assignments are deleted when a user gets assigned a new company role`() {
    ...
    currentAssignments =
      getCompanyRoleAssignments(companyId = companyId, userId = dataUploaderUserId)
  ...
  private fun tryToUseCompanyRoleGetAndHeadEndpointAndAsserThatItsForbidden(companyId: UUID) {
    ...
    assertErrorCodeInCommunityManagerClientException(
      exceptionWhenGettingCompanyRolesForAnotherCompany,
      403,
    )
    ...
    assertErrorCodeInCommunityManagerClientException(
      exceptionWhenCheckingCompanyRolesForAnotherCompany,
      403,
    )
  }
  ...
  private fun tryToAssignAndRemoveCompanyMembersAndAssertThatItsForbidden(companyId: UUID) {
    enumValues<CompanyRole>().forEach {
      ...
      val exceptionWhenTryingToDeleteCompanyMembers =
        assertThrows<ClientException> { removeCompanyRole(it, companyId, dataUploaderUserId) }
      assertErrorCodeInCommunityManagerClientException(
        exceptionWhenTryingToDeleteCompanyMembers,
        403,
      )
    }
  }
