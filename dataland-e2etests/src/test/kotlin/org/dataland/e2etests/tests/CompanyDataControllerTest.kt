...
class CompanyDataControllerTest {
  ...
  private val fullPatchObject =
    CompanyInformationPatch(
      ...
      identifiers = mapOf(IdentifierType.Duns.value to listOf("Test-DUNS${UUID.randomUUID()}")),
    )
  ...
  fun `post a dummy company and check if patching basic properties works as expected`() {
    ...
    val updatedCompany =
      apiAccessor.companyDataControllerApi.patchCompanyById(companyId, patchObject)
    assertEquals(
      patchObject.companyName,
      updatedCompany.companyInformation.companyName,
      "The company should have been updated",
    )
    assertEquals(
      patchObject.website,
      updatedCompany.companyInformation.website,
      "The website should have been set",
    )
    assertEquals(
      startingCompanyInformation.sector,
      updatedCompany.companyInformation.sector,
  ...
  fun `post a dummy company and check if patching identifiers works as expected`() {
    ...
    val companyInformationToUpload =
      apiAccessor.testDataProviderEuTaxonomyForFinancials
        ...
        .first()
    val uploadedCompany =
      apiAccessor.companyDataControllerApi.postCompany(companyInformationToUpload)
    val patchObject =
      CompanyInformationPatch(
        identifiers =
          mapOf(
            IdentifierType.Lei.value to
              listOf("Test-Lei1${UUID.randomUUID()}", "Test-Lei2${UUID.randomUUID()}"),
  ...
  fun `post a dummy company and check if patching alternative names works as expected`() {
    ...
    val companyId = uploadInfo.actualStoredCompany.companyId
    val patchObject =
      CompanyInformationPatch(companyAlternativeNames = listOf("Alt-Name-1", "Alt-Name-2"))
    apiAccessor.jwtHelper.authenticateApiCallsWithJwtForTechnicalUser(TechnicalUser.Admin)
    val updatedCompany =
      apiAccessor.companyDataControllerApi.patchCompanyById(companyId, patchObject)
    assertEquals(
      patchObject.companyAlternativeNames,
      updatedCompany.companyInformation.companyAlternativeNames,
    ...
    val putCompanyInformation =
      CompanyInformation(
        ...
        companyAlternativeNames =
          listOf("Alt-Name-1${UUID.randomUUID()}", "Alt-Name-2${UUID.randomUUID()}"),
        identifiers = mapOf(IdentifierType.Lei.value to listOf("Test-Lei${UUID.randomUUID()}")),
        countryCode = "DE",
      )
    apiAccessor.jwtHelper.authenticateApiCallsWithJwtForTechnicalUser(TechnicalUser.Admin)
    val updatedCompany =
      apiAccessor.companyDataControllerApi.putCompanyById(companyId, putCompanyInformation)
    assertEquals(
      putCompanyInformation.companyName,
      updatedCompany.companyInformation.companyName,
      "The company should have been updated",
    )
    assertEquals(
      putCompanyInformation.headquarters,
      updatedCompany.companyInformation.headquarters,
  ...
  fun `post a dummy company and check if the putting mechanism for identifiers works as expected`() {
        ...
        identifiers =
          mapOf(
            IdentifierType.Duns.value to
              listOf("Test-Duns${UUID.randomUUID()}", "Test-Duns2${UUID.randomUUID()}")
          ),
        countryCode = "DE",
      )
    ...
    apiAccessor.jwtHelper.authenticateApiCallsWithJwtForTechnicalUser(TechnicalUser.Admin)
    var updatedCompany =
      apiAccessor.companyDataControllerApi.putCompanyById(companyId, put1CompanyInformation)
    assertTrue(
    ...
    updatedCompany =
      apiAccessor.companyDataControllerApi.putCompanyById(companyId, put2CompanyInformation)
    assertTrue(
  ...
  fun `post a dummy company and check if that specific company can be queried by its company Id`() {
    ...
    assertEquals(
      expectedStoredCompany.copy(
        companyInformation =
          expectedStoredCompany.companyInformation.copy(
            companyContactDetails =
              expectedStoredCompany.companyInformation.companyContactDetails?.sorted()
          )
      ),
  ...
  fun `post a dummy company as teaser company and test if it is retrievable by company ID as unauthorized user`() {
    ...
    assertEquals(
      expectedStoredTeaserCompany.copy(
        companyInformation =
          expectedStoredTeaserCompany.companyInformation.copy(
            companyContactDetails =
              expectedStoredTeaserCompany.companyInformation.companyContactDetails?.sorted()
          )
      ),
  ...
  fun `post a dummy company as a user type which does not have the rights to do so and receive an error code 403`() {
    ...
    val exception =
      assertThrows<ClientException> {
        apiAccessor.companyDataControllerApi.postCompany(testCompanyInformation).companyId
      }
    assertEquals(
      "Client error : 403 ",
      exception.message,
  ...
  fun `post a dummy company twice and receive the expected error code and message`() {
    ...
    val storedCompanyIdentifier =
      storedCompany.companyInformation.identifiers.entries.first { it.value.isNotEmpty() }
  ...
  fun `check if the new companies search via name and ids endpoint works as expected`() {
    ...
    val sortedCompanyNames =
      apiAccessor.companyDataControllerApi
        .getCompaniesBySearchString(searchString = testString)
        .map { it.companyName }
    assertEquals(
  ...
  private fun uploadCompaniesInReverseToExpectedOrder(expectedSearchString: String) {
    ...
    companyId =
      uploadModifiedBaseCompany(checkOtherCompanyFalse, listOf("1${expectedSearchString}2"))
    uploadDummyDataset(companyId = companyId, bypassQa = false)
    companyId =
      uploadModifiedBaseCompany(checkOtherCompanyTrue, listOf("1${expectedSearchString}2"))
    uploadDummyDataset(companyId = companyId, bypassQa = true)
  }
  ...
  private fun uploadModifiedBaseCompany(name: String, alternativeNames: List<String>?): String {
    val companyInformation =
      baseCompanyInformation.copy(
        ...
        identifiers = mapOf(IdentifierType.Isin.value to listOf(UUID.randomUUID().toString())),
      )
  ...
  private fun uploadDummyDataset(
    ...
    apiAccessor.dataControllerApiForEuTaxonomyNonFinancials
      .postCompanyAssociatedEutaxonomyNonFinancialsData(
        dummyCompanyAssociatedDataWithoutCompanyId.copy(
          companyId = companyId,
  ...
  fun `counts the number of datasets for a company`() {
    ...
    sleep(100)
    val exceptionMap: Map<DataTypeEnum, Long> =
      mapOf(DataTypeEnum.eutaxonomyMinusNonMinusFinancials to 2)
    val expectedMap =
      DataTypeEnum.entries.associate { framework ->
        val numOfReportingPeriods = exceptionMap[framework] ?: 0
        framework.toString() to
          AggregatedFrameworkDataSummary(numberOfProvidedReportingPeriods = numOfReportingPeriods)
      }
    val aggregatedFrameworkDataSummary =
      apiAccessor.companyDataControllerApi
        .getAggregatedFrameworkDataSummary(companyId = companyId)
        .toSortedMap()

    assertEquals(expectedMap, aggregatedFrameworkDataSummary)
  }
  ...
  fun `post a dummy company and check if it can be retrieved by the companiesInfo endpoint`() {
    ...
    assertEquals(
      expectedCompanyInformation.copy(
        companyContactDetails = expectedCompanyInformation.companyContactDetails?.sorted()
      ),
  ...
  fun `check that an exception is thrown if the contactDetails does not have the valid format`() {
    ...
    val companyId = uploadInfo.actualStoredCompany.companyId
    val patchObject =
      CompanyInformationPatch(companyContactDetails = listOf("Email-without-proper-format"))
    apiAccessor.jwtHelper.authenticateApiCallsWithJwtForTechnicalUser(TechnicalUser.Admin)
  ...
  fun `check that the Dataland uploader can patch contactDetails and website of a company without an owner`() {
    ...
    val updatedCompany =
      apiAccessor.companyDataControllerApi.patchCompanyById(companyId, patchObject)
    assertEquals(
      patchObject.companyContactDetails,
      updatedCompany.companyInformation.companyContactDetails,
  ...
  fun `check that the Dataland uploader cannot patch contactDetails or website if the company has a companyOwner`() {
    ...
    assertTrue(
      ...
      "The exception should indicate unauthorized access (HTTP 403)",
    )
  ...
  fun `check that the Dataland uploader cannot patch unallowed fields`() {
    ...
    assertThrows<ClientException> {
      apiAccessor.companyDataControllerApi.patchCompanyById(companyId, fullPatchObject)
    }
  ...
  fun `check that patching with an invalid company ID and invalid patch fields returns an exception`() {
    ...
    val responseBody = (exception.response as ClientError<*>).body as String
    assertTrue(responseBody.contains("Dataland does not know the company ID"))
  }
}
