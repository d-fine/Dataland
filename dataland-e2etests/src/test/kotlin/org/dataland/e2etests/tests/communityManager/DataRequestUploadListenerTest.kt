...
class DataRequestUploadListenerTest {
  ...
  fun `post single data request and provide data and check that status has changed to answered`() {
    ...
    newlyStoredRequests.forEach {
      assertEquals(
        RequestStatus.Open,
        it.requestStatus,
        "The status of a newly stored data request is not 'Open'.",
      )
    }
    uploadDataset(mapOfIds)
    val requestsStoredAfterSingleRequest =
      getNewlyStoredRequestsAfterTimestamp(timestampBeforeSingleRequest)
  ...
  private fun postSingleDataRequestAsTechnicalUserAndReturnDataRequestId(
    ...
    val companyId =
      getIdForUploadedCompanyWithIdentifiers(permId = System.currentTimeMillis().toString())
  ...
  fun `as a premium user assert that patching the status of a non answered request to closed is forbidden`() {
    val dataRequestId =
      postSingleDataRequestAsTechnicalUserAndReturnDataRequestId(TechnicalUser.PremiumUser)

    authenticateAsTechnicalUserAndAssertThatPatchingOfDataRequestIsForbidden(
      technicalUser = TechnicalUser.PremiumUser,
      dataRequestId = dataRequestId,
    ...
    jwtHelper.authenticateApiCallsWithJwtForTechnicalUser(TechnicalUser.PremiumUser)
    val closedDataRequest =
      requestControllerApi.patchDataRequest(dataRequestId, RequestStatus.Resolved)
    assertEquals(
  ...
  fun `patch a data request as a reader and check that it is forbidden`() {
    ...
    RequestStatus.entries.forEach {
      authenticateAsTechnicalUserAndAssertThatPatchingOfDataRequestIsForbidden(
        technicalUser = TechnicalUser.Reader,
        dataRequestId = dataRequestId,
        requestStatus = it,
        accessStatus = null,
      )
  ...
  fun `patch an answered but not owned data request to closed as a premium user and check that it is forbidden`() {
    val dataRequestId =
      postSingleDataRequestAsTechnicalUserAndReturnDataRequestId(TechnicalUser.Admin)
  ...
  fun `patch an open or answered data request to withdrawn and assert success`() {
    ...
    jwtHelper.authenticateApiCallsWithJwtForTechnicalUser(TechnicalUser.PremiumUser)
    val openToWithdrawnDataRequest =
    ...
    assertEquals(
      RequestStatus.Withdrawn,
      openToWithdrawnDataRequest.dataRequestStatusHistory.last().status,
    ...
    assertEquals(
      RequestStatus.Withdrawn,
      answeredToWithdrawnDataRequest.dataRequestStatusHistory.last().status,
      errorMessageForRequestStatusHistory,
    )
  }
  ...
  fun `add a message to an open request do not change the status and assert success`() {
    val dataRequestId =
      postSingleDataRequestAsTechnicalUserAndReturnDataRequestId(TechnicalUser.PremiumUser)
    jwtHelper.authenticateApiCallsWithJwtForTechnicalUser(TechnicalUser.PremiumUser)
    val newMessageDataRequest =
      requestControllerApi.patchDataRequest(dataRequestId, null, null, contacts, message)

    assertEquals(
      message,
      newMessageDataRequest.messageHistory.last().message,
      "The message was not patched correctly.",
    )
    assertEquals(
      contacts,
      newMessageDataRequest.messageHistory.last().contacts,
  ...
  fun `add a message to an answered request and change the status to open and assert success`() {
    ...
    assertEquals(
      message,
      newMessageAndOpenDataRequest.messageHistory.last().message,
      "The message was not patched correctly.",
    )
    assertEquals(
      contacts,
      newMessageAndOpenDataRequest.messageHistory.last().contacts,
      "The contacts were not patched correctly.",
    )
    assertEquals(
      RequestStatus.Open,
      newMessageAndOpenDataRequest.dataRequestStatusHistory.last().status,
      errorMessageForRequestStatusHistory,
    )
