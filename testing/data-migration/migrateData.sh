...
source "$(dirname "$0")"/migrationConfig.sh

if [[ -z $SOURCE || -z $SOURCE_TOKEN || -z $TARGET || -z $TARGET_TOKEN || -z $ONLY_ACTIVE || -z $FRAMEWORKS ]]; then
  echo "At least one of the required configurations is not set."
  exit 1
fi
...
output="$(dirname "$0")/data_migration_$timestamp"
...
#migrate data for selected frameworks
