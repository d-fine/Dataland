...
echo $KEYCLOAK_DATALAND_ADMIN_PASSWORD
...
services="backend api-key-manager data-sourcing-service document-manager internal-storage qa-service community-manager email-service data-exporter specification-service user-service accounting-service"
