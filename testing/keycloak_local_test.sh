...
echo "Check if test company name was passed as argument."
...
if [[ $postcompany_response =~ $regex ]]; then
  ...
  echo "Matching successful, company could be posted. Company Id is ${test_companyId}."
else
  echo "Matching failed."
  exit 1
fi
echo "Trying to get the company that was posted in the last step with user jwt token."
getspecificcompany_response=$(curl --location --request GET "${backend_url}/companies/${test_companyId}" --header "Authorization: Bearer ${jwt_token_user}")
...
echo "Post one EU taxonomy data set for test company ${test_company_name} and one data set for teaser company ${teaser_company_name} and use admin jwt token."
