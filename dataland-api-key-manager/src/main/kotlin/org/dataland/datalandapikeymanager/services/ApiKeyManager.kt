...
class ApiKeyManager(
  ...
  private val validationMessageWrongApiKey =
    "The API key you provided for your Dataland account is not correct."
  private val validationMessageExpiredApiKey =
  ...
  private val revokementMessageSuccess =
  ...
  /**
   * Gets meta info about the API key of a keycloak user ID, which is derived from the Bearer token
   * that is used for authorization. The Frontend needs this meta info to display the current API
   * key status to the user.
   *
   * @return is an ApiKeyMetaInfo object with all available information about the API key, which can
   *   be used by the Frontend to inform the user about the current status (valid key/expired key/no
   *   key at all/error)
   */
  fun getApiKeyMetaInfoForFrontendUser(): ApiKeyMetaInfo {
    ...
    return if (apiKeyEntityOptional.isEmpty) {
      logger.info(
        "Dataland user with the Keycloak user Id $keycloakUserId has no API key registered."
      )
  ...
  fun validateApiKey(receivedApiKey: String): ApiKeyMetaInfo {
    ...
    val apiKeyEntityOptional = apiKeyRepository.findById(receivedAndParsedApiKey.keycloakUserId)
  ...
  private fun getApiKeyMetaInfo(secret: String, apiKeyEntity: ApiKeyEntity): ApiKeyMetaInfo {
    ...
    logger.info(
      "Validated Api Key for user ${apiKeyEntity.keycloakUserId} as active=${apiKeyMetaInfo.active}"
    )
    return apiKeyMetaInfo
  }
