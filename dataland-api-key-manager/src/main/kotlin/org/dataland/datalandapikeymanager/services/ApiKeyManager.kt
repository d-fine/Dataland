...
(@Autowired private val storedHashedAndBase64EncodedApiKeyRepository: StoredHashedAndBase64EncodedApiKeyRepository) {
    ...
    fun generateNewApiKey(daysValid: Int?): ApiKeyAndMetaInfo {
        ...
        val keycloakUserIdBase64Encoded = encodeToBase64(apiKeyMetaInfo.keycloakUserId!!.toByteArray(Charsets.UTF_8))
        ...
        val newApiKeyHashedAndBase64Encoded = encodeToBase64(hashString(newApiKey, newSalt))
        ...
        val hashedAndBase64EncodedApiKeyEntityToStore = StoredHashedAndBase64EncodedApiKeyEntity(
            ...
            encodeToBase64(newSalt)
        )

        storedHashedAndBase64EncodedApiKeyRepository.save(hashedAndBase64EncodedApiKeyEntityToStore)
        logger.info(
            "Generated Api Key with hashed value $newApiKeyHashedAndBase64Encoded and meta info $apiKeyMetaInfo."
        )
    ...
    private fun checkIfApiKeyForUserIsCorrectAndReturnApiKeyMetaInfoWithActivityStatus(
        ...
        if (receivedApiKeyHashedAndBase64Encoded !=
            storedHashedAndBase64EncodedApiKeyOfUser.apiKeyHashedAndBase64Encoded
        ) {
            logger.info(
                "The provided Api Key " +
                    "for the encoded Keycloak user Id $keycloakUserIdBase64Encoded is not correct."
            )
            ...
            return storedHashedAndBase64EncodedApiKeyOfUser.apiKeyMetaInfo.copy(
    ...
    fun validateApiKey(receivedApiKey: String): ApiKeyMetaInfo {
        ...
        val storedHashedAndBase64EncodedApiKeyOfUser =
            storedHashedAndBase64EncodedApiKeyOfUserOptional.get().toApiModel()
        val salt = EncodingUtils.decodeFromBase64(storedHashedAndBase64EncodedApiKeyOfUser.saltBase64Encoded)
        val receivedApiKeyHashedAndBase64Encoded = encodeToBase64(hashString(receivedApiKey, salt))

        return checkIfApiKeyForUserIsCorrectAndReturnApiKeyMetaInfoWithActivityStatus(
            receivedApiKeyHashedAndBase64Encoded,
            storedHashedAndBase64EncodedApiKeyOfUser,
            receivedAndParsedApiKey.parsedKeycloakUserIdBase64Encoded
        )
    }
    ...
    fun revokeApiKey(): RevokeApiKeyResponse {
        ...
        if (storedHashedAndBase64EncodedApiKeyRepository.findById(
            ...
            logger.info(
                "No revokement took place since there is no API key registered for the encoded " +
            ...
            logger.info(
                "The API key for the encoded Keycloak user Id $keycloakUserIdBase64Encoded was successfully " +
