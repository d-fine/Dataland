...
interface ApiKeyAPI {
   ...
   *
   * @param daysValid defines how many days the generated API key can be used, a null value results
   *   in an infinite validity
  ...
  @ApiResponses(
    value = [ApiResponse(responseCode = "200", description = "Successfully retrieved new api key.")]
  )
  @GetMapping(value = ["/generateApiKey"], produces = ["application/json"])
   ...
   * Information about the user is derived from the Bearer token of the request. This method is
   * needed by the Frontend to display the API key status to a logged in user.
   *
  ...
  @GetMapping(value = ["/getApiKeyMetaInfoForUser"], produces = ["application/json"])
  @PreAuthorize("hasRole('ROLE_USER')")
  @SecurityRequirement(name = "default-bearer-auth")
  @SecurityRequirement(name = "default-oauth")
  ...
  @GetMapping(value = ["/validateApiKey"], produces = ["application/json"])
   ...
   *
   * @return a response model which informs about the success of the revokement process together
   *   with a message.
   */
  @Operation(
  ...
  @PostMapping(value = ["/revokeApiKey"], produces = ["application/json"])
  @PreAuthorize("hasRole('ROLE_USER')")
  @SecurityRequirement(name = "default-bearer-auth")
  @SecurityRequirement(name = "default-oauth")
