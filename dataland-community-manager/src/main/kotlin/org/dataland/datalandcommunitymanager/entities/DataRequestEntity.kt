...
data class DataRequestEntity(
  @Id @Column(name = "data_request_id") val dataRequestId: String,
  val userId: String,
  val creationTimestamp: Long,
  val dataType: String,
  val reportingPeriod: String,
  val datalandCompanyId: String,
  @OneToMany(mappedBy = "dataRequest") var messageHistory: List<MessageEntity>,
  @OneToMany(mappedBy = "dataRequest") var dataRequestStatusHistory: List<RequestStatusEntity>,
  var lastModifiedDate: Long,
) {
  val requestStatus: RequestStatus
    get() =
      (dataRequestStatusHistory.maxByOrNull { it.creationTimestamp }?.requestStatus)
        ?: RequestStatus.Open

  val accessStatus: AccessStatus
    get() =
      (dataRequestStatusHistory.maxByOrNull { it.creationTimestamp }?.accessStatus)
        ?: AccessStatus.Public
  ...
  /**
   * Adds a messageEntity to the messageHistory. Note, this is not automatically saved in the
   * database, you also need to persist the messageEntity.
   */
  ...
  /**
   * Adds a requestStatusEntity to the dataRequestStatusHistory. Note, this is not automatically
   * saved in the database, you also need to persist the requestStatusEntity.
   */
  ...
  fun getDataTypeDescription(): String {
    return DataTypeEnum.entries
      .find { it.value == dataType }
      .let { readableFrameworkNameMapping[it] } ?: dataType
  }
}
