...
data class MessageEntity(
  ...
  @Column(columnDefinition = "TEXT") val contacts: String,
  @Column(columnDefinition = "TEXT") val message: String?,
  ...
  companion object {
    ...
    /**
     * This method checks if a contact is either a valid email address or represents the company
     * owner
     *
     * @param contact which should be checked if it is an email address or the company owner keyword
     * @return true if either it was confirmed that it is a email address or the company owner
     *   keyword
     */
    private fun isContact(
      ...
      return contact.isEmailAddress() ||
        (contact == COMPANY_OWNER_KEYWORD &&
          companyRolesManager
            .getCompanyRoleAssignmentsByParameters(CompanyRole.CompanyOwner, companyId, null)
            .isNotEmpty())
    }

    /**
     * This method checks whether a given contact is valid or not. If it is not valid then an
    ...
    /**
     * This method adds a contact email address of the user id or a company owner to the email
     * recipient list
     *
     * @param contact is either an email address or the company owner keyword
     * @param companyRolesManager is the service to handle all tasks in regard to company roles for
     *   users
  ...
  init {
    ...
    require(
      contacts.split(emailSeparator).all { it.isEmailAddress() || it == COMPANY_OWNER_KEYWORD }
    )
