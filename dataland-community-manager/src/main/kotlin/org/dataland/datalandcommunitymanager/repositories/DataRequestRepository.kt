...
interface DataRequestRepository : JpaRepository<DataRequestEntity, String> {
  ...
  /**
   * This method queries data requests and aggregates all the userIds, so that the result contains
   * the count of data requests for one specific identifierValue, identifierType and framework. It
   * also filters these results based on the provided identifier value and frameworks.
   *
   * @param reportingPeriod to check for
  ...
  /**
   * A function for searching for data request information by dataType, userID, requestID,
   * requestStatus, accessStatus, reportingPeriod or dataRequestCompanyIdentifierValue
   *
  ...
  @Query(
    ...
    value =
      TemporaryTables.TABLE_FILTERED +
        TemporaryTables.TABLE_FILTERED_ORDER_AND_LIMIT +
        TemporaryTables.TABLE_FILTERED_END +
  ...
  /**
   * This method counts the number of data requests that a user has performed from a specified
   * timestamp.
   *
   * @param userId to check for
  ...
  fun getNumberOfDataRequestsPerformedByUserFromTimestamp(userId: String, timestamp: Long): Int
}
