...
interface RequestApi {
  ...
  fun postBulkDataRequest(
    @Valid @RequestBody bulkDataRequest: BulkDataRequest
  ...
  fun postSingleDataRequest(
    @Valid @RequestBody singleDataRequest: SingleDataRequest
  ...
  @GetMapping(value = ["/{dataRequestId}"], produces = ["application/json"])
  @PreAuthorize(
    "hasRole('ROLE_ADMIN') or @SecurityUtilsService.isUserAskingForOwnRequest(#dataRequestId)"
  )
  ...
  @ApiResponses(
    value = [ApiResponse(responseCode = "200", description = "Successfully patched data request.")]
  )
  @PatchMapping(value = ["/{dataRequestId}/requestStatus"], produces = ["application/json"])
  @PreAuthorize(
   ...
   * @param emailAddress If set, only the requests from users which email address partially matches
   *   emailAddress are returned
   * @param requestStatus If set, only the requests with a request status in requestStatus are
   *   returned
   * @param accessStatus If set, only the requests with an access status in accessStatus are
   *   returned
  ...
  @GetMapping(value = ["/numberOfRequests"], produces = ["application/json"])
  @PreAuthorize(
    "hasRole('ROLE_ADMIN') or" +
      "@SecurityUtilsService.isUserCompanyOwnerForCompanyId(#datalandCompanyId)"
  )
  ...
  /**
   * A method to check if the logged-in user can access a specific dataset. The dataset is specified
   * by a companyId, dataType and a reportingPeriod.
   *
    ...
    value =
      [
        ...
        ApiResponse(
          responseCode = "404",
          description =
            "Either the specified dataset does not exist or the user cannot access the dataset.",
        ),
      ]
  )
  @RequestMapping(
