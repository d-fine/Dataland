 ...
 */
@RestController
class CompanyRolesController(@Autowired private val companyRolesManager: CompanyRolesManager) :
  CompanyRolesApi {
  ...
  override fun assignCompanyRole(
    ...
    return ResponseEntity.ok(
      companyRolesManager
        .assignCompanyRoleForCompanyToUser(companyRole, companyId.toString(), userId.toString())
        .toApiModel()
    )
  }
  ...
  override fun getCompanyRoleAssignments(
    ...
    return ResponseEntity.ok(entities.map { it.toApiModel() })
  }

  override fun removeCompanyRole(companyRole: CompanyRole, companyId: UUID, userId: UUID) {
    logger.info(
