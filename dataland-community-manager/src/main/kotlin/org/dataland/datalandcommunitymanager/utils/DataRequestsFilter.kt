...
/**
 * A filter class used in the searchDataRequestEntity-Method which allows convenient usage of SEPL
...
data class DataRequestsFilter(
  ...
  /**
   * This function should be called when the email address filter is not empty, i.e. if
   * shouldFilterByEmailAddress is true. The keycloakUserControllerApiService is required to get the
   * user ids for the email addresses.
   */
  fun setupEmailAddressFilter(
