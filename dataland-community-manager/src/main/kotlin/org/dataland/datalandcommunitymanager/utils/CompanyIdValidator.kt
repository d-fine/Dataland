...
/**
 * Implementation of a service that can check if a companyId exists on Dataland by asking the
 * backend microservice
 */
@Service("CompanyIdValidator")
class CompanyIdValidator(@Autowired private val companyApi: CompanyDataControllerApi) {

  /**
   * Checks if a companyId exists on Dataland by trying to retrieve it in the backend. If it does
   * not exist the method catches the not-found-exception from the backend and throws a
   ...
   *
   * @param companyId is the companyId to check for
   * @returns nothing if the company exists or throws an resource not found exception if the company
   *   does not exists
   */
