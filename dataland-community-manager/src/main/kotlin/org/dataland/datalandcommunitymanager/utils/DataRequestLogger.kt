...
/**
 * Implementation of a request manager service for all operations concerning the processing of data
 * requests
 */
...
class DataRequestLogger {
  ...
  private val singleDataRequestLogger =
    LoggerFactory.getLogger(SingleDataRequestManager::class.java)
  ...
  /**
   * Logs an appropriate message when it has been checked if a specific data request already exists
   * and that check returned "true".
   */
  ...
  /**
   * Logs an appropriate message when it has been checked if an identifier value can be
   * cross-referenced with a companyId that already exists on Dataland.
   */
  ...
  fun logMessageForPatchingRequestMessage(dataRequestId: String) {
    singleDataRequestLogger.info("Patching request $dataRequestId " + "with new message.")
  }
}
