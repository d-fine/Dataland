...
class DataRequestProcessingUtils(
  ...
  /**
   * Returns the ID of the company corresponding to a provided identifier value, else null if none
   * is found
   *
   * @param identifierValue the identifier value
   * @param returnOnlyUnique boolean if only unique matches should be returned (null if not unique)
   *   or if an exception will be thrown (default)
  ...
  fun getDatalandCompanyIdAndNameForIdentifierValue(
    ...
    dataRequestLogger.logMessageWhenCrossReferencingIdentifierValueWithDatalandCompanyId(
      identifierValue,
  ...
  /**
   * For a given dataRequestEntity, this function adds and persists a new entry to the
   * messageHistory of the dataRequestEntity. The new entry contains a set of contacts, an optional
   * message and a modificationTime. This function should be called within a transaction.
   */
  ...
  /**
   * For a given dataRequestEntity, this function adds and persists a new entry to the
   * requestStatusHistory of the dataRequestEntity. The new entry contains a requestStatis, an
   * accessStatus and a modificationTime. This function should be called within a transaction.
   */
  fun addNewRequestStatusToHistory(
    ...
    val requestStatusObject =
      StoredDataRequestStatusObject(requestStatus, modificationTime, accessStatus)
  ...
  /**
   * Retrieves the data requests already existing on Dataland for the provided specifications and
   * the current user
   *
  ...
  fun findAlreadyExistingDataRequestForCurrentUser(
    ...
    val foundRequests =
      dataRequestRepository
        ...
        ?.filter { it.requestStatus == requestStatus }
  ...
  /**
   * Checks whether a request already exists on Dataland in a non-final status (i.e. in status
   * "Open" or "Answered"
   *
