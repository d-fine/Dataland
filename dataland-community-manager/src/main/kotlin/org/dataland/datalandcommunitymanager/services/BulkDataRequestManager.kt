...
/**
 * Implementation of a request manager service for all operations concerning the processing of bulk
 * data requests
 */
...
class BulkDataRequestManager(
   ...
   *
   * @param bulkDataRequest info provided by a user in order to request a bulk of datasets on
   *   Dataland.
  ...
  fun processBulkDataRequest(bulkDataRequest: BulkDataRequest): BulkDataRequestResponse {
    ...
    for (userProvidedIdentifier in bulkDataRequest.companyIdentifiers) {
      val datalandCompanyIdAndName =
        utils.getDatalandCompanyIdAndNameForIdentifierValue(
          userProvidedIdentifier,
          returnOnlyUnique = true,
        )
    ...
    sendBulkDataRequestInternalEmailMessage(
      bulkDataRequest,
      userProvidedIdentifierToDatalandCompanyIdMapping.values.toList(),
      correlationId,
    )
  ...
  private fun storeDataRequests(
    ...
    for (framework in dataTypes) {
      for (reportingPeriod in reportingPeriods) {
        if (
          !utils.existsDataRequestWithNonFinalStatus(datalandCompanyId, framework, reportingPeriod)
        ) {
  ...
  private fun errorMessageForEmptyInputConfigurations(
    ...
    return when {
      identifiers.isEmpty() && frameworks.isEmpty() && reportingPeriods.isEmpty() ->
        "All " + "provided lists are empty."

      identifiers.isEmpty() && frameworks.isEmpty() ->
        "The lists of company identifiers and " + "frameworks are empty."

      identifiers.isEmpty() && reportingPeriods.isEmpty() ->
        "The lists of company identifiers and " + "reporting periods are empty."

      frameworks.isEmpty() && reportingPeriods.isEmpty() ->
        "The lists of frameworks and reporting " + "periods are empty."
  ...
  private fun assureValidityOfRequests(bulkDataRequest: BulkDataRequest) {
    ...
    if (identifiers.isEmpty() || frameworks.isEmpty() || reportingPeriods.isEmpty()) {
      val errorMessage =
        errorMessageForEmptyInputConfigurations(identifiers, frameworks, reportingPeriods)
  ...
  private fun buildResponseMessageForBulkDataRequest(
    ...
    return when (numberOfRejectedCompanyIdentifiers) {
      0 ->
        "All of your $totalNumberOfRequestedCompanyIdentifiers distinct company identifiers were accepted."
      1 ->
  ...
  private fun throwInvalidInputApiExceptionBecauseAllIdentifiersRejected() {
    ...
    val message =
      "The company identifiers you provided could not be uniquely matched with an existing " +
        "company on dataland"
    throw InvalidInputApiException(summary, message)
  }
}
