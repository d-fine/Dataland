...
class DataRequestTimeScheduler(
  ...
  /**
   * Cron job that identifies stale answered requests, patches them to closed and triggers an email
   * notification
   */
  ...
  fun patchStaleAnsweredRequestToClosed() {
    ...
    val searchFilterForAnsweredDataRequests =
      DataRequestsFilter(null, null, null, null, null, setOf(RequestStatus.Answered), null)
    val staleAnsweredRequests =
      dataRequestRepository.searchDataRequestEntity(searchFilterForAnsweredDataRequests).filter {
        it.lastModifiedDate < thresholdTime
      }
