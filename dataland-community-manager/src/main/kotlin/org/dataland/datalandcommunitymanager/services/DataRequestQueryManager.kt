...
constructor(
  ...
  /**
   * This method retrieves all the data requests for the current user from the database and logs a
   * message.
   *
   ...
   */
  private fun convertRequestEntityToExtendedStoredDataRequest(
    dataRequestEntity: DataRequestEntity
  ): ExtendedStoredDataRequest {
    val companyInformation =
      companyDataControllerApi.getCompanyInfo(dataRequestEntity.datalandCompanyId)
  ...
  fun getDataRequestById(dataRequestId: String): StoredDataRequest {
    ...
    val emailAddress =
      keycloakUserControllerApiService.getUser(dataRequestEntity.userId).email ?: ""
  ...
  fun getDataRequests(
    ...
    val userIdsToEmails =
      usersMatchingEmailFilter.associate { it.userId to it.email }.toMutableMap()
      ...
      extendedStoredDataRequests.map {
        ...
        it.userEmailAddress =
          it.userId
            ...
            ?.let {
              userIdsToEmails.getOrPut(it) {
                keycloakUserControllerApiService.getUser(it).email ?: ""
              }
            }
