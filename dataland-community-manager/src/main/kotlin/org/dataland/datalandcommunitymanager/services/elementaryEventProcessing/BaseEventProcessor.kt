...
/**
 * Base class to define basic functionalities required for processing elementary events on message
 * queues
 */
@Component
abstract class BaseEventProcessor(
   ...
   * elementary events and potentially triggering notifications Event-specific processing logic
  ...
  fun processEvent(
    ...
    val storedElementaryEvent =
      createAndSaveElementaryEvent(elementaryEventBasicInfo, elementaryEventType)
  ...
  /**
   * Each EventProcessor listens to a different messageQueue which will contain different message
   * payloads. Thus, the payload validation needs to be implemented in the child classes.
   *
  ...
  /**
   * Parses a message payload from the rabbit mq as object. The object mapper itself throws errors
   * if parsing is not possible.
   *
   * @param jsonString the content of the message
   * @returns an object that contains basic info about the elementary event associated with the
   *   payload
   */
  ...
  private fun isNotificationServiceEnabled(): Boolean {
    val isNotificationServiceEnabled =
      notificationFeatureFlagAsString?.toBooleanStrictOrNull() ?: false
    if (!isNotificationServiceEnabled) {
      logger.info(
        "Notification service feature flag is disabled. Skipping elementary event processing."
      )
    }
