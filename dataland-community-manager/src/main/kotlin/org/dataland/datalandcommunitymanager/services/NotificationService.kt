...
/**
 * Service that handles creation of notification events and sending notifications to interested
 * parties in case of elementary events
 */
...
constructor(
  ...
  enum class NotificationEmailType {
    Single,
    Summary,
  }

  /**
   * Checks if notification event shall be created or not. If yes, it creates it and sends a message
   * to the queue to trigger notification emails.
   */
  @Transactional
  fun notifyOfElementaryEvents(
    ...
    determineNotificationEmailType(latestElementaryEvent, unprocessedElementaryEvents)?.let {
  ...
  /**
   * Checks if the requirements for creating a notification event are met. If yes, it returns the
   * type of notification mail that shall be sent. Else it simply returns null.
   */
  ...
  /**
   * Creates and persists a new notification event and also puts the reference to this newly created
   * notification event into the associated elementary events.
   */
   ...
   * @param elementaryEventType of the elementary events for which the notification event was
   *   created
  ...
  fun getLastNotificationEventOrNull(
    ...
    return notificationEventRepository
      .findNotificationEventByCompanyIdAndElementaryEventType(companyId, elementaryEventType)
      .maxByOrNull { it.creationTimestamp }
  }

  /**
   * Gets days passed since last notification event for a specific company. If there was no last
   * notification event, it returns "null".
   *
   * @param companyId for which a notification event might have happened
   * @param elementaryEventType of the elementary events for which the notification event was
   *   created
  ...
  fun getDaysPassedSinceLastNotificationEvent(
    ...
    return getLastNotificationEventOrNull(companyId, elementaryEventType)?.let {
