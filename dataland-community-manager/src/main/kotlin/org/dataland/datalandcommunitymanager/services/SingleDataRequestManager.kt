...
/**
 * Implementation of a request manager service for all operations concerning the processing of
...
constructor(
   ...
   *
   * @param singleDataRequest info provided by a user in order to request a single dataset on
   *   Dataland
  ...
  fun processSingleDataRequest(singleDataRequest: SingleDataRequest): SingleDataRequestResponse {
    ...
    singleDataRequest.reportingPeriods.forEach { reportingPeriod ->
      val processedReportingPeriod = processReportingPeriod(reportingPeriod, preprocessedRequest)
    ...
    return buildResponseForSingleDataRequest(
      singleDataRequest,
      reportingPeriodsMap[ReportingPeriodKeys.ReportingPeriodsOfStoredDataRequests]?.toList()
        ?: listOf(),
      reportingPeriodsMap[ReportingPeriodKeys.ReportingPeriodsOfDublicateDataRequests]?.toList()
        ?: listOf(),
      reportingPeriodsMap[ReportingPeriodKeys.ReportingPeriodsOfDataAccessRequests]?.toList()
        ?: listOf(),
    )
  }
  ...
  private fun processReportingPeriod(
    ...
    return if (
      shouldCreateAccessRequestToPrivateDataset(
  ...
  private fun shouldCreateAccessRequestToPrivateDataset(
    ...
    return (dataType == DataTypeEnum.vsme &&
  ...
  private fun performQuotaCheckForNonPremiumUser(numberOfReportingPeriods: Int, companyId: String) {
    ...
    if (
      !userInfo.roles.contains(DatalandRealmRole.ROLE_PREMIUM_USER) &&
      ...
      val numberOfDataRequestsPerformedByUserFromTimestamp =
        dataRequestRepository.getNumberOfDataRequestsPerformedByUserFromTimestamp(
          userInfo.userId,
          getEpochTimeStartOfDay(),
        )

      if (
        numberOfDataRequestsPerformedByUserFromTimestamp + numberOfReportingPeriods >
          maxRequestsForUser
      ) {
  ...
  private fun sendDataAccessRequestEmailMessage(
    ...
    if (reportingPeriodsOfStoredAccessRequests.isNullOrEmpty()) {
      ...
      val dataTypeDescription =
        readableFrameworkNameMapping[preprocessedRequest.dataType]
          ?: preprocessedRequest.dataType.toString()
  ...
  private fun buildResponseForSingleDataRequest(
    ...
    return SingleDataRequestResponse(
      buildResponseMessageForSingleDataRequest(
        ...
        numberOfReportingPeriodsCorrespondingToDuplicates =
          reportingPeriodsOfDuplicateDataRequests.size,
      ),
