...
class PrivateDataUploadProcessor(
  ...
  /**
   * Method that listens to private data storage requests, persists them as elementary events and
   * asks the Notification service to potentially send notifications
   *
   * @param payload content of the private data storage message
   * @param correlationId the correlation ID of the current user process that has triggered this
   *   message
  ...
  fun processEvent(
    ...
    super.processEvent(createElementaryEventBasicInfo(payload), correlationId, type)
  }
  ...
  override fun validateIncomingPayloadAndReturnDataId(
    ...
    return payloadJsonObject.getString("dataId").takeIf { it.isNotEmpty() }
