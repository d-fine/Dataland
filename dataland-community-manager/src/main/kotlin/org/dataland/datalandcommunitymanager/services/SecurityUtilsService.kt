...
/**
 * Implements utility functions that can be used e.g., in PRE_AUTHORIZE for several authentication
 * use-cases
 */
...
class SecurityUtilsService(
  ...
  /**
   * Returns true if the request status is subject to change and conditions are met. This is the
   * case when the request is to be changed from answered to either closed or open or if the status
   * is changed to withdrawn.
   */
  @Transactional
  fun isRequestStatusChangeableByUser(
    ...
    val currentRequestStatus =
      dataRequestRepository.findById(requestId.toString()).get().requestStatus
    val statusChangeFromAnsweredToResolved =
      currentRequestStatus == RequestStatus.Answered &&
        requestStatusToPatch == RequestStatus.Resolved
    ...
    return (statusChangeFromAnsweredToResolved ||
  ...
  /**
   * Returns true if the request message history is subject to change and conditions are met. This
   * is the case when no contacts are provided or the request status is open or answered and patched
   * to open as well
   */
  @Transactional
   ...
   */
  @Transactional
  fun isNotTryingToPatchAccessStatus(accessStatusPatch: AccessStatus?): Boolean {
  ...
  fun isUserCompanyOwnerForCompanyId(companyId: String?): Boolean {
    return if (companyId.isNullOrBlank()) {
      ...
      try {
        ...
        logger.error(
          "The user is not the company owner for the specified company. Catched error: $e"
        )
        false
      }
    }
