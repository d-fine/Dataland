...
/**
 * Class to manage the clients interaction with keycloak, e.g. retrieving user information like
 * email address
 */
...
class KeycloakUserControllerApiService(
  ...
  fun getUser(userId: String): KeycloakUserInfo {
    ...
    try {
      val user = objectMapper.readValue(response, KeycloakUserInfo::class.java)
      return user
    } catch (e: JacksonException) {
      logger.warn(
        "Failed to parse response from Keycloak. userId $userId. Response $response, exception: $e"
      )
  ...
  fun searchUsers(emailAddressSearchString: String): List<KeycloakUserInfo> {
    ...
    try {
      val listOfUsers: List<KeycloakUserInfo> =
        objectMapper.readValue(response, object : TypeReference<List<KeycloakUserInfo>>() {})
      return listOfUsers
    } catch (e: JacksonException) {
