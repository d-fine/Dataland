...
class DataAccessManager(
  ...
  /**
   * This method finds all DataRequestsEntity for a specified dataset that have a specific
   * accessStatus.
   *
  ...
  /**
   * This method checks if the requesting user has been granted access to the information for a
   * specific company, reporting period and data type. If the datatype input has no correspond entry
   * in the datatype enum class then an exception thrown If datatype is not vsme the method returns
  ...
  fun hasAccessToDataset(
    ...
    val hasAccess = hasAccessToPrivateDataset(companyId, reportingPeriod, dataTypeEnum, userId)
    ...
    if (!existingRequestsOfUser.isNullOrEmpty()) {
      ...
      if (
        ...
        dataRequestProcessingUtils.addNewRequestStatusToHistory(
          ...
          requestStatus = dataRequestEntity.requestStatus,
          accessStatus = AccessStatus.Pending,
          modificationTime = modificationTime,
        )
        logger.info(
          "Patching request ${dataRequestEntity.dataRequestId} with status ${AccessStatus.Pending}"
        )
      }
    } else {
   ...
   * @return true if the access request already exists in pending status for the current user, false
   *   otherwise
   */
  ...
  fun findAlreadyExistingAccessRequestForCurrentUser(
    ...
    val foundRequests =
      dataRequestRepository
        ...
        ?.filter { it.accessStatus == accessStatus }
