...
class NotificationEmailSender(
    ...
    fun sendExternalAndInternalNotificationEmail(
        ...
        val (externalEmailContent, internalEmailContent) = when (notificationEmailType) {
    ...
    private fun buildExternalAndInternalSingleEmail(
        ...
        val internalEmailContent = KeyValueTable(
            internalEmailSubject, internalEmailTextTitle, internalEmailHtmlTitle,
            listOf(
                "Company" to companyIdAndNameContent(
    ...
    private fun buildExternalAndInternalSummaryEmail(
        ...
        val externalEmailContent = MultipleDatasetsUploadedEngagement(
            companyName = companyName,
            companyId = latestElementaryEvent.companyId.toString(),
            frameworkData = frameworkData.map {
    ...
    private fun companyIdAndNameContent(companyId: String, companyName: String): Value =
        Value.List(
    ...
    private fun dataTypeLink(dataType: DataTypeEnum, companyId: String) : Value =
        Value.RelativeLink("/companies/${companyId}/frameworks/${dataType.value}", dataType.name)

    private fun frameworkValue(frameworkData: Map<DataTypeEnum, List<String>>, companyId: String) : Value =
        frameworkData.map {
            Value.List(
