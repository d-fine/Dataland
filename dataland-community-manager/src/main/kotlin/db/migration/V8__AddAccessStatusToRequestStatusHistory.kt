...
class V8__AddAccessStatusToRequestStatusHistory : BaseJavaMigration() {
  ...
  private fun addAccessStatusToStatusHistoryTable(context: Context) {
    context.connection
      .createStatement()
      .execute("ALTER TABLE request_status_history " + "ADD COLUMN access_status varchar(255) ")
  }

  private fun makeAccessStatusRowNonNullable(context: Context) {
    context.connection
      .createStatement()
      .execute("ALTER TABLE request_status_history " + "ALTER COLUMN access_status SET NOT NULL ")
  }

  private fun migrateSmeToVsme(context: Context) {
    context.connection
      .createStatement()
      .execute("UPDATE data_requests " + "SET data_type = 'vsme' " + "WHERE data_type = 'sme' ")
  }
  ...
  private fun setInitialAccessStatusDependingOnDataType(context: Context) {
    val requestsForNonVsmeDataType =
      context.connection
        .createStatement()
        .executeQuery("SELECT data_request_id FROM data_requests WHERE data_type != 'vsme' ")
