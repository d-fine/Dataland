...
class RequestEmailManagerTest {
  ...
  fun `validate that a access granted email is only sent on granted`() {
    ...
    for (accessStatus in AccessStatus.entries) {
      ...
      if (accessStatus == AccessStatus.Granted) {
        verify(accessRequestEmailSender, times(1)).notifyRequesterAboutGrantedRequest(any(), any())
      } else {
        verify(accessRequestEmailSender, times(0)).notifyRequesterAboutGrantedRequest(any(), any())
      }
  ...
  fun `validate that a access requested email is send`() {
    ...
    dataRequestEntity.messageHistory =
      listOf(
        MessageEntity("", MessageEntity.COMPANY_OWNER_KEYWORD, "Message", 0L, dataRequestEntity)
      )
