...
class CompanyRolesManagerTest {
  ...
  private lateinit var companyOwnershipAcceptedEmailMessageSender:
  ...
  fun initializeCompanyRolesManager() {
    ...
    companyOwnershipAcceptedEmailMessageSender =
      mock(CompanyOwnershipAcceptedEmailMessageSender::class.java)
    `when`(
        companyOwnershipAcceptedEmailMessageSender.getNumberOfOpenDataRequestsForCompany(
  ...
  fun `check that a company ownership can only be requested for existing companies`() {
    `when`(mockCompanyDataControllerApi.isCompanyIdValid("non-existing-company-id"))
      .thenThrow(ClientException("Client error", HttpStatus.NOT_FOUND.value()))
    val exception =
      assertThrows<ResourceNotFoundApiException> {
        companyRolesManager.validateIfCompanyHasAtLeastOneCompanyOwner("non-existing-company-id")
      }
  ...
  fun `check that a company ownership can only be requested if the user is not already a company owner`() {
    ...
    `when`(mockCompanyDataControllerApi.getCompanyById(existingCompanyId))
  ...
  fun `check that email for users becoming company company owner is not generated if company does not exist`() {
    `when`(mockCompanyDataControllerApi.getCompanyById(anyString()))
      .thenThrow(ClientException("Client error", HttpStatus.NOT_FOUND.value()))
    val exception =
      assertThrows<ResourceNotFoundApiException> {
