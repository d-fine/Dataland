...
class DataRequestTimeSchedulerTest {
  ...
  private val staleLastModified =
    Instant.now().minus(Duration.ofDays(staleDaysThreshold + 1)).toEpochMilli()
  ...
  fun setUpDataRequestTimeScheduler() {
    ...
    `when`(alterationManager.patchDataRequest(dataRequestIdStaleAndAnswered, RequestStatus.Closed))
      .thenReturn(null)
    dataRequestRepository = mock(DataRequestRepository::class.java)
    dataRequestTimeScheduler =
      DataRequestTimeScheduler(alterationManager, dataRequestRepository, staleDaysThreshold)
  }
