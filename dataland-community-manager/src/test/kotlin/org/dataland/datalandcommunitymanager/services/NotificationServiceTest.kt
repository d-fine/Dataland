...
class NotificationServiceTest {
  ...
  private val testCompanyInformation =
    CompanyInformation(
      ...
      companyContactDetails =
        listOf("emailAddress@dummymail.de", "emailAddress@fakemailingfortest.de"),
    )
  ...
  fun setupNotificationService() {
    ...
    `when`(metaDataControllerApiMock.getDataMetaInfo(anyString()))
      .thenReturn(testDataMetaInformation)
    `when`(companyDataControllerApiMock.getCompanyInfo(testCompanyId.toString()))
  ...
  private fun createNotificationEventEntityForDataUploads(
    ...
    return NotificationEventEntity(
      ...
      elementaryEventType = ElementaryEventType.UploadEvent,
      creationTimestamp =
        Instant.now().minus(creationTimeInDaysBeforeNow, ChronoUnit.DAYS).toEpochMilli(),
    )
  }
  ...
  private fun setTheReturnValueForNotificationEventRepoQuery(
    ...
    `when`(
      ...
      .thenReturn(notificationEventEntitiesToReturn)
  }

  private fun parseJsonStringIntoTemplateEmailMessage(jsonString: String): TemplateEmailMessage {
    return notificationService.objectMapper.readValue(jsonString, TemplateEmailMessage::class.java)
  }
  ...
  fun `summary mail if no notification event in last 30 days and two unprocessed elementary events`() {
    ...
    val unprocessedElementaryEvents =
      listOf(createUploadElementaryEventEntity(60), latestElementaryEvent)
    val notificationEmailType =
      notificationService.determineNotificationEmailType(
        latestElementaryEvent,
        unprocessedElementaryEvents,
  ...
  fun `summary mail if notification event in last 30 days and unprocessed elementary events reach threshold`() {
    ...
    unprocessedElementaryEvents.add(newLatestElementaryEvent)
    ...
    assertEquals(
      ...
      notificationEmailTypeForTenElementaryEvents,
    )
  }
  ...
  fun `getting the last notification event for a company and elementary event type works as expected`() {
    ...
    val lastNotificationEvent =
      notificationService.getLastNotificationEventOrNull(
        testCompanyId,
        ElementaryEventType.UploadEvent,
      )
  ...
  fun `counting the days passed since the last notifiation event works as expected`() {
    ...
    val daysPassedSinceLastNotificationEvent =
      notificationService.getDaysPassedSinceLastNotificationEvent(
        testCompanyId,
        ElementaryEventType.UploadEvent,
      )
  ...
  fun `check if the conversion of frameworks and reporting periods to a single string works as expected`() {
    ...
    val expectedOutputString = "Heimathafen: 2021, 2023<br>SFDR: 2024<br>LkSG: 2020"

    val outputString =
      notificationService.createFrameworkAndYearStringFromElementaryEvents(elementaryEvents)
  ...
  fun `creating and storing a notification event works and is also reflected in the associated elementary events`() {
    ...
    val unprocessedElementaryEvents =
      listOf(latestElementaryEvent, createUploadElementaryEventEntity(11))

    lateinit var storedNotificationEventEntity: NotificationEventEntity

    `when`(
        notificationService.notificationEventRepository.saveAndFlush(
          any(NotificationEventEntity::class.java)
        )
      )
      .thenAnswer { invocation ->
        ...
        assertEquals(
          ElementaryEventType.UploadEvent,
          storedNotificationEventEntity.elementaryEventType,
        )
    ...
    `when`(
        notificationService.elementaryEventRepository.saveAndFlush(
          any(ElementaryEventEntity::class.java)
        )
      )
      .thenAnswer { invocation ->
    ...
    notificationService.createNotificationEventAndReferenceIt(
      latestElementaryEvent,
      unprocessedElementaryEvents,
    )
  }
  ...
  private fun mockBuildingMessageAndSendingItToQueueForSingleMail() {
    ...
    `when`(
      ...
      .then {
        ...
        assertEquals(
          readableFrameworkNameMapping.getValue(testDataType),
          arg1.properties["framework"],
        )
