...
class DataRequestAlterationManagerTest {
  ...
  private fun mockRepos() {
    ...
    `when`<Any>(dataRequestRepository.findById(dataRequestId))
      .thenReturn(Optional.of(dummyDataRequestEntity))
    dummyDataRequestEntities.forEach {
      `when`<Any>(dataRequestRepository.findById(it.dataRequestId)).thenReturn(Optional.of(it))
    }
    `when`(
    ...
    doNothing().`when`(processingUtils).addNewRequestStatusToHistory(any(), any(), any(), any())
    doNothing()
      .`when`(processingUtils)
      .addMessageToMessageHistory(any(), anySet(), anyString(), any())
  }
  ...
  fun setupDataRequestAlterationManager() {
    ...
    `when`(metaDataControllerApi.getDataMetaInfo(metaData.dataId)).thenReturn(metaData)
  ...
  fun `validate that a request response email is sent when a request status is patched to answered or closed`() {
    ...
    verify(requestEmailManager, times(1))
      .sendEmailsWhenStatusChanged(any(), eq(RequestStatus.Answered), eq(null), eq(null))
    dataRequestAlterationManager.patchDataRequest(
      dataRequestId = dataRequestId,
    ...
    verify(requestEmailManager, times(1))
      .sendEmailsWhenStatusChanged(any(), eq(RequestStatus.Closed), eq(null), eq(null))
    verify(processingUtils, times(2)).addNewRequestStatusToHistory(any(), any(), any(), any())
  ...
  fun `validate that no email is sent and the history is updated when an access status is patched`() {
    ...
    verify(processingUtils, times(1)).addNewRequestStatusToHistory(any(), any(), any(), any())
  ...
  fun `validate that a request answered email is sent when request statuses are patched from open to answered`() {
    ...
    verify(processingUtils, times(dummyDataRequestEntities.size))
      .addNewRequestStatusToHistory(any(), any(), any(), any())
    verify(processingUtils, times(0))
      .addMessageToMessageHistory(any(), anySet(), anyString(), any())
  }
  ...
  fun `validate that the sending of a request email is triggered when a request message is added`() {
    ...
    verify(requestEmailManager, times(1)).sendSingleDataRequestEmail(any(), anySet(), anyString())

    verify(processingUtils, times(1))
      .addMessageToMessageHistory(any(), anySet(), anyString(), any())

    verify(processingUtils, times(0)).addNewRequestStatusToHistory(any(), any(), any(), any())
  }
}
