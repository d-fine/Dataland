...
class PrivateDataUploadProcessorTest {
  ...
  fun `empty dataId leads to rejection exception`() {
    val payload =
      JSONObject(mapOf("dataId" to "", "actionType" to ActionType.StorePrivateDataAndDocuments))
        .toString()
    ...
    assertEquals(
      "Message was rejected: The dataId in the message payload is empty.",
      exception.message,
    )
  }
  ...
  fun `unexpected action type leads to rejection exception`() {
    ...
    val payload =
      JSONObject(mapOf("dataId" to "some-data-Id", "actionType" to unexpectedActionType)).toString()
  ...
  fun `happy path of successful validation`() {
    ...
    val actualElementaryEventBasicInfo =
      privateDataUploadProcessor.createElementaryEventBasicInfo(payload)
