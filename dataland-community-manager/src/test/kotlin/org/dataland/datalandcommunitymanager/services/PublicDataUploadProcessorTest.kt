...
class PublicDataUploadProcessorTest {
  ...
  fun `empty identifier leads to rejection exception`() {
    val payload = JSONObject(mapOf("identifier" to "")).toString()

    val exception =
      assertThrows<MessageQueueRejectException> {
        publicDataUploadProcessor.validateIncomingPayloadAndReturnDataId(
          payload,
    ...
    assertEquals(
      "Message was rejected: The identifier in the message payload is empty.",
      exception.message,
    )
  }
  ...
  fun `non empty dataId leads to valid return of dataId`() {
    val dummyId = "123"
    val payload = JSONObject(mapOf("identifier" to dummyId)).toString()
  ...
  fun `create valid elementaryEventBasicInfo`() {
    ...
    val actualElementaryEventBasicInfo =
      publicDataUploadProcessor.createElementaryEventBasicInfo(jsonString)
