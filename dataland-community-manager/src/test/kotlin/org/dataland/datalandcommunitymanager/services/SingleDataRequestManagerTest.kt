...
class SingleDataRequestManagerTest {
  ...
  private val dummyCompanyIdAndName =
    CompanyIdAndName("Dummy Company AG", companyIdRegexSafeCompanyId)
  ...
  private fun createDataRequestRepositoryMock(): DataRequestRepository {
    ...
    `when`(
        dataRequestRepositoryMock.getNumberOfDataRequestsPerformedByUserFromTimestamp(
          anyString(),
          anyLong(),
        )
      )
  ...
  private fun createDataRequestProcessingUtilsMock(): DataRequestProcessingUtils {
    ...
    `when`(utilsMock.storeDataRequestEntityAsOpen(anyString(), any(), anyString(), any(), any()))
      .thenAnswer {
        DataRequestEntity(
  ...
  private fun testWhichEmailMessageIsSentFor(
    ...
    verify(singleDataRequestEmailMessageSenderMock, times(expectedInternalMessagesSent))
      .sendSingleDataRequestInternalMessage(any(), anyString())
  }
  ...
  fun `validate that access request is created and email is send`() {
    ...
    `when`(
      ...
      .thenReturn(
        listOf(
          CompanyRoleAssignmentEntity(CompanyRole.CompanyOwner, companyIdRegexSafeCompanyId, "123")
        )
      )

    `when`(
        utilsMock.matchingDatasetExists(
          companyIdRegexSafeCompanyId,
          reportingPeriod,
          DataTypeEnum.vsme,
        )
  ...
  private fun verifyExpectedBehaviour(
    ...
    verify(accessRequestEmailSender, times(1)).notifyCompanyOwnerAboutNewRequest(any(), any())

    verifyNoInteractions(singleDataRequestEmailMessageSenderMock)

    verify(utilsMock, times(0)).storeDataRequestEntityAsOpen(any(), any(), any(), any(), any())
  }
}
