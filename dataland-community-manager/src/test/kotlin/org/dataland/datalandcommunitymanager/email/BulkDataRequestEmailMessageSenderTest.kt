...
class BulkDataRequestEmailMessageSenderTest {
  ...
  private val bulkDataRequest =
    BulkDataRequest(
      companyIdentifiers =
        setOf("AR8756188701," + "9856177321", "28f05156-e1ba-1ea8-8d1e-d4833f6c7afgh"),
      dataTypes = setOf(DataTypeEnum.p2p, DataTypeEnum.lksg),
      reportingPeriods = setOf("2020, 2023"),
    )
  private val acceptedCompanyIdentifiers =
    listOf(CompanyIdAndName(companyId = "AR8756188701,9856177321", companyName = companyName))
  ...
  fun setup() {
    ...
    Mockito.`when`(companyApiMock.getCompanyInfo(ArgumentMatchers.anyString()))
      .thenReturn(companyInfoMock)
  ...
  private fun buildInternalBulkEmailMessageMock() {
    Mockito.`when`(
      ...
      .thenAnswer() {
        val arg1 =
          objectMapper.readValue(it.getArgument<String>(0), InternalEmailMessage::class.java)
