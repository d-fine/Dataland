...
class WebSecurityConfig(
  ...
  @Value("\${dataland.authorization.apikey.enable:false}")
  private val enableApiKeyAuthentication: Boolean,
) {
  ...
  /**
   * Defines which AuthenticationProviders are to handle specific requests. JWT authentication is
   * always enabled. Api-Key authentication is enabled if enabled in the application settings.
   */
  @Bean
  ...
  private fun authorizePublicLinksAndAddJwtConverter(http: HttpSecurity) {
    ...
    http
      .authorizeHttpRequests {
        it.requestMatchers(*linkMatchers).permitAll().anyRequest().fullyAuthenticated()
      }
      .logout { it.disable() }
      .csrf { it.disable() }
