...
class V17__MigrateReferencedReportsTest {
  ...
  fun `check migration script for SFDR `() {
    TestUtils()
      .testMigrationOfSingleDataset(
        "sfdr",
        "V17/originalSfdr.json",
        "V17/expectedSfdr.json",
        { dataTableEntity ->
          V17__MigrateReferencedReports().migrateReferencedReports(dataTableEntity, "sfdr")
        },
      )
  }
  ...
  fun `check migration script for Non Financial `() {
    TestUtils()
      .testMigrationOfSingleDataset(
        ...
        { dataTableEntity ->
          V17__MigrateReferencedReports()
            .migrateReferencedReports(dataTableEntity, "eutaxonomy-non-financials")
        },
      )
  }
  ...
  fun `check migration script for Financial `() {
    TestUtils()
      .testMigrationOfSingleDataset(
        ...
        { dataTableEntity ->
          V17__MigrateReferencedReports()
            .migrateReferencedReports(dataTableEntity, "eutaxonomy-financials")
        },
      )
  }
}
