...
class DatabaseStringDataStore(
  ...
  /**
   * Method that listens to the storage_queue and stores data into the database in case there is a
   * message on the storage_queue
   *
  ...
  /**
   * Method that stores data into the database in case there is a message on the storage_queue and
  ...
  fun persistentlyStoreDataAndSendMessage(dataId: String, correlationId: String, payload: String) {
    ...
    logger.info(
      "Inserting data into database with data ID: $dataId and correlation ID: $correlationId."
    )
    storeDataItemWithoutTransaction(DataItem(dataId, objectMapper.writeValueAsString(data)))
    cloudEventMessageHandler.buildCEMessageAndSendToQueue(
      payload,
      MessageType.DataStored,
      correlationId,
  ...
  /**
   * Stores a Data Item while ensuring that there is no active transaction. This will guarantee that
   * the write is commited after exit of this method.
   *
  ...
  fun selectDataSet(dataId: String, correlationId: String): String {
    return dataItemRepository
      ...
      .orElseThrow {
        logger.info(
          "Data with data ID: $dataId could not be found. Correlation ID: $correlationId."
        )
  ...
  /**
   * Deletes a Data Item while ensuring that there is no active transaction. This will guarantee
   * that the write is commited after exit of this method.
   *
  ...
  fun deleteDataItemWithoutTransaction(dataId: String, correlationId: String) {
    logger.info("Received DataID $dataId and CorrelationId: $correlationId")
    logger.info(
      "Deleting data from database with data ID: $dataId and correlation ID: $correlationId."
    )
