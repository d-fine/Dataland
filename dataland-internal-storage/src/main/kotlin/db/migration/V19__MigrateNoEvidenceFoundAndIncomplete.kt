...
/**
 * This migration script moves all SFDR Incomplete quality buckets to NoDataFound and removes the
 * value NoEvidenceFound by putting NoDataFound into the quality bucket and setting the value to
 * null
 */
class V19__MigrateNoEvidenceFoundAndIncomplete : BaseJavaMigration() {
   ...
   * 1. If value of the data point is "NoEvidenceFound", set value to null and the quality bucket to
   *    "NoDataFound"
   * 2. If the quality bucket of the data point is "Incomplete" set it to "NoDataFound" instead
   */
  private fun recursivelyChangeRelevantDataPointFields(jsonObject: JSONObject, objectName: String) {
    ...
    if (obj !== null && obj is JSONObject) {
      ...
      if (isExtendedDataPoint) {
        ...
        obj.keys().forEach { recursivelyChangeRelevantDataPointFields(obj, it) }
      }
    }
  }
  ...
  fun migrateSfdrData(dataTableEntity: DataTableEntity) {
    ...
    dataset.keys().forEach { recursivelyChangeRelevantDataPointFields(dataset, it) }
