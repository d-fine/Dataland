...
/**
 * This migration script updates all datasets where the page field is smaller or equal to 0, setting
 * the value to NULL. It also converts valid page integers into strings.
 */
class V23__MigratePageZeroToNull : BaseJavaMigration() {
  ...
  override fun migrate(context: Context?) {
    frameworksToMigrate.forEach { framework ->
      migrateCompanyAssociatedDataOfDatatype(context, framework) { dataTableEntity ->
        migratePageFields(dataTableEntity)
      }
    }
  }

  /**
   * Migrates the page fields in the given `DataTableEntity` to match the new validation rules. If a
   * page field has a value less than or equal to 0, it sets the value to NULL. Valid integer page
      ...
      jsonObject.keys().forEachRemaining { key ->
        ...
        when {
          ...
          key == "page" && value != JSONObject.NULL -> {
            logger.info(
              "Page field has unexpected value '$value' in dataset ${dataTableEntity.dataId}"
            )
            jsonObject.put(key, JSONObject.NULL)
          }
          value is JSONObject -> {
