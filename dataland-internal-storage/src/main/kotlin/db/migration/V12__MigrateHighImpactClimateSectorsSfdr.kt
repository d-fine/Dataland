...
/**
 * This migration script updates the existing SFDR datasets, more specifically it introduces an
 * additional layer to each sector in the high-impact climate sectors
 */
class V12__MigrateHighImpactClimateSectorsSfdr : BaseJavaMigration() {
  ...
  /**
   * Migrate the extended data points corresponding to nace codes in high impact climate sectors to
   * an additional layer with the new field named "highImpactClimateSectorEnergyConsumptionInGWh"
   */
  fun migrateSfdrHighImpactClimateSectors(dataTableEntity: DataTableEntity) {
    ...
    val applicableHighImpactClimateSectorsObject =
      ((dataset.getOrJavaNull("environmental") as JSONObject?)?.getOrJavaNull("energyPerformance")
          as JSONObject?)
    ...
    if (applicableHighImpactClimateSectorsObject !== null) {
      applicableHighImpactClimateSectorsObject.keys().forEach { naceCode ->
        if (applicableHighImpactClimateSectorsObject.has(naceCode)) {
          sectorsToMigrate[naceCode] =
            applicableHighImpactClimateSectorsObject.get(naceCode) as JSONObject
        }
      }
