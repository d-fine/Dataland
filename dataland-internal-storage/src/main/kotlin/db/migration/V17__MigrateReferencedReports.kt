...
/**
 * This migration script removes the isGroupLevel and Currency fields of referencedReports and also
 * renames the reportDate field to publicationDate
 */
class V17__MigrateReferencedReports : BaseJavaMigration() {
  ...
  fun migrateReferencedReports(dataTableEntity: DataTableEntity, framework: String) {
    ...
    if (referencedReports !== null) {
      referencedReports.keys().forEach { removeTwoFieldsAndRenameOne(referencedReports, it) }
    }
  ...
  private fun removeTwoFieldsAndRenameOne(referencedReports: JSONObject, reportName: String) {
