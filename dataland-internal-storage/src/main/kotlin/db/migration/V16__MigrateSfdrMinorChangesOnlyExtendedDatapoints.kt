...
/**
 * This migration script updates the existing SFDR datasets and migrates all existing BaseDataPoints
 * to Extended ones Furthermore, some Points are moved from Wastes to Biodiversity
 */
class V16__MigrateSfdrMinorChangesOnlyExtendedDatapoints : BaseJavaMigration() {
  ...
  private val frameworksToMigrateDataPointsNoSfdr =
    listOf("eutaxonomy-non-financials", "eutaxonomy-financials", "sme")
  ...
  private fun checkRecursivelyForBaseDataPoint(dataset: JSONObject, objectName: String) {
    ...
    if (obj !== null && obj is JSONObject) {
      ...
      obj.keys().forEach { checkRecursivelyForBaseDataPoint(obj, it) }
    }
  }
  ...
  fun migrateSfdrData(dataTableEntity: DataTableEntity) {
    ...
    dataset.keys().forEach { checkRecursivelyForBaseDataPoint(dataset, it) }
  ...
  override fun migrate(context: Context?) {
    ...
    frameworksToMigrateDataPointsNoSfdr.forEach {
      migrateCompanyAssociatedDataOfDatatype(context, it, this::migrateDataPoints)
    }
  }
}
