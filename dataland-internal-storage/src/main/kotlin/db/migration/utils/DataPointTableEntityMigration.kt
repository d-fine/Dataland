...
/**
 * Function that provides a list of all DataPointTableEntities, regardless of their data point type
 */
fun getAllDataPointTableEntities(context: Context?): List<DataPointTableEntity> {
    val objectMapper = ObjectMapper()
    val getQueryResultSet =
        context!!
            .connection
            .prepareStatement("SELECT * from public.data_point_items")
            .executeQuery()
...
/**
 * Gets all data point entries regardless of their data point type, modifies them and writes them back to the table
 * @context the context of the migration script
 * @migrate migration script for a single DataPointTableEntity
 */
fun migrateAllDataPointTableEntities(
    context: Context?,
    migrate: CompanyAssociatedDataPointMigration,
) {
    val dataPointTableEntities = getAllDataPointTableEntities(context)
    dataPointTableEntities.forEach {
        logger.info("Migrating ${it.dataPointType} datapoint with id: ${it.dataPointId}")
        migrate(it)
        it.executeUpdateQuery(context!!)
    }
}
