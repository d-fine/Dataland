...
/**
 * This class provides functionality to help with common issues that might occur while writing a
 * migration script. It provides methods to transform data and rename fields. It provides a buffer
 * for fields that should be removed at a later point which is not possible immediately when
 * iterating over an objects keys
 */
class MigrationHelper {
  private data class FieldToRemove(val fieldHolder: JSONObject, val fieldName: String)
  ...
  fun removeQueuedFields() {
    fieldsToRemove.forEach { it.fieldHolder.remove(it.fieldName) }
  ...
  inline fun <O, reified N> migrateValueFromTo(
    ...
    fieldHolder.put(
      newFieldName,
      transformation((fieldHolder.getOrJavaNull(oldFieldName) ?: return) as O),
    )
  }
  ...
  /**
   * Migrates the transformed value of a field to a new field in the same object and queues the old
   * field for removal
   *
   * @param fieldHolder the object that contains both fields
   * @param newFieldName the name of the field to migrate
  ...
  /**
   * Migrates the transformed value of a data point to a new data point in the same object and
   * queues the old field for removal
   *
  ...
  inline fun <O, reified N> migrateDataPointValueFromTo(
    ...
    val oldDataPointObject =
      (dataPointHolder.getOrJavaNull(oldDataPointName) ?: return) as JSONObject
  ...
  fun migrateReferencedReports(
    ...
    val referencedReportsObject =
      parentCategoryOfReferencedReports.getOrJavaNull("referencedReports") ?: return
  ...
  fun migrateAssurance(
    ...
    if (framework == "euTaxonomyNonFinancials") {
      ...
      check(framework == "euTaxonomyFinancials") {
        "Migration of assurance may not be implemented for " + "this framework"
      }
    }
    ...
    migrationHelper.migrateOneSingleObjectOfDataSource(
      assuranceParentObject,
      dataObject,
      migrationFieldNamesForReports,
      framework,
    )
  }
    ...
    if (dataSourceObject.has("report")) {
      ...
      dataSourceObject.put(
        ...
        getFileReferenceFromReferencedReports(
          fileNameToSearchInReferencedReports,
          dataObject,
          framework,
        ),
      )
  ...
  /**
   * This function reads the fileReference hash from referenced reports in order to store it in the
   * "DataSource" Object.
   */
  private fun getFileReferenceFromReferencedReports(
    ...
    if (fileName != "") {
      ...
      if (framework == "euTaxonomyNonFinancials") {
        ...
        check(framework == "euTaxonomyFinancials") {
          "Retrieval of reference from reports may not be implemented" + " for this framework"
        }
      }
