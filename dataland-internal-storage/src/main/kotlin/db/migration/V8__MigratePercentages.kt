...
/**
 * This migration script updates the old version eutaxonomy for non financials datasets to the new
 * version and the new version is integrated into the old datatype
 */
class V8__MigratePercentages : BaseJavaMigration() {
  companion object {
    ...
    private val euTaxonomyNonFinancialsFinancialShareFields =
      listOf("nonEligibleShare", "eligibleShare", "nonAlignedShare", "alignedShare")
  ...
  override fun migrate(context: Context?) {
    ...
    migrationScriptMapping.forEach {
      migrateCompanyAssociatedDataOfDatatype(context, it.key, it.value)
    }
  }
  ...
  private fun migrateFinancialShare(
    ...
    val financialShareObject =
      (financialShareHolder.getOrJavaNull(financialShareName) ?: return) as JSONObject
    migrationHelper.migrateValue(
      financialShareObject,
  ...
  fun migrateEuTaxonomyFinancials(dataTableEntity: DataTableEntity) {
    migrateDataset(dataTableEntity) { dataObject ->
      val financialServiceTypesWithPercentageDataPointsOnly =
        listOf("creditInstitutionKpis", "investmentFirmKpis", "insuranceKpis")
      ...
      val eligibilityKpis =
        (dataObject.getOrJavaNull("eligibilityKpis") ?: return@migrateDataset) as JSONObject
      eligibilityKpis.keys().forEach { key ->
        val financialServiceKpis =
          (eligibilityKpis.getOrJavaNull(key) ?: return@forEach) as JSONObject
        migrateAllEuTaxonomyFinancialsKpisFor(financialServiceKpis)
      }
    }
  }
  ...
  fun migrateEuTaxonomyNonFinancials(dataTableEntity: DataTableEntity) {
    migrateDataset(dataTableEntity) { dataObject ->
      ...
      listOf("revenue", "capex", "opex").forEach { cashFlowType ->
        val cashFlowObject =
          (dataObject.getOrJavaNull(cashFlowType) ?: return@forEach) as JSONObject
  ...
  private fun migrateLksgSocial(dataObject: JSONObject) {
    val migrationHelper = MigrationHelper()
    ((dataObject.getOrJavaNull("social") as JSONObject?)?.getOrJsonNull(
  ...
  private fun migrateLksgProcurmentCategories(dataObject: JSONObject) {
    val migrationHelper = MigrationHelper()
    (((dataObject.getOrJavaNull("general") as JSONObject?)?.getOrJavaNull(
