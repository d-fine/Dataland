...
/**
 * This migration script migrates the eu taxonomy non financials data to the various new class
 * structures for data points
 */
class V9__MigrateRefactoredDataPointClassesEuTaxNonFinancials : BaseJavaMigration() {
  ...
  private val oldToNewFieldNamesForReports = mapOf("reference" to "fileReference")

  private val oldToNewFieldNamesForDocuments = mapOf("report" to "fileName")

  private val oldToNewFieldNamesForAssurance = mapOf("assurance" to "value")

  private val oldToNewFieldNamesForCurrencyDataPoint = mapOf("unit" to "currency")
  ...
  fun migrateRefactoredDataPointClasses(dataTableEntity: DataTableEntity) {
    ...
    migrationHelper.migrateAssurance(
      dataObject,
      oldToNewFieldNamesForAssurance,
      oldToNewFieldNamesForDocuments,
      migrationHelper,
      framework = "euTaxonomyNonFinancials",
    )
  ...
  /**
   * This function iterates through all cash flow categories in order to migrate the "DataSource"
   * Object
   */
  private fun iterateThroughCashFlowCategories(
    ...
    listOf("revenue", "capex", "opex").forEach { cashFlowType ->
      ...
      migrationHelper.migrateOneSingleObjectOfDataSource(
        parentObjectOfDataSource as JSONObject,
        dataObject,
        migrationFieldNames,
