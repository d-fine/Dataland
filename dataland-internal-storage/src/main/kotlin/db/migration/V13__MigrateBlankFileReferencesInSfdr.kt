...
/**
 * This migration script updates the existing SFDR datasets, more specifically it sets the full
 * dataSource objects to null whenever the corresponding fileReferences are given by an empty string
 */
class V13__MigrateBlankFileReferencesInSfdr : BaseJavaMigration() {
  override fun migrate(context: Context?) {
    migrateCompanyAssociatedDataOfDatatype(context, "sfdr", this::migrateBlankFileReferences)
  }
  ...
  /**
   * Migrate the data points with blank file references to containing null-valued a dataSource
   * instead
   */
