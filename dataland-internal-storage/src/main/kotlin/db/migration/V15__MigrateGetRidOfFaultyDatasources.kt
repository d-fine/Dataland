...
/**
 * This migration script removes all file references that are not hashes and all file references
 * that are hashes but do not exist.
 */
class V15__MigrateGetRidOfFaultyDatasources : BaseJavaMigration() {
   ...
   *
   * blob_id in the blob_items table is the same as the fileReference, the actual hash value of the
   * document. Entries in blob_items are assumed to be consistent with the document_meta_info table
   * of the DocumentManagerDB.
   */
  ...
  override fun migrate(context: Context?) {
    ...
    dataTypesToMigrate.forEach {
      migrateCompanyAssociatedDataOfDatatype(context, it, this::migrateFaultyFileReferences)
    }
  }
  ...
  /**
   * Remove company reports with invalid fileReference from map of company reports. If no company
   * reports are left, replace referencedReports with null.
   */
   ...
   */
  private fun checkRecursivelyForFaultyFileReferences(dataset: JSONObject, objectName: String) {
    ...
    if (obj !== null && obj is JSONObject) {
      ...
      obj.keys().forEach { checkRecursivelyForFaultyFileReferences(obj, it) }
    }
  }
   ...
   *
   * There are three objects storing fileReferences: Base and ExtendedDocumentReference and
   * CompanyReport. ExtendedDocumentReference and BaseDocumentReference are consistently referred to
   * as "dataSource". CompanyReport only occurs list-like as Map<String, CompanyReport>? which is
   * consistently referred to as "referencedReports". Both cases are handled separately.
   */
  fun migrateFaultyFileReferences(dataTableEntity: DataTableEntity) {
    val dataset = dataTableEntity.dataJsonObject
    dataset.keys().forEach { checkRecursivelyForFaultyFileReferences(dataset, it) }
