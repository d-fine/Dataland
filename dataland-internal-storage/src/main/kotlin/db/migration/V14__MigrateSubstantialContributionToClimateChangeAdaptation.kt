...
/**
 * This migration script changes the substring "Adaption" to "Adaptation" in all field names that
 * are "substantialContributionToClimateChangeAdaptionInPercent" in all eu-taxonomy-non-financial
 * datasets
 */
class V14__MigrateSubstantialContributionToClimateChangeAdaptation : BaseJavaMigration() {
  ...
  private val pairOfOldToNewDnshFieldName =
    "dnshToClimateChangeAdaption" to "dnshToClimateChangeAdaptation"

  /**
   * Migrates substantialContributionToClimateChangeAdaption field to
  ...
  fun migrateSubstantialContributionToClimateChangeAdaptation(dataTableEntity: DataTableEntity) {
    ...
    listOf("revenue", "capex", "opex").forEach { cashFlowType ->
      mapOfOldToNewFieldNames.forEach {
        val euTaxoDataSubsetCashFlowType =
          euTaxoDataSet.getOrJavaNull(cashFlowType) ?: return@forEach
