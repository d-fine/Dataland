...
/**
 * This migration script completes the list of referenced reports with all reports referenced in
 * ExtendedDataPoints.
 */
class V18__CompleteListOfReferencedReports : BaseJavaMigration() {

  private val frameworksToMigrate =
    listOf("eutaxonomy-non-financials", "eutaxonomy-financials", "sfdr")
  ...
  private fun findAllDataPoints(jsonObject: JSONObject): List<JSONObject> {
    ...
    return if (
      dataSource !== null && dataSource.has("fileName") && dataSource.has("fileReference")
    ) {
      listOf(dataSource)
    } else {
      jsonObject
        ...
        .asSequence()
        .mapNotNull { key ->
          jsonObject.optJSONObject(key)?.let { subJsonObject -> findAllDataPoints(subJsonObject) }
        }
        .flatten()
        .toList()
    }
  ...
  /**
   * Gets the referencedReports based on the framework If the object is null we insert a new object
  ...
  private fun updateReferencedReports(referencedReports: JSONObject, dataPoints: List<JSONObject>) {
    ...
    val allFileReferences =
      referencedReportsFileReferences.keys.union(dataPointsFileReferences.keys)
  ...
  private fun updateFileReference(
    ...
    if (referencedReport != null) {
      ...
      // there is no referenced report, hence we add a new json object into the list of referenced
      // reports
