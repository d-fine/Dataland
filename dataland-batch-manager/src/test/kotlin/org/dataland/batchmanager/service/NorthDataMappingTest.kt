...
class NorthDataMappingTest {
  ...
  private val expectedMinimalCompanyInformationPatch =
    CompanyInformationPatch(sectorCodeWz = testSectorCodeWz, identifiers = expectedIdentifiers)
  ...
  fun `test north data parsing and conversion`() {
    ...
    assertEquals(expectedMinimalCompanyInformationPatch, onlyElement.toCompanyPatch(setOf("Lei")))
    assertEquals(
  ...
  private fun assertExpectedAndActualNorthDataDataAreTheSame(
    onlyElement: NorthDataCompanyInformation
  ) {
    assertEquals(
