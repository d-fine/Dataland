...
class ProcessDataUpdatesTest {
  ...
  fun `test ingestion performs successfully if a file is provided`() {
    ...
    verify(mockCsvParser, times(1))
      .readGleifCompanyDataFromBufferedReader((any() ?: emptyBufferedReader))
    verify(mockCsvParser, times(1))
      .readGleifRelationshipDataFromBufferedReader((any() ?: emptyBufferedReader))
