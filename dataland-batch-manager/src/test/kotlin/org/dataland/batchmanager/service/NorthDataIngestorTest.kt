...
class NorthDataIngestorTest {
  ...
  fun `test NorthData update process`() {
    ...
    `when`(mockCsvParser.getCsvStreamFromNorthDataZipFile(any() ?: mockFile))
      .thenReturn(emptyBufferedReader)
    `when`(mockCsvParser.readNorthDataFromBufferedReader(emptyBufferedReader))
      .thenReturn(infoIterable)
