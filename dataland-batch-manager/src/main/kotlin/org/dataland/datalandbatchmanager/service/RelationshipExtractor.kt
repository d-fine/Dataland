...
class RelationshipExtractor {
  ...
  /**
   * Converts the return object of the relationship file csv parser to a final map of child lei -
   * parent lei according to a simple logic.
   *
   * @return The final mapping of child LEI to parent LEI as MutableMap<String, String>
   */
  fun prepareFinalParentMapping(
    gleifParser: Iterable<GleifRelationshipInformation>
  ): Map<String, String> {
    val mappings = parseCsvToGroupedMap(gleifParser)

    val localFinalParentMapping =
      mappings[GleifRelationshipTypes.IS_ULTIMATELY_CONSOLIDATED_BY] ?: mutableMapOf()
  ...
  private fun loopAndAddToFinalParentMapIfNotPresent(
    ...
    relationshipMap?.keys?.forEach { startLei ->
      if (startLei !in localFinalParentMapping) {
        relationshipMap[startLei]?.let { parentLei ->
          localFinalParentMapping[startLei] = parentLei
        }
      }
    }
  }
   ...
   *
   * @param gleifParser The iterator returned from the csv reader to loop through the
   *   GleifRelationshipInformation
   */
