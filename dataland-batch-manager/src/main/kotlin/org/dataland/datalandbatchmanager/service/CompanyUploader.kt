...
/**
 * Class for handling the upload of the company information retrieved from GLEIF to the Dataland
 * backend
 */
@Service
class CompanyUploader(
  ...
  private fun checkForDuplicateIdentifierAndGetConflictingCompanyId(
    ...
    if (
      conflictingIdentifiers == null ||
        !conflictingIdentifiers.isArray ||
        conflictingIdentifiers.size() == 0
    ) {
      return Pair(null, null)
    }
    ...
    if (conflictingCompanyIds.size != 1) {
      logger.error(
        "Found conflicting identifiers for two different companies $conflictingCompanyIds"
      )
      return Pair(null, null)
    }
  ...
  /**
   * Uploads a single Company to the dataland backend performing at most MAX_RETRIES retries. If the
   * company (identified by the LEI) already exists on dataland, it is patched instead. This
   * function absorbs errors and logs them.
   */
  ...
  private fun patchSingleCompany(
    ...
    retryOnCommonApiErrors { companyDataControllerApi.patchCompanyById(companyId, companyPatch) }
  }
  ...
  private fun updateIsinsOfCompany(isins: Set<String>, companyId: String) {
    val updatedIdentifiers = mapOf(IdentifierType.Isin.value to isins.toList())
