 ...
 * @param gleifGoldenCopyIngestor reads in the csv file from GLEIF and creates
 *   GleifCompanyInformation objects
 * @param northDataAccessor downloads the golden copy files from NorthData
 * @param northdataDataIngestor reads in the csv file from NorthData and creates
 *   NorthDataCompanyInformation objects
...
class ProcessDataUpdates(
  ...
  /**
   * Downloads the entire GLEIF golden copy file and uploads all included companies to the Dataland
   * Backend. Does so only if the property "dataland.dataland-batch-manager.get-all-gleif-companies"
   * is set.
   */
  fun processFullGoldenCopyFileIfEnabled() {
    ...
    if (allGleifCompaniesForceIngest || flagFileGleif?.exists() == true) {
      ...
      logger.info(
        "Gleif flag file not present & no force update variable set => Not performing any download"
      )
    }
  }
  ...
  /**
   * Downloads the entire NorthData golden copy file and uploads all included companies to the
   * Dataland Backend. Does so only if the property
   * "dataland.dataland-batch-manager.get-all-northdata-companies" is set.
   */
  fun processNorthDataFullGoldenCopyFileIfEnabled() {
    ...
    if (allNorthDataCompaniesForceIngest || flagFileNorthData?.exists() == true) {
      ...
      logger.info(
        "NorthData flag file not present & no force update variable set => Not performing any download"
      )
    }
  }
  ...
  @Suppress(
    "UnusedPrivateMember"
  ) // Detect does not recognise the scheduled execution of this function
