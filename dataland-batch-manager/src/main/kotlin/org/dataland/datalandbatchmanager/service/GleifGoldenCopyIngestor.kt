...
class GleifGoldenCopyIngestor(
  ...
  fun processRelationshipFile(updateAllCompanies: Boolean = false) {
    ...
    val duration = measureTime {
      ...
      if (updateAllCompanies)
        companyUploader.updateRelationships(relationshipExtractor.finalParentMapping)
    }
    logger.info(
      "Finished processing of GLEIF RR file $newRelationshipFile in ${formatExecutionTime(duration)}."
    )
  }
  ...
  private fun uploadCompanies(zipFile: File) {
    ...
    try {
      uploadThreadPool
        .submit {
          StreamSupport.stream(gleifIterable.spliterator(), true).forEach {
  ...
  /**
   * Replaces the locally saved old mapping file with the recently downloaded one after creating
   * delta is done
   *
