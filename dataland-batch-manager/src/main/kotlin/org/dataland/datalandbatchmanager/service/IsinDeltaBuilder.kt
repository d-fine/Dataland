...
class IsinDeltaBuilder {
  ...
  /**
   * Coverts CSV file to a LEI-ISIN map, while also aggregating all ISINs of a specific LEI into a
   * list
   *
  ...
  private fun parseCsvToGroupedMap(csvFile: File): Map<String, Set<String>> {
    ...
    val csvParser =
      csvMapper.readerFor(Map::class.java).with(csvSchema).readValues<Map<String, String>>(csvFile)
