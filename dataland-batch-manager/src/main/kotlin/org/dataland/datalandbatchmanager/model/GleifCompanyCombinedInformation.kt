...
data class GleifCompanyCombinedInformation(
  ...
  /**
   * function to transform a company information object from GLEIF to the corresponding Dataland
   * object.
   *
   * @return the Dataland companyInformation object with the information of the corresponding GLEIF
   *   object
   */
  override fun toCompanyPost(): CompanyInformation {
    return CompanyInformation(
      ...
      sectorCodeWz = null,
      website = null,
      identifiers = mapOf("Lei" to listOf(gleifCompanyInformation.lei)),
      parentCompanyLei = finalParentLei,
    )
  }

  /**
   * Transform the GLEIF company information to a PATCH object that can be used to update the
  ...
  override fun toCompanyPatch(conflictingIdentifiers: Set<String?>?): CompanyInformationPatch? {
    ...
    if (
      (conflictingIdentifiers != null) &&
        !(conflictingIdentifiers.size == 1 &&
          conflictingIdentifiers.contains(IdentifierType.Lei.value))
    ) {
      return null
    }
  ...
  override fun getNameAndIdentifier(): String {
    return "${gleifCompanyInformation.companyName} " + " (LEI: ${gleifCompanyInformation.lei})"
  }
}
