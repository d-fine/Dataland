...
@Component
class EmailSender(@Autowired private val mailjetClient: MailjetClient) {
  private val logger = LoggerFactory.getLogger(javaClass)

  /**
   * This method checks incoming email objects if their receivers or cc property contains the domain
   * name "@example.com" and suppresses their forwarding to the mailjet client
   *
   * @param email the email to be checked
   */
  fun sendEmailWithoutTestReceivers(email: Email) {
    val receiversWithoutExampleDomains =
      email.receivers.filterNot { it.emailAddress.contains("@example.com") }
    ...
    if (receiversWithoutExampleDomains.isEmpty() && !ccWithoutExampleDomains.isNullOrEmpty()) {
      sendEmail(Email(email.sender, ccWithoutExampleDomains, listOf(), email.content))
    } else if (
      receiversWithoutExampleDomains.isEmpty() && ccWithoutExampleDomains.isNullOrEmpty()
    ) {
      logger.info(
        "No email was sent. After filtering example.com email domain no recipients remain."
      )
    } else {
      sendEmail(email)
    }
  ...
  private fun convertListOfEmailContactsToJoinedString(emailContacts: List<EmailContact>): String {
    return emailContacts.joinToString(", ") { emailContact -> emailContact.emailAddress }
  }
}
