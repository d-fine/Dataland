...
/** --- Non-API model --- A class that stores the sender, receiver and content of an email */
data class Email(
...
fun TransactionalEmail.TransactionalEmailBuilder.integrateEmailIntoTransactionalEmailBuilder(
  ...
  return this.integrateSenderIntoTransactionalEmailBuilder(email.sender)
    ...
    .apply {
      email.cc?.let { ccReceivers -> this.integrateCcIntoTransactionalEmailBuilder(ccReceivers) }
    }
