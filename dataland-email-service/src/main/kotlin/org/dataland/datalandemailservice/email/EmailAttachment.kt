...
data class EmailAttachment(val filename: String, val content: ByteArray, val contentType: String)
...
fun TransactionalEmail.TransactionalEmailBuilder.attachments(
  ...
  attachments.forEach {
    this.attachment(
      Attachment.fromInputStream(ByteArrayInputStream(it.content), it.filename, it.contentType)
    )
  }
  return this
}
