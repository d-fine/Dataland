...
class NotificationEmailFactoriesTest {
  ...
  fun `validate the text of the notification mail for one single data upload`() {
    val singleNotificationEmailFactory =
      SingleNotificationEmailFactory(proxyPrimaryUrl, senderEmail, senderName)
    ...
    assertEquals("New data for $dummyCompanyName on Dataland", mail.content.subject)
  ...
  fun `validate the text of the summary notification mail for several data uploads`() {
    val summaryNotificationEmailFactory =
      SummaryNotificationEmailFactory(proxyPrimaryUrl, senderEmail, senderName)
    ...
    assertTrue(
      summaryNotificationEmailFactory
        .buildEmail(receiverEmail, properties)
        .content
        .textContent
        .contains("24 hours")
    )
