...
class DataRequestedClaimOwnershipEmailFactoryTest {
  ...
  private fun buildTestEmail(reportingPeriods: String): Email {
    ...
    val email =
      DataRequestedClaimOwnershipEmailFactory(
        ...
        .buildEmail(receiverEmail = receiverEmail, properties = properties)
  ...
  fun `validate that the output of the claim ownership mail is correctly formatted`() {
    ...
    assertTrue(
      email.content.htmlContent.contains("href=\"https://$proxyPrimaryUrl/companies/$companyId\"")
    )
  }
  ...
  fun validateTextContent(reportingPeriods: String) {
    ...
    assertTrue(email.content.textContent.contains(" from $companyName for the year$pluralSuffix"))
    assertTrue(email.content.textContent.contains(" $reportingPeriods.\n"))
    assertTrue(
      email.content.textContent.contains("User $requesterEmail sent the following message:\n")
    )
