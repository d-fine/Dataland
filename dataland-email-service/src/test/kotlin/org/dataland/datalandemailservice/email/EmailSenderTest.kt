...
class EmailSenderTest {
  ...
  fun `check if the mail sender works as expected`() {
    ...
    assertThrows<EmailSendException> { emailSender.sendEmailWithoutTestReceivers(email) }
  }
  ...
  fun `check if the suppressing of test receivers works as expected  `() {
    ...
    val email =
      Email(senderContact, listOf(receiversContact), listOf(senderCC), EmailContent("", "", ""))
    val emailSender = callEmailSender()
    assertThrows<EmailSendException> { emailSender.sendEmailWithoutTestReceivers(email) }
  }
  ...
  fun `check if the suppressing of test receivers and carbon copy works as expected`() {
    ...
    val email =
      Email(senderContact, listOf(receiversContact), listOf(senderCC), EmailContent("", "", ""))
    val emailSender = callEmailSender()
    assertDoesNotThrow { emailSender.sendEmailWithoutTestReceivers(email) }
  }
}
