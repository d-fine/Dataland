...
fun getProperties(setOptionalProperties: Boolean): Map<String, String> {
  ...
  if (setOptionalProperties) {
    properties = properties + mapOf("dataTypeDescription" to DATA_TYPE_DESCRIPTION)
  }
  return properties
}
...
fun validateTextContentOfBasicRequestResponseProperties(email: Email) {
  ...
  assertTrue(email.content.textContent.contains("$PROXY_PRIMARY_URL/requests/$DATA_REQUEST_ID"))
}
