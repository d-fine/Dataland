...
class CompanyManagerTest(
  ...
  fun `retrieve companies as a list and check for each company if it can be found as expected`() {
    val allCompaniesInStore =
      testCompanyQueryManager.searchCompaniesByNameOrIdentifierAndGetApiModel("", resultLimit)
    assertTrue(
  ...
  fun `search for name substring to check the ordering of results`() {
    ...
    val responsesStartingWith = searchResponse.takeWhile { it.companyName.startsWith(searchString) }
  ...
  fun `check if the companies search via name and ids sorts the results as expected`() {
    ...
    val sortedCompanyNames =
      testCompanyQueryManager
        .searchCompaniesByNameOrIdentifierAndGetApiModel(searchString = testString, resultLimit)
        .map { it.companyName }
    assertEquals(
    ...
    val otherCompanyNames =
      testCompanyQueryManager
        .searchCompaniesByNameOrIdentifierAndGetApiModel(searchString = "other_name", resultLimit)
        .map { it.companyName }
  ...
  private fun uploadModifiedBaseCompany(
    ...
    val companyInformation =
      baseCompanyInformation.copy(
        ...
        identifiers =
          mapOf(IdentifierType.Isin to listOf(identifier ?: UUID.randomUUID().toString())),
      )
