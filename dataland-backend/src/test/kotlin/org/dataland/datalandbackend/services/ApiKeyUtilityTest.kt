...
class ApiKeyUtilityTest {
  ...
  private val testApiKeyBase64EncodedKeycloakUserId =
    "YzVlZjEwYjEtZGUyMy00YTAxLTkwMDUtZTYyZWEyMjZlZTgz"
  private val testApiKeySecret =
    "da030a8c290f43a022bdb3da59f6ee4d7b5e290997fbdb3a22f1a7ce5a1a51c87f7666a3476a950b"
  ...
  private fun getCrc(keycloakUserIdBase64Encoded: String, apiKeySecret: String): String {
    ...
    return EncodingUtils.calculateCrc32Value(parsedApiKeyWithoutCrc32Value.toByteArray()).toString()
  }
  ...
  fun `check if correct api key can be parsed`() {
    val apiKey =
      testApiKeyBase64EncodedKeycloakUserId + "_" + testApiKeySecret + "_" + testApiKeyCrc32Value
    val parsedApiKey = apiKeyUtility.parseApiKey(apiKey)
    val expectedParsedApiKey = ParsedApiKey(testKeycloakUserId, testApiKeySecret)
  ...
  fun `check if exception thrown if the provided api key does not have the exact number of delimiters`() {
    val apiKeyWithOneDelimiterTooMany =
      testApiKeyBase64EncodedKeycloakUserId +
        "_" +
        testApiKeySecret +
        "_" +
        testApiKeyCrc32Value +
        "_"
    val apiKeyWithOneDelimiterTooFew =
      testApiKeyBase64EncodedKeycloakUserId + "_" + testApiKeySecret + "+" + testApiKeyCrc32Value
    val expectedInvalidInputApiExceptionMessage =
      apiKeyUtility.validateApiKeyDelimitersExceptionMessage
  ...
  fun `check that generated api keys can be parsed`() {
    ...
    assertEquals(
      initialParsedApiKey,
      reParsedApiKey,
      "expecting that initial and re-parsed api key match",
    )
  }
}
