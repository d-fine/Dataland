...
abstract class NumberValidationTestBase {
  ...
  protected fun assertNonDataPointExceptionThrown(value: Any?) {
    val exception = assertThrows<InvalidInputApiException> { validate(value) }
  ...
  protected fun assertDataPointExceptionThrown(dataPoint: BaseDataPoint<*>) {
    val exception = assertThrows<InvalidInputApiException> { validate(dataPoint) }
    Assertions.assertTrue(
      exception.message.contains("as data point value is not handled by number validator")
    )
  }
