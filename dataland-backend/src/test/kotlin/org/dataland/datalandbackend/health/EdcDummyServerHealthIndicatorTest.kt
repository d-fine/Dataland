...
@SpringBootTest
class EdcDummyServerHealthIndicatorTest {
    val edcDummyServerHealthIndicator = EdcDummyServerHealthIndicator()

    @Test
    fun `test if health of the Skyminder Dummy Server has the status DOWN because it is not responding`() {
        val status: String = edcDummyServerHealthIndicator.health().status.code
        assertEquals(
            status, "DOWN",
    ...
    fun `test if the Skyminder Dummy Server container is not responding and therefore the output is false`() {
        val isRunningSkyminderServer: Boolean = edcDummyServerHealthIndicator.isRunningSkyminderServer()
        assertFalse(
