 ...
 * @param companyBaseManager the company base manager service to handle basic information about
 *   companies
 */
@RestController
class CompanyDataController(
  ...
  override fun postCompany(companyInformation: CompanyInformation): ResponseEntity<StoredCompany> {
    logger.info(
      "Received a request to post a company with name '${companyInformation.companyName}'"
    )
  ...
  override fun getCompaniesBySearchString(
    ...
    return ResponseEntity.ok(
      companyQueryManager.searchCompaniesByNameOrIdentifierAndGetApiModel(searchString, resultLimit)
    )
  }

  override fun existsIdentifier(identifierType: IdentifierType, identifier: String) {
    try {
      companyIdentifierRepositoryInterface.getReferenceById(
  ...
  override fun getCompanyIdByIdentifier(
    ...
    try {
      ...
      throw ResourceNotFoundApiException(companyNotFoundSummary, companyNotFoundMessage, e)
    }
  }
  ...
  override fun getCompanyById(companyId: String): ResponseEntity<StoredCompany> {
    return ResponseEntity.ok(
      companyQueryManager.getCompanyApiModelById(
        companyId,
