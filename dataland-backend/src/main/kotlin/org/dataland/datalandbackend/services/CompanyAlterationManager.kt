...
class CompanyAlterationManager(
      ...
      companyIdentifierRepositoryInterface.findAllById(
        identifierMap.flatMap { identifierPair ->
          identifierPair.value.map {
            CompanyIdentifierEntityId(identifierType = identifierPair.key, identifierValue = it)
          }
        }
      )
  ...
  private fun createAndAssociateIdentifiers(
    ...
    try {
      ...
      val cause = ex.cause
      if (
        cause is ConstraintViolationException && cause.constraintName == "company_identifiers_pkey"
      ) {
   ...
   * @return information of the newly created entry in the company data store of Dataland, including
  ...
  fun addCompany(companyInformation: CompanyInformation): StoredCompanyEntity {
    ...
    val savedCompany =
      createStoredCompanyEntityWithoutForeignReferences(companyId, companyInformation)
