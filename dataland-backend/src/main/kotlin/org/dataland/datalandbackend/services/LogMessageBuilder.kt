...
class LogMessageBuilder {
  /**
   * Generates an "access denied" message if a user without the necessary role tries to bypass the
   * QA process
   *
  ...
  val bypassQaDeniedExceptionMessage =
    "You do not have the required permissions to bypass QA checks."

  /**
   * Generates a message to inform that a correlationId has generated been for an operation and
   * potentially logs the companyId and/or dataId in association with this operation to improve
   * traceability.
   *
  ...
  /**
   * Generates a message to inform that a dataset for a specific company has been successfully
   * posted
   *
  ...
  /**
   * Generates a message to inform that a request was received to return all datasets together with
   * meta info for a specific company, data type and reporting period
   *
