...
class CompanyRoleChecker(
   ...
   * @return a Boolean indicating whether the user has the role for the company associated with the
   *   dataset
   */
  fun hasCurrentUserGivenRoleForCompanyOfDataId(dataId: String, role: CompanyRole): Boolean {
    val companyId =
      dataMetaInformationManager.getDataMetaInformationByDataId(dataId).company.companyId
    return hasCurrentUserGivenRoleForCompany(companyId, role)
  }

  /**
   * Checks whether the current user has any company role for the companyId associated with the
   * dataId
   *
   * @param dataId of the dataset to get the associated company for
   * @returns a boolean stating if the user has any company role for the company associated with the
   *   dataId
   */
  ...
  /**
   * Checks whether the patch contains only fields that are allowed to be altered by a
   * non-keycloak-admin-user and throws and error if not
   *
  ...
  /**
   * A user can bypass QA if (a) the user has reviewer-rights (b) the user has the company uploader
   * or company owner role for the company associated with the data upload This function checks
   * these conditions.
   *
  ...
  fun canUserBypassQa(companyId: String): Boolean {
    ...
    val isUserCompanyOwnerOrUploader =
      companyRolesControllerApi.getCompanyRoleAssignments(null, companyUUID, userUUID).any {
        it.companyRole == CompanyRole.CompanyOwner || it.companyRole == CompanyRole.DataUploader
      }
  ...
  fun canUserPatchFieldsForCompany(
    ...
    val companyRoles =
      companyRolesControllerApi.getCompanyRoleAssignments(null, companyUUID, userUUID).map {
        it.companyRole
      }
