...
/**
 * Service to execute access to private data checks to decide whether a user can access a resource
 * or not
 *
  ...
  fun hasUserAccessToPrivateResources(dataId: String): Boolean {
    ...
    return try {
      requestManager.hasAccessToDataset(
        ...
        metaDataEntity.dataType,
        metaDataEntity.reportingPeriod,
    ...
    metaDataEntities.forEach { metaDataEntity ->
      try {
        requestManager.hasAccessToDataset(
          ...
          metaDataEntity.dataType.toString(),
          metaDataEntity.reportingPeriod,
          UUID.fromString(userId),
        )
        return true
