 ...
 * @param dataIdAndHashToEurodatIdMappingRepository the repository to map dataId to document hashes
 *   and document Ids
 * @param externalStorageDataGetter is a util class which contains the necessary storage clients to
 *   be used here
...
class PrivateDataManager(
  ...
  @Autowired
  private val dataIdAndHashToEurodatIdMappingRepository: DataIdAndHashToEurodatIdMappingRepository,
  ...
  private val documentHashesInMemoryStorage =
  ...
  fun processPrivateVsmeDataStorageRequest(
    ...
    val correlationId =
      generateCorrelationId(companyId = companyAssociatedVsmeData.companyId, dataId = null)
    logger.info(
  ...
  private fun sendReceptionMessage(
    ...
    cloudEventMessageHandler.buildCEMessageAndSendToQueue(
      payload,
      MessageType.PrivateDataReceived,
      correlationId,
      ExchangeName.PrivateRequestReceived,
      RoutingKeyNames.privateDataAndDocument,
    )
    logger.info(
  ...
  /**
   * The method removes entries in the in memory storages which are connected to the specified
   * dataId
   *
   * @param dataId the dataId for which connected entries in the in-memory storages should be
   *   removed
  ...
  fun persistMappingInfo(dataId: String, correlationId: String) {
    logger.info("Persisting mapping info for dataId $dataId and correlationId $correlationId")
    val dataIdToJsonMappingEntity =
      DataIdAndHashToEurodatIdMappingEntity(dataId = dataId, hash = "JSON", eurodatId = "JSON")
  ...
  fun persistMetaInfo(dataId: String, correlationId: String): DataMetaInformationEntity {
    logger.info("Persisting meta info for dataId $dataId and correlationId $correlationId")
  ...
  private fun removeDocumentsAndHashesFromInMemoryStorages(
    ...
    documentHashes.keys.forEach { hash -> documentInMemoryStorage.remove(hash) }
   ...
   *
   * @param dataId is the identifier for which all stored data entries in the temporary storage are
   *   filtered
  ...
  fun getPrivateVsmeData(dataId: String, correlationId: String): VsmeData {
    return objectMapper.readValue(
      dataManagerUtils
        .getStorableDataset(
          dataId,
          DataType.of(VsmeData::class.java),
          correlationId,
  ...
  private fun getJsonStringFromCacheOrExternalStorage(
    ...
    return jsonDataInMemoryStorage[dataId]
      ?: dataManagerUtils.getDatasetAsJsonStringFromStorageService(
        dataId,
        correlationId,
        ::getJsonStringFromExternalStorage,
      )
  }
  ...
  fun retrievePrivateDocumentById(
    ...
    return if (inMemoryStoredDocument != null) {
      ...
      DocumentStream(
        ...
        DocumentType.Pdf,
        InputStreamResource(
          externalStorageDataGetter.getBlobFromExternalStorage(eurodatId, correlationId)
        ),
      )
    }
  }
