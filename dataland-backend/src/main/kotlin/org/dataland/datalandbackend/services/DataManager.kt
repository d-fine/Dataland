...
constructor(
  ...
  /**
   * Method to make the data manager add data to a data store, store metadata in Dataland and
   * sending messages to the relevant message queues
   *
  ...
  /**
   * Persists the data meta-information to the database ensuring that the database transaction ends
   * directly after this function returns so that a MQ-Message might be sent out after this function
   * completes
   *
  ...
  fun storeDataSetInTemporaryStoreAndSendMessage(
    ...
    cloudEventMessageHandler.buildCEMessageAndSendToQueue(
      payload,
      MessageType.PublicDataReceived,
      correlationId,
  ...
  private fun getJsonStringFromCacheOrInternalStorage(
    ...
    return publicDataInMemoryStorage[dataId]
      ?: dataManagerUtils.getDatasetAsJsonStringFromStorageService(
        dataId,
        correlationId,
        ::getJsonStringFromInternalStorage,
      )
  }
  ...
  fun deleteCompanyAssociatedDataByDataId(dataId: String, correlationId: String) {
    try {
      ...
      val payload =
        JSONObject(
            mapOf("dataId" to dataId, "bypassQa" to false, "actionType" to ActionType.DeleteData)
          )
          .toString()
      cloudEventMessageHandler.buildCEMessageAndSendToQueue(
        payload,
