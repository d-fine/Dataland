...
class CompanyQueryManager(
  ...
  /**
   * Method to split the return type of method searchCompaniesAndGetApiModel into a list of lists
   * each not exceeeding the given size
   *
  ...
  fun getCompaniesInChunks(
    ...
    return if (filter.searchStringLength == 0) {
      if (areAllDropdownFiltersDeactivated(filter)) {
        companyRepository.getAllCompaniesWithDataset(chunkSize, offset)
      } else {
  ...
  private fun areAllDropdownFiltersDeactivated(filter: StoredCompanySearchFilter): Boolean {
    return (filter.dataTypeFilterSize + filter.sectorFilterSize + filter.countryCodeFilterSize == 0)
  }
  ...
  private fun fetchAllStoredCompanyFields(
    ...
    companiesWithFetchedFields =
      companyRepository.fetchCompanyContactDetails(companiesWithFetchedFields)
    companiesWithFetchedFields =
      companyRepository.fetchCompanyAssociatedByDataland(companiesWithFetchedFields)
