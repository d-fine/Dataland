...
class DataMetaInformationManager(
  ...
  /**
   * Marks the given dataset as the latest dataset for the combination of dataType, company and
   * reporting period Ensures that only one dataset per group has the active status
   */
  ...
  /**
   * The method sets a new dataset active in the metadata database and sets the existing dataset to
   * inactive
   *
   ...
   * @param reportingPeriod if not empty, it filters the requested meta info to a specific reporting
   *   period
  ...
  fun getUserDataMetaInformation(userId: String): List<DataMetaInformationForMyDatasets>? {
    return dataMetaInformationRepositoryInterface.getUserUploadsDataMetaInfos(userId).map {
      DataMetaInformationForMyDatasets.fromDatasetMetaInfoEntityForMyDatasets(it)
    }
  }
}
