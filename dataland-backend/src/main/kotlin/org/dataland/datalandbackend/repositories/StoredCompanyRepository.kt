...
interface StoredCompanyRepository : JpaRepository<StoredCompanyEntity, String> {
  ...
  @Query(
    ...
    value =
      " SELECT filtered_data.company_id AS companyId," +
        ...
        " INNER JOIN " +
        TemporaryTables.TABLE_FILTERED_DROPDOWN_RESULTS +
   ...
   * - searchString: If not empty, only companies that contain the search string in their name are
   *   returned
   * - country Code: If not empty, only companies with a country Code in the given set of country
   *   codes
   * - sector Code: If not empty, only companies with a sector Code in the given set of sector codes
   *   (Prefix-Matches are ordered before Center-Matches, e.g. when searching for "a" Allianz will
   *   come before Deutsche Bank)
   */
  @Query(
    nativeQuery = true,
    value =
      " WITH " +
        ...
        " FROM " +
        TemporaryTables.TABLE_FILTERED_TEXT_RESULTS +
        " INNER JOIN " +
        TemporaryTables.TABLE_FILTERED_DROPDOWN_RESULTS +
   ...
   * - searchFilter.searchString: If not empty, only companies that contain the search string in
   *   their name are returned (Prefix-Matches are ordered before Center-Matches, e.g. when
   *   searching for "a" Allianz will come before Deutsche Bank)
   */
  @Query(
    nativeQuery = true,
    value =
  ...
  fun fetchCompanyAssociatedByDataland(
