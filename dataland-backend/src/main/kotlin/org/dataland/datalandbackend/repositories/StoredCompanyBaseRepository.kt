...
interface StoredCompanyBaseRepository : JpaRepository<StoredCompanyEntity, String> {
   ...
   *   returned (Prefix-Matches are ordered before Center-Matches, e.g. when searching for "a"
   *   Allianz will come before Deutsche Bank)
   */
  @Query(
    nativeQuery = true,
    value =
  ...
  fun getNumberOfCompanies(@Param("searchFilter") searchFilter: StoredCompanySearchFilter): Int

  /**
   * A function for counting the number of companies by various filters (excluding searchString):
   * - dataTypeFilter: If set, only companies with at least one datapoint
   * - countryFilter: If set, only companies with at least one datapoint
   * - sectorFilter: If set, only companies with at least one datapoint of one of the supplied
   *   dataTypes are returned
   */
  @Query(
    nativeQuery = true,
    value = " SELECT COUNT(*)" + " FROM " + TemporaryTables.TABLE_FILTERED_DROPDOWN_RESULTS,
  )
