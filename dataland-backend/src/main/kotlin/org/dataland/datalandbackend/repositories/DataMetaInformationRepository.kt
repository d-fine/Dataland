...
interface DataMetaInformationRepository : JpaRepository<DataMetaInformationEntity, String> {
  ...
  /**
   * A function for searching for dataMetaInformation by dataType and companyId. If companyId is not
   * empty, then only metaInformation for data with that companyId is returned If an invalid
   * companyId is supplied, no results are returned (but no error is thrown) If dataType is not
   * empty, then only metaInformation for data with that dataType is returned If an invalid dataType
   * is supplied, no results are returned (but no error is thrown) If reportingPeriod is not empty,
   * then only metaInformation for data with for that reportingPeriod is returned
   */
  @Query(
  ...
  /**
   * Retrieves the currently active dataset for the given triplet of reporting Period, company and
   * dataType
   */
  @Query(
    "SELECT dataMetaInformation FROM DataMetaInformationEntity dataMetaInformation " +
  ...
  /**
   * Counts the dataset meta information filtered by company ID, data type and if it is currently
   * active
   *
