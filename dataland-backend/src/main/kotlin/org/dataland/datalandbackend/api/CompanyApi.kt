...
interface CompanyApi {
  ...
  @ApiResponses(
    value = [ApiResponse(responseCode = "200", description = "Successfully added company.")]
  )
  ...
  /**
   * A method to retrieve just the basic information about specific companies If the filters are not
   * set, all companies in the data store are returned.
   *
   ...
   * @param countryCodes If set & non-empty, this function only returns companies that have a
   *   country code contained in the set
   * @param sectors If set & non-empty, this function only returns companies that belong to a sector
   *   in the set uploaded by the current user
   * @return basic information about all companies with approved framework data matching the search
   *   criteria
   */
  @Operation(
  ...
  /**
   * A method to retrieve just the number of companies identified by different filters If the
   * filters are not set, all companies in the data store are returned
   *
   * @param searchString string used for substring matching
   * @param dataTypes If set & non-empty, this function only counts companies that have data for a
   *   dataType in the set
   * @param countryCodes If set & non-empty, this function only counts companies that have a country
   *   code contained in the set
   * @param sectors If set & non-empty, this function only counts companies that belong to a sector
   *   in the set uploaded by the current user
  ...
  @GetMapping(value = ["/numberOfCompanies"], produces = ["application/json"])
  @PreAuthorize("hasRole('ROLE_USER')")
   ...
   *
   * @param searchString string used for substring matching in the name and the identifiers of a
   *   company
  ...
  /**
   * A method used to retrieve all available distinct values for framework type, country code &
   * sector to be used by the search UI
   */
  @Operation(
  ...
  @ApiResponses(
    value = [ApiResponse(responseCode = "200", description = "Successfully retrieved values.")]
  )
  @GetMapping(value = ["/meta-information"], produces = ["application/json"])
  @PreAuthorize("hasRole('ROLE_USER')")
  ...
  fun patchCompanyById(
    ...
    @Valid @RequestBody companyInformationPatch: CompanyInformationPatch,
  ): ResponseEntity<StoredCompany>
  ...
  @GetMapping(value = ["/teaser"], produces = ["application/json"])
  ...
  @GetMapping(value = ["/{companyId}/info"], produces = ["application/json"])
  fun getCompanyInfo(
    @PathVariable("companyId") companyId: String
  ...
  @RequestMapping(method = [RequestMethod.HEAD], value = ["/{companyId}"])
  @PreAuthorize("hasRole('ROLE_USER')")
  fun isCompanyIdValid(@PathVariable("companyId") companyId: String)
}
