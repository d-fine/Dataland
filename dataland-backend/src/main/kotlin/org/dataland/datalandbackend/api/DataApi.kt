...
interface DataApi<T> {
  ...
  fun postCompanyAssociatedData(
    @Valid @RequestBody companyAssociatedData: CompanyAssociatedData<T>,
  ...
  @GetMapping(value = ["/{dataId}"], produces = ["application/json"])
  ...
  /**
   * A method to retrieve framework datasets together with their meta info for one specific company
   * identified by its company ID, optionally filtered to one specific reporting period
   *
   * @param companyId identifier of the company in Dataland
   * @param showOnlyActive if set to true, only active datasets will be returned (e.g. no outdated
   *   ones)
   * @param reportingPeriod identifies a specific reporting period (e.g. a year or quarter)
   * @return a list of all datasets for the chosen company and framework, filtered by the chosen
   *   arguments
   */
  @Operation(
