...
interface MetaDataApi {
     ...
     * When bypassQa=true and currentlyActive=true the dataset is marked as non-sourceable without
     * QA review (admin only).
     * When bypassQa=true and currentlyActive=false a non-sourceability object is deactivated (the dataset is now
     * sourceable, admin only).
     * When bypassQa=false and currentlyActive=false the entry enters QA review and is marked as non-sourceable
     * if approved and stays false if rejected.
     * bypassQa=false and currentlyActive=true throws an error because only admins can mark an entry as active
     * immediately.
     * @param nonSourceabilityRequest request body containing dataset dimensions and reason.
     * @param bypassQa when true the entry skips QA review (requires ROLE_ADMIN).
     * @param currentlyActive determines whether a non-sourceability entry is marked as active or not.
     */
    @Operation(
