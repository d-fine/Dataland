...
/**
 * This is a utility class that encapsulates the name of DataTypes. During construction, it is
 * verified that the name actually represents a datatype. But this class is Serialised and
 * Deserialized as a simple string The DataTypeSchemaCustomizer ensures that this property gets
 * displayed as an Enum in the OpenApi spec
 */
data class DataType @JsonCreator constructor(@JsonValue val name: String) {
  companion object {
    private val allowedDataTypes = DataTypesExtractor().getAllDataTypes()

    /**
     * Resolves a string to a valid datatype (ignoring case as that would otherwise sometimes cause
     * issues)
     */
    @JsonCreator
    @JvmStatic
    fun valueOf(input: String): DataType {
      val str = allowedDataTypes.find { it.equals(input, ignoreCase = true) } ?: input
      return DataType(str)
    }
