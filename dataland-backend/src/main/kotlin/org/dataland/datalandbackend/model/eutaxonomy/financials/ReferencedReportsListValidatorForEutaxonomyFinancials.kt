...
/**
 * Annotation Class holding the validation logic for referenced reports list. It checks if the
 * referenced reports list is complete
 */
...
/**
 * Class holding the validation logic for referenced reports list. It checks if the referenced
 * reports list is complete
 */
class ReferencedReportsListValidatorForEutaxonomyFinancials :
  ...
  override fun isValid(
    ...
    return extendedDocumentsFileReferences.all { referencedReportsFileReference.contains(it) }
  }
  ...
  private fun collectFileReferences(data: Any?, fileReferences: MutableList<String>) {
    ...
    when (data) {
      ...
      is Map<*, *> -> {
        data.values.forEach { value -> collectFileReferences(value, fileReferences) }
      }
      else -> {
