...
/**
 * --- Non-API model --- Class for defining the fields needed by the Data Manager to handle data
 * storage
 *
...
data class StorableDataSet(
  ...
  /**
   * Checks the consistency of this dataset retrieved from the store with the metadata stored in the
   * Dataland database. Throws an InternalServerErrorApiException if inconsistencies are detected
   */
  fun requireConsistencyWith(metaDataEntry: DataMetaInformationEntity) {
    if (
      dataType != DataType.valueOf(metaDataEntry.dataType) ||
