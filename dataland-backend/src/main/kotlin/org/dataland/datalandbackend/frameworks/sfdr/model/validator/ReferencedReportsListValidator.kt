...
class ReferencedReportsListConstraintValidator :
    ...
    private fun getExtendedDocumentReferences(dataset: SfdrData): List<String?> {
        return listOf(
            ...
            dataset.environmental?.greenhouseGasEmissions?.enterpriseValueInEUR?.dataSource?.fileReference,
            dataset.environmental?.greenhouseGasEmissions?.totalRevenueInEUR?.dataSource?.fileReference,
            dataset.environmental?.greenhouseGasEmissions?.carbonFootprintInTonnesPerMillionEUREnterpriseValue?.dataSource?.fileReference,
