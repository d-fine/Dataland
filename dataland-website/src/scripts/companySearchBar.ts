...
export function initCompanySearchBar(): void {
  const input = document.querySelector<HTMLInputElement>('#company-search-input');
  const listbox = document.querySelector<HTMLUListElement>('#company-search-listbox');
  const warning = document.querySelector<HTMLParagraphElement>('#company-search-warning');

  if (!input || !listbox || !warning) {
    throw new Error('CompanySearchBar: DOM elements not found');
  }

  const searchInput: HTMLInputElement = input;
  const resultsListbox: HTMLUListElement = listbox;
  const warningEl: HTMLParagraphElement = warning;
  ...
  function renderSuggestions(): void {
    ...
    const items: HTMLLIElement[] = suggestions.map((company, index) => {
      ...
      `;
      li.addEventListener('mousedown', (event: MouseEvent): void => {
