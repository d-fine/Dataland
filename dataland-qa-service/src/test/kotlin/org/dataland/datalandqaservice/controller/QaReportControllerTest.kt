...
@SpringBootTest(classes = [DatalandQaService::class, NoBackendRequestQaReportConfiguration::class])
class QaReportControllerTest(
  ...
  @MockBean private lateinit var metaDataControllerApi: MetaDataControllerApi
  ...
  private fun createMockDataIdForAnSfdrDataset(): String {
    ...
    Mockito.`when`(metaDataControllerApi.getDataMetaInfo(dataId))
      .thenReturn(
        DataMetaInformation(
          dataId = dataId,
  ...
  fun `make sure changing the status of a report as a reviewer other than the reporter throws an exception`() {
    ...
    withReviewerAuthentication { qaMetaInfo = createEmptyQaReport() }

    withReviewerAuthentication("some-other-reviewer") {
  ...
  fun `check that the reviewer user id filter works`() {
    ...
    withReviewerAuthentication("reviewer-1") {
      ...
      assertEquals(
        1,
        qaReportController.getAllQaReportsForDataset(dataId, false, "reviewer-1").body!!.size,
      )
      assertEquals(
        0,
        qaReportController.getAllQaReportsForDataset(dataId, false, "other-reviewer").body!!.size,
      )
    }
  }
  ...
  fun `requesting a qa report for a non matching data type should fail`() {
    ...
    withReviewerAuthentication {
      val ex =
        assertThrows<InvalidInputApiException> { qaReportController.getQaReport(dataId, reportId) }
      assertTrue(
        ex.message.contains(
          "is not associated with data type 'sfdr'," + " but with data type 'some-data-type'"
        ),
