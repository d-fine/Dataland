...
import org.dataland.datalandqaservice.org.dataland.datalandqaservice.utils.PreApprovalServiceUtils
...
class PreApprovalService(
    ...
    fun patchConfig(
        ...
        PreApprovalServiceUtils.validateNoDuplicateExemptFields(patch.exemptFields)
        val current = config
        val merged =
            current.copy(
    ...
    fun putConfig(
        ...
        PreApprovalServiceUtils.validateNoDuplicateExemptFields(newConfig.exemptFields)
        val current = config
        val replaced =
            PreApprovalConfig(
    ...
    private fun validateInput(preApprovalConfig: PreApprovalConfig) {
        val issues =
            PreApprovalServiceUtils.findExemptFieldsOverlappingIndividualThresholdsIssues(preApprovalConfig) +
                PreApprovalServiceUtils.findDecimalAndIntegerThresholdOverlapIssues(preApprovalConfig) +
                PreApprovalServiceUtils.findUnknownDataPointTypeIdIssues(preApprovalConfig, specificationService)
