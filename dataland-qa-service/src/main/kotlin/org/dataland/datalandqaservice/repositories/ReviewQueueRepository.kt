...
interface ReviewQueueRepository : JpaRepository<ReviewQueueEntity, String> {
  /**
   * A function for getting a list of dataset IDs with pending reviews ascendingly ordered by
   * reception time
   */
  @Query(
    "SELECT status.dataId FROM ReviewQueueEntity status " + "ORDER BY status.receptionTime ASC"
  )
