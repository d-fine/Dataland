...
interface QaReportApi<QaReportType> {
  /**
   * A method to store QA reports for a specific dataset on Dataland. The new uploaded QA report is
   * marked as active. All other QA reports for the same dataset are marked as inactive.
   *
  ...
  @Operation(
    ...
    description =
      "The uploaded QA report is added to the database, the generated report id is returned.",
  )
  @ApiResponses(
    value =
      [ApiResponse(responseCode = "200", description = "Successfully added QA report to dataland.")]
  )
  @PostMapping(value = ["/{dataId}/reports"], produces = ["application/json"])
  ...
  /**
   * A method to change the status of a QA report. Inactive QA reports are still shown in the QA
   * report list.
   *
   * @param dataId identifier used to uniquely specify data in the data store
  ...
  @PatchMapping(value = ["/{dataId}/reports/{qaReportId}/status"], produces = ["application/json"])
  @PreAuthorize("hasRole('ROLE_REVIEWER')")
  ...
  @ApiResponses(
    value = [ApiResponse(responseCode = "200", description = "Successfully retrieved QA report.")]
  )
  @GetMapping(value = ["/{dataId}/reports/{qaReportId}"], produces = ["application/json"])
  ...
  @GetMapping(value = ["/{dataId}/reports"], produces = ["application/json"])
  @PreAuthorize("hasRole('ROLE_USER')")
