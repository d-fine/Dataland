...
interface QaApi {
  ...
  @ApiResponses(
    value = [ApiResponse(responseCode = "200", description = "Successfully retrieved dataset IDs.")]
  )
  ...
  @ApiResponses(
    value =
      [
        ApiResponse(
          responseCode = "200",
          description = "Found a QA review status corresponding the id.",
        ),
        ApiResponse(
          responseCode = "404",
          description = "Found no QA review status corresponding the id.",
        ),
      ]
  )
  @GetMapping(value = ["/datasets/{dataId}"], produces = ["application/json"])
  @PreAuthorize(
  ...
  @PostMapping(value = ["/datasets/{dataId}"])
  @PreAuthorize("hasRole('ROLE_REVIEWER')")
  fun assignQaStatus(
