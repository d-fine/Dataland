...
if ls "$keycloak_user_dir"/*-users-*.json &>/dev/null; then
  ...
  imported_expected_technical_users=$(sudo docker exec $keycloak_database_container_name psql -U keycloak -d keycloak -t -c "select count(*) from user_entity where realm_id = 'datalandsecurity' and username in ('data_reader','data_uploader','data_reviewer','data_admin','service-account-dataland-batch-manager','service-account-dataland-automated-qa','service-account-dataland-community-manager')")
