...
if ls "$keycloak_user_dir"/*-users-*.json &>/dev/null; then
  ...
  exported_expected_technical_users=$(sudo docker exec --env USER_PATTERN='"username" : "(data_(reader|uploader|reviewer|premium_user|admin|judge)|service-account-dataland-[a-z-]+)"' "$keycloak_initializer_container_name" bash -c 'grep -E -l "$USER_PATTERN" /keycloak_users/datalandsecurity-users-*.json | wc -l')
  ...
  imported_expected_technical_users=$(sudo docker exec $keycloak_database_container_name psql -U keycloak -d keycloak -t -c "select count(*) from user_entity where realm_id = 'datalandsecurity' and username ~ '^(data_(reader|uploader|reviewer|premium_user|admin|judge)|service-account-dataland-[a-z-]+)$'")
