...
if [[ $RESET_STACK_AND_REPOPULATE == true ]]; then
  ...
  delete_docker_volume_if_existent_remotely "accounting_service_data" "$target_server_url" "$location"
  delete_docker_volume_if_existent_remotely "data_sourcing_service_data" "$target_server_url" "$location"
