...
sudo scp ./healthCheck.sh ubuntu@"$target_server_url":/usr/local/bin/healthCheck.sh
sudo scp ./health-check.service ubuntu@"$target_server_url":/etc/systemd/system/health-check.service
