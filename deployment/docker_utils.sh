function get_container_id_by_service_name () {
  docker_container_service_map=$(sudo docker inspect --format='{{.Id}} "{{index .Config.Labels "com.docker.compose.service"}}";' $(sudo docker ps -aq))
...
function get_services_that_are_not_healthy_from_list () {
  docker_healthcheck=$(sudo docker inspect --format='"{{index .Config.Labels "com.docker.compose.service"}}";{{ if .State.Health}}{{.State.Health.Status}}{{else}}unknown{{end}}' $(sudo docker ps -aq))
  for service in "$@"
  do
...
function get_services_in_docker_compose_profile_that_require_healthcheck () {
  sudo docker compose --profile $1 config --services 2>/dev/null | grep -vx -e "e2etests" -e "eurodat-client" -e "loki"
}
