...
if [[ "$mode" == initialize ]]; then
  ...
  rm $(grep -E -l '"username" : "service-account-dataland-(batch-manager|automated-qa|community-manager|email-service|document-manager|backend)"' "$dataland_realm_folder"/datalandsecurity-users-*.json) || echo "No service account users to be cleaned up"
