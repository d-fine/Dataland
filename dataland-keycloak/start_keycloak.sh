...
if [[ "$mode" == initialize ]]; then
  ...
  rm $(grep -E -l '"username" : "data_(reader|uploader|reviewer|admin)"' "$dataland_realm_folder"/datalandsecurity-users-*.json) || echo "No technical users to be cleaned up"
  ...
  rm $(grep -E -l '"username" : "service-account-dataland-(batch-manager|automated-qa|community-manager)"' "$dataland_realm_folder"/datalandsecurity-users-*.json) || echo "No service account users to be cleaned up"
