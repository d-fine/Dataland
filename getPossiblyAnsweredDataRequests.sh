...
for REQUEST in $REQUESTS; do
  ...
  COMPANY_ID=$(jq -r '.dataRequestCompanyIdentifierValue' <<< "$REQUEST")
...
#  META_INFOS=$(curl -L -s -X GET -H "accept: application/json" -H "Authorization: Bearer $ADMIN_BEARER_TOKEN" "https://$TARGET/api/metadata?companyId=$COMPANY_ID" | jq -c '.[]')
#  for META_INFO in $META_INFOS; do
#    echo "LOL"
#    echo $META_INFO
#  done
