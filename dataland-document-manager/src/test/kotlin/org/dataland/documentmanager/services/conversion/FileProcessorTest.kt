...
class FileProcessorTest(@Autowired val toPdfConverters: List<FileConverter>) {
  ...
  fun `check if list of converts is complete`() {
    ...
    assertEquals(
      expectedToPdfConverters.size,
      toPdfConverters.size,
  ...
  fun `check if an error is thrown if there are file converters with overlapping file extension responsibility`() {
    val exception =
      assertThrows<IllegalArgumentException> {
        FileProcessor(
          listOf(
            object : FileConverter(mapOf("a" to setOf("abc"), "b" to setOf("defg"))) {
              override val logger = LoggerFactory.getLogger("TestLogger")

              override fun convert(file: MultipartFile, correlationId: String) =
                "test".encodeToByteArray()
            },
            object : FileConverter(mapOf("b" to setOf("hijk"), "c" to setOf("lmnop"))) {
              override val logger = LoggerFactory.getLogger("TestLogger")
  ...
  fun `check if no error is thrown if the pdf converter is initialized correctly`() {
    FileProcessor(
      listOf(
        ...
        object : FileConverter(mapOf("c" to setOf("lmnop"))) {
          override val logger = LoggerFactory.getLogger("TestLogger")
  ...
  fun `verifies that an unsupported type is detected`() {
    ...
    assertEquals("File extension json could not be recognized", thrown.message)
  }
}
