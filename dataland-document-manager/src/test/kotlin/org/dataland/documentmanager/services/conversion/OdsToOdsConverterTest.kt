...
class OdsToOdsConverterTest {
  ...
  fun `verify that the converter validates a ods file but do not convert a ods file`() {
    val testInput =
      MockMultipartFile(testFileName, testFileName, mimeType, TestUtils().loadFileBytes(testOds))
  ...
  fun `check that an empty ods file is not validated`() {
    val testInput =
      MockMultipartFile(testFileName, testFileName, mimeType, TestUtils().loadFileBytes(emptyOds))
    assertEquals(mimeType, Tika().detect(testInput.bytes))
    val exception =
