...
class UserRolesChecker(
  ...
  /**
   * Method to check whether the currently authenticated user is company owner or company uploader
   * of any company and therefore has document uploader rights
   *
   * @return a Boolean indicating whether the user is a company owner or uploader of any company or
   *   not
   */
  ...
  fun isCurrentUserCompanyOwnerOrCompanyUploader(): Boolean {
    ...
    val roles =
      companyRolesControllerApi.getCompanyRoleAssignments(null, null, UUID.fromString(userId)).map {
        it.companyRole
      }
