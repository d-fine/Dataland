...
@Component
class FileProcessor(@Autowired val toPdfConverters: List<FileConverter>) {
  ...
  private fun validateNoOverlapInConverterFileExtensions() {
    ...
    require(
      aggregatedConverterFileExtensions.size == aggregatedConverterFileExtensions.distinct().size
    ) {
  ...
  fun processFile(file: MultipartFile, correlationId: String): ByteArray {
    logger.info(
      "Trying to process uploaded file ${file.originalFilename}. (correlation ID: $correlationId)"
    )
