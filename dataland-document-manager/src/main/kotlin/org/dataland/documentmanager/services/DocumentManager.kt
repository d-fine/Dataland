...
class DocumentManager(
  ...
  /**
   * Stores the meta information of a document, saves it temporarily locally and notifies that it
   * can be retrieved for other use
   *
  ...
  fun temporarilyStoreDocumentAndTriggerStorage(document: MultipartFile): DocumentUploadResponse {
    ...
    logger.info(
      "Started temporary storage process for document with correlation ID: $correlationId"
    )
    ...
    cloudEventMessageHandler.buildCEMessageAndSendToQueue(
      ...
      MessageType.DocumentReceived,
      correlationId,
      ExchangeName.DocumentReceived,
    )
    return DocumentUploadResponse(documentMetaInfo.documentId)
  }
