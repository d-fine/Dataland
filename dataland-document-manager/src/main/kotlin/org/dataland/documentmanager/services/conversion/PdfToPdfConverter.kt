...
class PdfToPdfConverter :
  FileConverter(allowedMimeTypesPerFileExtension = mapOf("pdf" to setOf("application/pdf"))) {
  ...
  override fun validateFileContent(file: MultipartFile, correlationId: String) {
    ...
    try {
      ...
      logger.info(
        "Document uploaded with correlation ID: $correlationId cannot be parsed as a PDF, aborting."
      )
