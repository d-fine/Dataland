...
class MessageQueueListener(
  ...
  /**
   * Method that listens to the stored queue and removes data entries from the temporary storage
   * once they have been stored in the persisted database. Further it logs success notification
   * associated containing documentId and correlationId
   *
  ...
  /**
   * Method that listens to the qa_queue and updates the metadata information after successful qa
   * process
   *
  ...
  fun updateDocumentMetaData(
    ...
    messageUtils.rejectMessageOnException {
      val metaInformation: DocumentMetaInfoEntity =
        documentMetaInfoRepository.findById(documentId).get()
