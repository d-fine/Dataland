...
) : FileConverter(allowedMimeTypesPerFileExtension = allowedMimeTypesPerFileExtension) {
  override fun convert(file: MultipartFile, correlationId: String): ByteArray {
    logger.info(
      "Converting ${file.lowercaseExtension()} to a pdf document. (correlation ID: $correlationId)"
    )
    val outputStream = ByteArrayOutputStream()

    val officeManager: OfficeManager = LocalOfficeManager.builder().officeHome(pathToLibre).build()
    officeManager.start()
