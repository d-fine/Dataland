...
abstract class FileConverter(
  ...
  init {
    require(allowedMimeTypesPerFileExtension.isNotEmpty()) {
      "No file extension for conversion is provided."
    }
    require(responsibleFileExtensions.all { it == it.lowercase() }) {
      "Some file extensions are not lowercase."
    }
  }
...
fun MultipartFile.lowercaseExtension() =
  this.originalFilename!!.let { File(it).extension }.lowercase()
