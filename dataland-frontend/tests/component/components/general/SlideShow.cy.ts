import TestSlideShow from "./TestSlideShow.vue";

const SLIDE_DELTA = 440;

describe("Component test for the SlideShow component", () => {
  it("Check if the buttons are working", () => {
    cy.mountWithPlugins(TestSlideShow, {}).then(() => {
      // Check if the left button is disabled initially
      cy.get(leftButtonSelector).should("have.class", "disabled");
      // Click right button and check position
      cy.get(rightButtonSelector).click();
      assertSlidesPosition(1);
      // Click right button again and check position
      cy.get(rightButtonSelector).click();
      assertSlidesPosition(2);
      // Try clicking the right button when it should be disabled and check position
      cy.get(rightButtonSelector).should("have.class", "disabled");
      assertSlidesPosition(2);
      // Click left button and check position
      cy.get(leftButtonSelector).click();
      assertSlidesPosition(1);
      // Click left button again and check position
      cy.get(leftButtonSelector).click();
      assertSlidesPosition(0);
      // Try clicking the left button when it should be disabled and check position
      cy.get(leftButtonSelector).should("have.class", "disabled");
      assertSlidesPosition(0);
      // Click right button and check position
      cy.get(rightButtonSelector).click();
      assertSlidesPosition(1);
    });
  });

  describe("Arrow button states", () => {
    it("Left arrow should be disabled initially and after moving to the first slide", () => {
      cy.mountWithPlugins(TestSlideShow, {}).then(() => {
        // Left arrow should be disabled initially
        cy.get(leftButtonSelector).should("have.class", "disabled");
        // Move to the second slide
        cy.get(rightButtonSelector).click();
        // Move back to the first slide
        cy.get(leftButtonSelector).click();
        // Left arrow should be disabled again
        cy.get(leftButtonSelector).should("have.class", "disabled");
      });
    });

    it("Right arrow should be disabled when we reach the last slide", () => {
      cy.mountWithPlugins(TestSlideShow, {}).then(() => {
        // Click the right button until it gets disabled
        // Assuming the number of slides is known or retrieved dynamically
        const slideCount = 5;
        for (let i = 0; i < slideCount - 1; i++) {
          cy.get(rightButtonSelector).click();
        }
        // Now the right arrow should be disabled
        cy.get(rightButtonSelector).should("have.class", "disabled");
      });
    });
  });

  describe("Event Listeners Cleanup", () => {
    it("should not respond to drag events after component is 'unmounted'", () => {
      cy.mountWithPlugins(TestSlideShow, {}).then(() => {
        // Assuming that slider is draggable initially
        cy.get(".test__slide")
          .first()
          .trigger("pointerdown", { button: 0 })
          .trigger("pointermove", { clientX: 100 })
          .trigger("pointerup", { button: 0 });

        // Capture the position after the initial drag
        let initialPosition: JQuery.Coordinates;
        cy.get(".test__slide")
          .first()
          .invoke("position")
          .then((position) => {
            initialPosition = position;
          });
        // Now, try to drag the slider again
        cy.get(".test__slide")
          .first()
          .trigger("pointerdown", { button: 0 })
          .trigger("pointermove", { clientX: 100 })
          .trigger("pointerup", { button: 0 });

        // Check the position after the second drag attempt - it should not have changed
        cy.get(".test__slide")
          .first()
          .invoke("position")
          .should((position) => {
            expect(position).to.deep.equal(initialPosition); // This checks if position has not changed
          });
      });
    });
  });

  it("Check if dragging is working", () => {
    cy.mountWithPlugins(TestSlideShow, {}).then(() => {
      assertSlidesPosition();
      dragSlideTo(1, leftOffset);
      assertSlidesPosition(1);
      dragSlideTo(2, leftOffset);
      assertSlidesPosition(2);
      dragSlideTo(2, leftOffset + SLIDE_DELTA);
      assertSlidesPosition(1);
      dragSlideTo(1, leftOffset + SLIDE_DELTA);
      assertSlidesPosition(0);
    });
  });

  it("Check if dragging is disabled on large screens", () => {
    cy.mountWithPlugins(TestSlideShow, {}).then(() => {
      cy.viewport(1900, 800);
      assertSlidesPosition();
      dragSlideTo(1, leftOffset);
      assertSlidesPosition();
    });
  });
});

const leftButtonSelector = "button[aria-label='Previous slide']";
const rightButtonSelector = "button[aria-label='Next slide']";
const leftOffset = 10;
const topOffset = 10;

/**
 * Checks that a slide show is centered at a given slide
 * @param centerSlide the index of the slide that is expected to be in the center
 */
function assertSlidesPosition(centerSlide?: number): void {
  const slidesSelector = ".test__slides";
  const expectedTransformValue =
    centerSlide == undefined ? "none" : `matrix(1, 0, 0, 1, ${-SLIDE_DELTA * centerSlide}, 0)`;
  cy.get(slidesSelector).should("have.css", "transform", expectedTransformValue);
}

/**
 * Drags the selected slide to the given position
 * @param slideIndex the slide to drag
 * @param targetX the targets position x value in viewport coordinates
 */
function dragSlideTo(slideIndex: number, targetX: number): void {
  cy.get(".test__slide")
    .eq(slideIndex)
    .trigger("pointerdown", leftOffset, topOffset, { button: 0 })
    .trigger("pointermove", {
      eventConstructor: "MouseEvent",
      clientX: targetX,
      clientY: topOffset,
    })
    .trigger("pointerup", { button: 0 });
}
