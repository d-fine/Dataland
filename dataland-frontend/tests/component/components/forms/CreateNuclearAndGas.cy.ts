...
describe('Component tests for the Nuclear and Gas that test dependent fields', () => {
  ...
  function fillAndValidateOtherSections(): void {
    ...
    cy.get(`div[data-test="nuclearAndGasTaxonomyAlignedRevenueDenominator"] div[data-test="toggleDataPointWrapper"] div[data-test="dataPointToggleButton"]`).should('exist')
        .click();
    subsections.forEach(subsection => {
      cy.get(`div[data-test="nuclearAndGasTaxonomyAlignedRevenueDenominator"] div[data-test="taxonomyAlignedShareDenominator${subsection}"] input[name="mitigationAndAdaptation"]`).clear().type('12');
      cy.get(`div[data-test="nuclearAndGasTaxonomyAlignedRevenueDenominator"] div[data-test="taxonomyAlignedShareDenominator${subsection}"] input[name="mitigation"]`).clear().type('5');
      cy.get(`div[data-test="nuclearAndGasTaxonomyAlignedRevenueDenominator"] div[data-test="taxonomyAlignedShareDenominator${subsection}"] input[name="adaptation"]`).clear().type('8');
    })

    cy.get(`div[data-test="nuclearAndGasTaxonomyNonEligibleRevenue"] div[data-test="toggleDataPointWrapper"] div[data-test="dataPointToggleButton"]`).should('exist')
        .click();
    subsections.forEach(subsection => {
      cy.get(`div[data-test="nuclearAndGasTaxonomyNonEligibleRevenue"] div[data-test="taxonomyNonEligibleShare${subsection}"] input[name="taxonomyNonEligibleShare${subsection}"]`).clear().type('15');
    })
  }
