...
describe('Unit test for the data type sorting in the Constants', () => {
  it('Check for the correct order of the frameworks', () => {
    expect(ALL_FRAMEWORKS_IN_ENUM_CLASS_ORDER).to.have.length(10);
    expect(ALL_FRAMEWORKS_IN_ENUM_CLASS_ORDER[0]).to.equal(DataTypeEnum.IntegrationTesting);
    expect(ALL_FRAMEWORKS_IN_ENUM_CLASS_ORDER[1]).to.equal(DataTypeEnum.Sfdr);
    expect(ALL_FRAMEWORKS_IN_ENUM_CLASS_ORDER[2]).to.equal(DataTypeEnum.EutaxonomyFinancials);
    expect(ALL_FRAMEWORKS_IN_ENUM_CLASS_ORDER[3]).to.equal(DataTypeEnum.EutaxonomyFinancials202673);
    expect(ALL_FRAMEWORKS_IN_ENUM_CLASS_ORDER[4]).to.equal(DataTypeEnum.EutaxonomyNonFinancials);
    expect(ALL_FRAMEWORKS_IN_ENUM_CLASS_ORDER[5]).to.equal(DataTypeEnum.EutaxonomyNonFinancials202673);
    expect(ALL_FRAMEWORKS_IN_ENUM_CLASS_ORDER[6]).to.equal(DataTypeEnum.NuclearAndGas);
    expect(ALL_FRAMEWORKS_IN_ENUM_CLASS_ORDER[7]).to.equal(DataTypeEnum.Lksg);
    expect(ALL_FRAMEWORKS_IN_ENUM_CLASS_ORDER[8]).to.equal(DataTypeEnum.Pcaf);
    expect(ALL_FRAMEWORKS_IN_ENUM_CLASS_ORDER[9]).to.equal(DataTypeEnum.Vsme);
  });
