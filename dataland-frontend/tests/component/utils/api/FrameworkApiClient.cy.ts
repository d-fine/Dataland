...
describe("It should be possible to construct Unified API Clients for every framework", () => {
   for (const frameworkIdentifier of Object.keys(DataTypeEnum)) {
      ...
      it(`Should be possible to construct Unified API Clients for the ${frameworkIdentifier} framework`, async () => {
         const frameworkApiClient = getUnifiedFrameworkDataControllerFromConfiguration(frameworkDataType, new Configuration());
         ...
         void await frameworkApiClient.getFrameworkData("test-data-id")
