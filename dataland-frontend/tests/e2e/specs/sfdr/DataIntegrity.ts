...
function validateFormUploadedData(companyId: string): void {
  ...
  MLDT.getCellValueContainer('Primary Forest And Wooded Land Of Native Species Exposure').should('contain.text', 'Yes');
...
function testRemovingOfHighImpactClimateSector(): void {
  cy.get('div[data-test="applicableHighImpactClimateSector"]:contains("A - AGRICULTURE, FORESTRY AND FISHING")')
    ...
    .click();
  cy.get('div[data-test="applicableHighImpactClimateSector"]:contains("A - AGRICULTURE, FORESTRY AND FISHING")').should(
    'not.exist'
  );
}
