...
describe("As a user I expect my api key will be generate correctly", () => {
  it("successfully generate api key", () => {
    ...
    cy.get("div.custom-dropdown").click();
    cy.get("ul.p-dropdown-items").find('[aria-label="Custom..."]').click();
    cy.get("span#expireTimeWrapper").should("not.exist");
    cy.get('[data-test="expireDataPicker"]').should("be.visible");
    cy.get("div.custom-dropdown").click();
    cy.get("ul.p-dropdown-items").find('[aria-label="No expiry"]').click();
    cy.get("span#expireTimeWrapper").should("contain.text", `The API Key has no defined expire date`);
