...
describe(
  ...
  () => {
    ...
    if (Cypress.env("REALDATA")) {
      teaserCompaniesPermIds = Cypress.env("TEASER_COMPANY_PERM_IDS")
        .toString()
        .split(",");
    }

    before(function () {
      cy.fixture("CompanyInformationWithEuTaxonomyData").then(function (
        companies
      ) {
    ...
    function addCompanyIdToTeaserCompanies(
      companyInformation: CompanyInformation,
      json: any
    ) {
      if (
        (Cypress.env("REALDATA") &&
          teaserCompaniesPermIds.includes({
            permId: getPermId(companyInformation),
          })) ||
      ...
      getKeycloakToken(
        ...
        .then((token) => {
          doThingsInChunks(companiesWithData, chunkSize, (element) => {
            return uploadSingleElementWithRetries(
              "companies",
