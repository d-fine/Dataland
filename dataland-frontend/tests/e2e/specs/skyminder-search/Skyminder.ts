describe("As a developer I want to ensure that the SkyMinder integration works fine", (): void => {
  beforeEach((): void => {
    cy.ensureLoggedIn();
    cy.visitAndCheckAppMount("/companies-only-search");
  });

  it("page should be present", function (): void {
    cy.visitAndCheckAppMount("/companies-only-search");
  ...
  it("Should display an error when I make a Skyminder search with no input", (): void => {
    cy.get('button[name="getSkyminderData"]').click();
    cy.get("body").should("contain", "Sorry");
  });

  it("Should display results when I make a valid Skyminder search", (): void => {
