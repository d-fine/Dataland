describe('User interactive tests', () => {
    ...
    it('Data table should be available upon request by clicking', () => {
        cy.get('button').contains('Get Skyminder by Name').click()
        cy.get('table').should('exist')
    })

    it('Data table should be available upon request by pressing enter for countryCode input', () => {
        cy.get('input[id="countryCode"]').type("{enter}", {force: true})
        cy.get('table').should('exist')
    })

    it('Data table should be available upon request by pressing enter for companyName input', () => {
