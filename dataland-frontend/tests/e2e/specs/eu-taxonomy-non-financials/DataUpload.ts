...
describeIf(
  ...
  function () {
    ...
    it(
      ...
      () => {
        // TODO read through test if it still makes sense
        getKeycloakToken(admin_name, admin_pw).then((token: string) => {
          ...
          return uploadCompanyViaApi(token, dummyCompanyInformation).then((storedCompany) => {
            ...
            cy.intercept({ method: "POST", url: `**/api/data/**`, times: 1 }, (request) => {
              ...
              frontendDocumentHash = "blub"; // TODO fix (assertDefined(data.general?.referencedReports)[TEST_PDF_FILE_NAME]).fileReference;
            }).as("submittedData");
            cy.get('button[data-test="submitButton"]').click();
            ...
            cy.intercept({ method: "POST", url: `**/api/data/**`, times: 1 }, (request) => {
              console.log(request); // TODO dummy-statenent so that request is used for now
              const data = {} as EutaxonomyNonFinancialsData; // TODO assertDefined(request.body as CompanyAssociatedDataEuTaxonomyDataForNonFinancials).data;
