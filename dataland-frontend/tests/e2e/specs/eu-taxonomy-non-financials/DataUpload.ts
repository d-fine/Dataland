...
describeIf(
  ...
  function () {
    ...
    function clickEditButtonAndEditAndValidateChange(companyId: string): Chainable<string> {
      ...
      cy.get(`div[name="revenue"] input[data-test="absoluteShareAmount"]`)
        .eq(0)
        .clear()
        .type(newValueForEligibleRevenueAfterEdit);
      cy.get(`div[name="revenue"] input[data-test="relativeShareInPercent"]`)
        .eq(0)
        .clear()
        .type(newValueForEligibleRevenueAfterEdit);
      cy.get('button[data-test="submitButton"]').click();
