...
describeIf(
  ...
  function () {
     ...
     * if already uploaded reports do exist in the form. TODO this doesnt make sense
    ...
    // TODO Emanuel: Feel free to make the test leaner if you find stuff is unnecessary.
    it(
      ...
      () => {
        getKeycloakToken(uploader_name, uploader_pw).then((token: string) => {
          return uploadCompanyViaApi(token, generateDummyCompanyInformation(testData.companyInformation.companyName))
            .then((storedCompany) => {
              ...
              cy.intercept({ method: "POST", url: `**/api/data/**`, times: 1 }, (request) => {
                ...
                console.log(frontendDocumentHash);
              }).as("postData");
            ...
            .then(() => {
              /* TODO Emanuel: Somehow the value of frontendDocumentHash is empty at this point, even though it is
              successfully set in the intercept of the post request (I made sure via a console.log).
              This seems to be due to cypress asynchrounous behaviour.
              It should be fixable by using .then chains correctly. In that way, you can assure that the variable
              frontendDocumentHash is set before the method validateFrontendAndBackendDocumentHashesCoincede() uses it.
              */
      ...
      () => {
        getKeycloakToken(admin_name, admin_pw).then((token) => {
          return uploadCompanyViaApi(token, generateDummyCompanyInformation("All fields filled")).then(
            (storedCompany) => {
              cy.intercept(`**companies**`).as("getDataForMyDatasetsPage");  // TODO remember to put a slash before "companies" in this line
