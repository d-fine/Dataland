module.exports = (on, config) => {
  ...
  switch (process.env.REALDATA) {
    case "false": {
      console.log(`Processing runAll.ts - REAL_DATA=${process.env.REALDATA}`);
      config.specPattern = ["tests/e2e/specs/runAll.ts"];
      break;
    }
    case "true": {
      console.log(`Processing test for scope real data - REAL_DATA=${process.env.REALDATA}`);
      config.specPattern = [
    ...
    default: {
      console.log(`Defaulting to all specs - REAL_DATA=${process.env.REALDATA}`);
