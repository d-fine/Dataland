...
import {AdditionalCompanyInformationGeneralGeneralFiscalYearDeviationOptions, type AdditionalCompanyInformationData} from "@clients/backend";
 ...
 */
export function generateAdditionalCompanyInformationData(nullProbability = DEFAULT_PROBABILITY): AdditionalCompanyInformationData {
  const dataGenerator = new AdditionalCompanyInformationGenerator(nullProbability);
  return {
    general: {
        general: {
            fiscalYearDeviation: dataGenerator.randomExtendedDataPoint(dataGenerator.valueOrNull(pickOneElement(Object.values(AdditionalCompanyInformationGeneralGeneralFiscalYearDeviationOptions)))),
