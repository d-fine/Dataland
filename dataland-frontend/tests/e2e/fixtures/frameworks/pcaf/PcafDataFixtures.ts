...
import {PcafGeneralCompanyCompanyExchangeStatusOptions, PcafGeneralCompanyMainPcafSectorOptions, PcafGeneralGeneralFiscalYearDeviationOptions, type PcafData} from "@clients/backend";
...
export function generatePcafData(nullProbability = DEFAULT_PROBABILITY): PcafData {
  ...
  return {
    general: {
        general: {
            fiscalYearDeviation: dataGenerator.randomExtendedDataPoint(dataGenerator.valueOrNull(pickOneElement(Object.values(PcafGeneralGeneralFiscalYearDeviationOptions)))),
            fiscalYearEnd: dataGenerator.randomExtendedDataPoint(dataGenerator.randomFutureDate()),
        },
        company: {
            mainPcafSector: dataGenerator.randomExtendedDataPoint(dataGenerator.valueOrNull(pickOneElement(Object.values(PcafGeneralCompanyMainPcafSectorOptions)))),
            companyExchangeStatus: dataGenerator.randomExtendedDataPoint(dataGenerator.valueOrNull(pickOneElement(Object.values(PcafGeneralCompanyCompanyExchangeStatusOptions)))),
        },
    },
    companyValue: {
        listedCompany: {
