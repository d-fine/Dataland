  ...
  const options = {
    fields: [
      ...
      },
      {
        ...
        value: (row: any) => euroGenerator(row.Revenue.totalAmount),
      },
      {
        label: "Total CapEx EURmm",
        value: (row: any) => euroGenerator(row.Capex.totalAmount),
      },
      {
        label: "Total OpEx EURmm",
        value: (row: any) => euroGenerator(row.Opex.totalAmount),
      },
      {
      ...
      },
      {
        ...
        value: (row: any) => percentageGenerator(row.Capex.eligiblePercentage),
      },
      {
        label: "Eligible OpEx",
        value: (row: any) => percentageGenerator(row.Opex.eligiblePercentage),
      },
      {
        label: "Aligned Revenue",
        value: (row: any) => percentageGenerator(row.Revenue.alignedPercentage),
      },
      {
        label: "Aligned CapEx",
        value: (row: any) => percentageGenerator(row.Capex.alignedPercentage),
      },
      {
        label: "Aligned OpEx",
        value: (row: any) => percentageGenerator(row.Opex.alignedPercentage),
      },
      { label: "IS/FS", value: "companyType", default: "IS" },
      {
        label: "NFRD mandatory",
        value: (row: any) => row["Reporting Obligation"],
      },
      {
        label: "Assurance",
        value: (row: any) => {
      ...
      ...stockIndexArray.map((e: any) => {
        return {
          label: humanize(e),
          value: (row: any) => stockIndexValue(row.indices, e),
        };
      }),
      ...identifierTypeArray.map((e: any) => {
        return {
          label: humanize(e),
          value: (row: any) => identifierValue(row.identifiers, e),
        };
      }),
    ],
    delimiter: ";",
