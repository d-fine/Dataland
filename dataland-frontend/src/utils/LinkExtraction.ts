...
// Generated with bash command `curl https://data.iana.org/TLD/tlds-alpha-by-domain.txt | awk 'NR > 1' | grep -v 'XN--' | paste -sd "|" - `
const allTldsRegexGroup =
...
// Segments have 1-63 characters and can contain letters, numbers, underscores, and hyphens.
const validDomainSegment = '(\\p{L}|\\p{N}|[_-]){0,63}';
...
/**
 * Extracts all links from the given text. Does not fully support all link formats and characters.
 * Only supports links that follow commonly used patterns.
 *
 * The following patterns are supported:
 * - https://example.com/asd/asd
 * - https://example.com
 * - www.example.com
 * - example.com
 * - example.com/asd/asd
 ...
 * @returns A segment array containing the text and links in the order they appear in the text.
 */
export function segmentTextIncludingLinks(text: string): Segment[] {
  ...
  while ((match = domainIdentificationRegex.exec(text)) !== null) {
    ...
    result.push({ type: 'link', text: match[0], href: linkHref });
