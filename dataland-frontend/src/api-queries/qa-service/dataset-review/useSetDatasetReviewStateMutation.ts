...
import { type DatasetJudgementResponse, type QaDecision } from '@clients/qaservice';
 ...
 * @param {QaDecision} targetState - State to set on the dataset when finishing judgement.
...
export function useSetDatasetReviewStateMutation(
  ...
  return useMutation({
    mutationFn: async () => {
      ...
      return await apiClient.apiClients.datasetJudgementController.finishJudgement(id, targetState);
    },
    onSuccess: async () => {
