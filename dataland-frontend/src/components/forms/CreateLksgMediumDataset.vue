<template>
  <Card class="col-12 page-wrapper-card p-3">
    <template #title>New Dataset - LksgMedium</template>
    <template #content>
      <div v-if="waitingForData" class="d-center-div text-center px-7 py-4">
        <p class="font-medium text-xl">Loading LksgMedium data...</p>
        <em class="pi pi-spinner pi-spin" aria-hidden="true" style="z-index: 20; color: #e67f3f" />
      </div>
      <div v-else class="grid uploadFormWrapper">
        <div id="uploadForm" class="text-left uploadForm col-9">
          <FormKit
            v-model="companyAssociatedLksgMediumData"
            :actions="false"
            type="form"
            :id="formId"
            :name="formId"
            @submit="postLksgMediumData"
          ...
          >
            ...
            <FormKit type="group" name="data" label="data">
              <FormKit
                type="group"
                v-for="category in lksgmediumDataModel"
                :key="category.name"
                    ...
                    <div class="uploadFormSection grid">
                      ...
                      <div class="col-9 formFields">
                        <FormKit v-for="field in subcategory.fields" :key="field" type="group" :name="subcategory.name">
                          <component
                            v-if="field.showIf(companyAssociatedLksgMediumData.data)"
                            :is="field.component"
                            :label="field.label"
        ...
        <SubmitSideBar>
          ...
          <div v-if="postLksgMediumDataProcessed">
          ...
          <ul>
            <li v-for="category in lksgmediumDataModel" :key="category">
              <ul>
...
import { type CompanyAssociatedDataLksgmediumData, DataTypeEnum, type LksgmediumData } from '@clients/backend';
...
import { lksgmediumDataModel } from '@/frameworks/lksgmedium/UploadConfig';
...
export default defineComponent({
  ...
  data() {
    return {
      formId: 'createLksgMediumForm',
      waitingForData: true,
      dataDate: undefined as Date | undefined,
      companyAssociatedLksgMediumData: {} as CompanyAssociatedDataLksgmediumData,
      lksgmediumDataModel: lksgmediumDataModel,
      route: useRoute(),
      message: '',
      listOfFilledKpis: [] as Array<string>,
      smoothScroll: smoothScroll,
      uploadSucceded: false,
      postLksgMediumDataProcessed: false,
      messageCounter: 0,
      checkCustomInputs,
  ...
  computed: {
    yearOfDataDate: {
      get(): string {
        const currentDate: string = this.companyAssociatedLksgMediumData.data?.general?.masterData?.dataDate;
    ...
    subcategoryVisibility(): Map<Subcategory, boolean> {
      return createSubcategoryVisibilityMap(this.lksgmediumDataModel, this.companyAssociatedLksgMediumData.data);
    },
  },
  props: {
    companyID: {
  ...
  created() {
    ...
    if (dataId && typeof dataId === 'string') {
      void this.loadLksgMediumData(dataId);
    } else {
  ...
  methods: {
    /**
     * Builds an api to get and upload LksgMedium data
     * @returns the api
     */
    buildLksgMediumDataApi(): PublicFrameworkDataApi<LksgmediumData> {
      const apiClientProvider = new ApiClientProvider(assertDefined(this.getKeycloakPromise)());
      const frameworkDefinition = getBasePublicFrameworkDefinition(DataTypeEnum.Lksgmedium);
    ...
    /**
     * Loads the LksgMedium-Dataset identified by the provided dataId and pre-configures the form to contain the data
     * from the dataset
     * @param dataId the id of the dataset to load
     */
    async loadLksgMediumData(dataId: string): Promise<void> {
      this.waitingForData = true;
      const lksgMediumDataControllerApi = this.buildLksgMediumDataApi();
      const dataResponse = await lksgMediumDataControllerApi.getFrameworkData(dataId);
      const lksgMediumResponseData = dataResponse.data;
      this.listOfFilledKpis = getFilledKpis(lksgMediumResponseData.data);
      this.companyAssociatedLksgMediumData = objectDropNull(
        lksgMediumResponseData as ObjectType
      ) as CompanyAssociatedDataLksgMediumData;
      this.waitingForData = false;
    },
    /**
     * Sends data to add LksgMedium data
     */
    async postLksgMediumData(): Promise<void> {
      this.messageCounter++;
      try {
        ...
        const lksgMediumDataControllerApi = this.buildLksgMediumDataApi();
        await lksgMediumDataControllerApi.postFrameworkData(this.companyAssociatedLksgMediumData);
        ...
        this.postLksgMediumDataProcessed = true;
      }
    },
    ...
    /**
     * If the passed field is the first field of the LksgMedium frameworks first category and subcategory, a custom
     * validation message is returned for the "is"-validation for that field.
     * @param field that potentially could be the first field of the LksgMedium framework
    ...
    getValidationMessageForFirstQuestion(field: Field): { is: string } | undefined {
      if (field.name === lksgmediumDataModel[0].subcategories[0].fields[0].name) {
