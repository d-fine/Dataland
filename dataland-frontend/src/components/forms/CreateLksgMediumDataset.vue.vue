<template>
  <Card class="col-12 page-wrapper-card p-3">
    <template #title>New Dataset - LkSG Medium</template>
    <template #content>
      ...
      <div v-else class="grid uploadFormWrapper">
                  ...
                  <template v-if="subcategoryVisibility.get(subcategory) ?? true">
                    ...
                    <div class="col-9 formFields">
                      <FormKit v-for="field in subcategory.fields" :key="field" type="group" :name="subcategory.name">
                        <component
                          v-if="field.showIf(CompanyAssociatedDataLksgmediumData.data)"
                          :is="field.component"
                          :label="field.label"
        ...
        <SubmitSideBar>
          ...
          <ul>
            <li v-for="category in lksgmediumDataModel" :key="category.name">
              <ul>
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      CompanyAssociatedDataLksgmediumData: {} as CompanyAssociatedDataLksgmediumData,
      lksgmediumDataModel,
      route: useRoute(),
      message: '',
  ...
  computed: {
    yearOfDataDate: {
      get(): string {
        const currentDate = this.CompanyAssociatedDataLksgmediumData.data?.general?.masterData?.dataDate;
    ...
    subcategoryVisibility(): Map<Subcategory, boolean> {
      return createSubcategoryVisibilityMap(this.lksgmediumDataModel, this.CompanyAssociatedDataLksgmediumData.data);
    },
  },
  props: {
    companyID: {
  ...
  methods: {
     ...
     * @returns the api
     */
    buildLksgDataApi(): PublicFrameworkDataApi<LksgmediumData> | undefined {
    ...
    async loadLKSGData(dataId: string): Promise<void> {
      ...
      this.CompanyAssociatedDataLksgmediumData = objectDropNull(lksgResponseData);
      this.waitingForData = false;
    },
    /**
    ...
    async postLkSGData(): Promise<void> {
      ...
      try {
        ...
        const lksgDataControllerApi = this.buildLksgDataApi();
        await lksgDataControllerApi!.postFrameworkData(this.CompanyAssociatedDataLksgmediumData);
