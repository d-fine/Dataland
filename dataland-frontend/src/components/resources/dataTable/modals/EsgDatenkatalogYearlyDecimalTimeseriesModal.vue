...
import {
  type EsgDatenkatalogYearlyDecimalTimeseriesDataConfiguration,
  type MappedOptionalDecimal,
  type YearlyTimeseriesData,
} from '@/components/resources/dataTable/conversion/esg-datenkatalog/EsgDatenkatalogYearlyDecimalTimeseriesData';
...
interface EsgDatenkatalogYearlyDecimalTimeseriesDialogData<T extends string> {
  label: string;
  input: YearlyTimeseriesData<MappedOptionalDecimal<T>>;
  options: EsgDatenkatalogYearlyDecimalTimeseriesDataConfiguration<T>;
}

type EsgDatenkatalogDecimalTimeseriesRow<T> = {
  key: T;
  label: string;
  unitSuffix: string;
} & { [key: string]: string };
...
const dialogData = computed<EsgDatenkatalogYearlyDecimalTimeseriesDialogData<KeyList> | undefined>(() => {
  return dialogRef?.value?.data as EsgDatenkatalogYearlyDecimalTimeseriesDialogData<KeyList>;
});

const tableData = computed<EsgDatenkatalogDecimalTimeseriesRow<KeyList>[]>(() => {
  const dialogDataValue = dialogData?.value;
  if (!dialogDataValue) {
    return [];
  }

  const generatedRows: EsgDatenkatalogDecimalTimeseriesRow<KeyList>[] = [];
...
const yearsForDisplay = computed<YearsForDisplay | undefined>(() => {
  ...
  for (const year of Object.keys(dialogDataValue.input.yearlyData)) {
    ...
    if (isNaN(numericalYear)) {
      throw new Error(`Non-numerical year ${numericalYear} received in EsgDatenkatalogYearlyDecimalTimeseriesModal`);
    }
