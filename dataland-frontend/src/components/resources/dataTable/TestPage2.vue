...
<template>
  <MultiLayerDataTable :config="viewConfiguration" :datasets="datasets" style="width: 100%" />
</template>
...
import { EuTaxonomyForNonFinancialsViewModel } from "@/components/resources/frameworkDataSearch/euTaxonomy/EuTaxonomyForNonFinancialsViewModel";
...
const rawEuTaxoDataset: EuTaxonomyDataForNonFinancials = {
  general: {
...
const euTaxoViewModel = new EuTaxonomyForNonFinancialsViewModel(rawEuTaxoDataset);
const demoDataset: MLDTDataset<EuTaxonomyForNonFinancialsViewModel> = {
  headerLabel: "Testing",
  dataset: euTaxoViewModel,
};

const viewConfiguration = convertDataModel(euTaxonomyForNonFinancialsDisplayDataModel);
