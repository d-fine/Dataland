...
class KeycloakTokenManagerTest {
  ...
  fun setup() {
    keycloakTokenManager =
      KeycloakTokenManager(objectMapper, httpClient, "http://keycloak", "client", "secret")
  }
    ...
    `when`(mockCall.execute())
      .thenReturn(
        Response.Builder()
          ...
          .message("")
          .code(200)
          .body("{ \"access_token\":\"${expectedToken}\", \"expires_in\": 0 }".toResponseBody())
          .build()
      )
    `when`(httpClient.newCall(any() ?: Request.Builder().url("http://into.void").build()))
      .thenReturn(mockCall)
