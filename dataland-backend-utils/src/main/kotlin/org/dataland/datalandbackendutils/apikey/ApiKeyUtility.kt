...
/**
 * This class should be used to validate that a given api key has the required format and the
 * correct checksum before a request is sent to /api-keys/validateApiKey to prevent unnecessary
 * traffic
 */
...
class ApiKeyUtility {
  ...
  private fun validateApiKeyDelimiters(receivedApiKey: String) {
    if (
      receivedApiKey.count { it.toString() == "_" } != numberOfUnderscoreDelimitersExpectedInApiKey
    ) {
  ...
  fun parseApiKey(receivedApiKey: String): ParsedApiKey {
    ...
    val (parsedKeycloakUserIdBase64Encoded, parsedApiKeySecret, parsedCrc32Value) =
      receivedApiKey.split("_")
  ...
  private fun getCrc(keycloakUserIdBase64Encoded: String, apiKeySecret: String): String {
    ...
    return calculateCrc32Value(apiKeyWithoutCrc32Value.toByteArray(charset)).toString()
  }
  ...
  /**
   * converts a ParsedApiKey Object (with secret and userId) to an API-Key in the correct parseable
   * format.
   */
  fun convertToApiKey(parsedApiKey: ParsedApiKey): String {
    ...
    return keycloakUserIdBase64Encoded +
      "_" +
      parsedApiKey.apiKeySecret +
      "_" +
      getCrc(
        keycloakUserIdBase64Encoded = keycloakUserIdBase64Encoded,
        apiKeySecret = parsedApiKey.apiKeySecret,
      )
  }
}
