...
class KeycloakTokenManager(
  ...
  fun getAccessToken(): String {
    if (
      currentAccessToken == null ||
        Instant.now().until(currentAccessTokenExpireTime, ChronoUnit.SECONDS) <
          LIFETIME_THRESHOLD_IN_SECONDS
    ) {
      updateAccessToken()
    }
  ...
  private fun updateAccessToken() {
    ...
    val parsedResponseBody =
      objectMapper.readValue(response.body!!.string(), KeycloakAccessTokenResponse::class.java)
    currentAccessToken = parsedResponseBody.accessToken
    currentAccessTokenExpireTime =
      Instant.now() + Duration.ofSeconds(parsedResponseBody.expiresIn.toLong())
