 ...
 *
 * @param okHttpClient must be authenticated to be authorized to fetch the user infos of keycloak
 *   users
 * @param objectMapper required to parse the response into a Kotlin object
 * @param keycloakBaseUrl required for the target url
 * @param userId required to set the path param of the target url to receive the desired user info
 *   from keycloak
...
fun getEmailAddress(
  ...
  val parsedResponseBody =
    objectMapper.readValue(response.body!!.string(), KeycloakUserInfo::class.java)
