...
class KnownErrorControllerAdvice(
  ...
  private fun prepareResponse(
    ...
    val returnedError =
      if (trace) error.copy(stackTrace = ExceptionUtils.getStackTrace(exception)) else error
    return ResponseEntity.status(error.httpStatus)
      .contentType(MediaType.APPLICATION_JSON)
      .body(ErrorResponse(errors = listOf(returnedError)))
  }

  /**
   * Handles HttpMessageNotReadbleException errors. These occur i.e. when the request body cannot be
   * parsed
   */
  ...
  /**
   * Handles AccessDeniedException errors. These occur i.e. if the user does not have permissions to
   * perform an action
   */
  ...
  /**
   * Handles HttpRequestMethodNotSupportedException errors. These occur whenever someone calls an
   * endpoint with a non-implemented HTTP-Method
   */
  @ExceptionHandler(HttpRequestMethodNotSupportedException::class)
  fun handleMethodNotSupportException(
  ...
  /**
   * Handles Invalid Input exceptions. These occur whenever the validators reject input in the sfdr
   * forms
   */
